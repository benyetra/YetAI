...
try:
    from app.services.etl.nfl.statistical_kicker_prediction import calculate_combined_score as enhanced_calculate_combined_score
...
def calculate_combined_score(stats, upcoming_surface=None, upcoming_location=None):
    ...
    if ENHANCED_PREDICTIONS_AVAILABLE:
        try:
            ...
            return enhanced_calculate_combined_score(stats, upcoming_surface, upcoming_location)
...
def calculate_combined_score_legacy(stats, upcoming_surface=None, upcoming_location=None):
    ...
    if red_zone_efficiency > 60:  # Assume 60% is average
        weights['Team Red Zone Efficiency'] *= 0.5  # Reduce the weight if it's above average
    ...
    if third_down_conversion_rate < 40:  # Assume 40% is average
        weights['3rd Down Conversion Rate'] *= 1.5  # Increase weight if below average (more field goal attempts)
    ...
    if redzone_touchdown_pct > 50:  # Assume 50% is average
        weights['Red Zone Touchdown Percentage'] *= 0.7  # Reduce weight if above average (fewer field goal attempts)
    ...
    if redzone_field_goal_pct > 80:  # Assume 80% is good
        weights['Red Zone Field Goal Percentage'] *= 1.2  # Boost if above average (more successful field goals)
    ...
    if temperature < 30:
        weights['Temperature'] *= 1.5  # Increase weight if it's cold (more attempts, harder success)
    ...
    if precipitation_probability > 50:  # Assume 50% precipitation is significant
        weights['Precipitation Probability'] *= 0.5  # Reduce weight if high precipitation (lower success)

    # Boost the weight if playing on a surface or location the kicker has excelled at
    if upcoming_surface:
        surface_boost = get_surface_boost(stats.get('career_surface_stats', []), upcoming_surface)
        weights['Career Surface Stats'] += surface_boost

    if upcoming_location:
        location_boost = get_location_boost(stats.get('career_location_stats', []), upcoming_location)
    ...
    normalized_stats = {
        ...
        'Career Surface Stats': extract_success_rate(stats.get('career_surface_stats', [])),
        'Career Location Stats': extract_success_rate(stats.get('career_location_stats', [])),
        'Career Field Position Stats': extract_success_rate(stats.get('career_field_position_stats', []))
    }

    # Calculate the combined score
    combined_score = sum(normalized_stats[stat] * weights[stat] for stat in normalized_stats)
