"""CLI: ``python -m app.services.etl.wnba.ml_training --stat points --start ...``"""
...
from app.services.etl.wnba.ml_training.run_train_props import run


def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Train WNBA XGB prop model")
    parser.add_argument(
        "--stat",
        required=True,
        choices=WNBA_ML_CONFIG.supported_stats,
    ...
    parser.add_argument(
        "--upload",
        action="store_true",
        help="Upload to s3://yetibets/wnba/ml_models/ after MAE gate passes",
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
