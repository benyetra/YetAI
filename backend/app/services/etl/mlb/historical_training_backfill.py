...
def get_weather_as_of(conn, venue_name, game_date_str, use_api=True):
    ...
    if venue_name in DOMED_STADIUMS:
        ...
        if coords:
            try:
                ...
                logger.debug('Open-Meteo failed for %s %s: %s', venue_name, game_date_str, e)
                w = None
        else:
            w = None
        if w is None:
    ...
    conn.execute(
        ...
        (
            venue_name, game_date_str,
            w['temperature'], w['wind_speed'], w['wind_direction'],
            w['humidity'], w['source'],
        ),
    )
    conn.commit()
...
def get_bullpen_fatigue_as_of(
    conn, team_id, season, as_of_date_str, accurate=False,
):
    ...
    if accurate:
        usage = fetch_team_bullpen_usage(
            team_id, target_date=date.fromisoformat(as_of_date_str),
        )
        if usage:
        ...
        if row:
            ...
            try:
                month_txns = _fetch_team_transactions_month(
                    team_id, cursor.year, cursor.month,
                )
            except Exception as e:
...
def enrich_context_features(
    ...
    weather = get_weather_as_of(
        conn, venue_name, game_date_str, use_api=use_weather_api,
    )
    try:
        ...
        park_factor = get_dynamic_park_factor(
            venue_name, base_park_factor, weather_run_adj,
        )
    except Exception:
        weather_run_adj = 0.0
    ...
    home_bp = get_bullpen_fatigue_as_of(
        conn, home_team_id, season, game_date_str, accurate=accurate_bullpen,
    )
    away_bp = get_bullpen_fatigue_as_of(
        conn, away_team_id, season, game_date_str, accurate=accurate_bullpen,
    )
