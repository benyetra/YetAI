...
def fix_trade_analyzer():
    ...
    try:
        ...
        league_2025.fantasy_user_id = (
            3  # FantasyUser 3 belongs to User 8 (byetra@gmail.com)
        )

        # Make sure the team is in the right league and marked as user team
        team = (
            db.query(FantasyTeam).filter(FantasyTeam.name == "Sir Spanks-A-LOT").first()
        )
        if team:
        ...
        print("\n🔍 Verification:")
        print(
            f"  League {league_2025.id} belongs to FantasyUser {league_2025.fantasy_user_id}"
        )
