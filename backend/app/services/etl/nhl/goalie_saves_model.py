...
def calculate_goalie_recent_form(goalie_id, games_back=10, use_weighted=True):
    ...
    recent_games = (
        ...
        .filter_by(goalie_id=goalie_id)
        .order_by(NHLGoalieActuals.game_date.desc())
        .limit(games_back)
        .all()
    )
    ...
    weights = [0.22, 0.18, 0.14, 0.12, 0.10, 0.08, 0.06, 0.05, 0.03, 0.02][
        : len(recent_games)
    ]
...
def get_team_defensive_rating(team_name):
    ...
    if (
        ...
        and team_stats.last_10_shots_against_per_game > 0
    ):
        return team_stats.last_10_shots_against_per_game

    return (
        team_stats.shots_against_per_game if team_stats.shots_against_per_game else 30.0
    )
...
def get_opponent_shots_per_game(team_name, games_back=10):
    ...
    if (
        ...
        and team_stats.last_10_shots_for_per_game > 0
    ):
...
def get_home_away_save_pct(goalie, is_home):
    ...
    if is_home:
        if (
            goalie.home_games_played
            and goalie.home_games_played >= 3
            and goalie.home_save_pct
        ):
            return goalie.home_save_pct
    else:
        if (
            goalie.away_games_played
            and goalie.away_games_played >= 3
            and goalie.away_save_pct
        ):
...
def get_opponent_offensive_strength(team_name):
    ...
    goals_pg = (
        ...
        if team_stats.last_10_goals_for_per_game
        else team_stats.goals_for_per_game
    )
    shooting_pct = (
        team_stats.last_10_shooting_pct
        if team_stats.last_10_shooting_pct
        else team_stats.shooting_pct
    )
...
def get_opponent_quality_rating(team_name):
    ...
    if overall_strength is None:
        ...
        overall_strength = (
            ...
            if offensive_rating and defensive_rating
            else 50.0
        )
...
def calculate_goalie_vs_team_history(goalie_id, opponent_team_name):
    ...
    if opponent_team_name not in vs_opponent_stats:
        ...
        vs_team_games = (
            db_session.query(NHLGoalieActuals)
            .filter_by(goalie_id=goalie_id, opponent_team_name=opponent_team_name)
            .all()
        )
    ...
    return (
        vs_team_sv_pct,
        len(vs_team_games) if isinstance(vs_team_games, list) else vs_team_games,
        vs_team_adj,
    )
...
def calculate_situational_performance(goalie_id, opponent_team_name, game_date):
    ...
    if opponent and opponent.wins:
        ...
        all_teams = (
            db_session.query(NHLTeamStats)
            .order_by(NHLTeamStats.wins.desc())
            .limit(16)
            .all()
        )
        ...
        if last_game.actual_goals_against >= 4:
            ...
            if len(bad_games) >= 2:  # Need at least 2 bad games in history
                ...
                if len(bounce_backs) >= 2:
                    ...
                    goalie_season_avg = sum(
                        g.actual_save_pct for g in recent_games[:10]
                    ) / min(10, len(recent_games))
...
def calculate_confidence_from_historical_accuracy(
    goalie_id, opponent_team_name, predicted_saves, game_date
):
    """
...
def predict_goalie_saves(
    ...
    if not goalie or goalie.games_played < 3:
        print(
            f"  ⚠ {goalie_name}: Insufficient data (GP: {goalie.games_played if goalie else 0})"
        )
        return None
    ...
    games_7d, games_14d, shots_7d, is_overworked, fatigue_adj = (
        calculate_workload_fatigue(goalie_id, game_date)
    )
    ...
    quality_weighted_sv_pct, avg_opp_strength = calculate_quality_weighted_recent_form(
        goalie_id, games_back=5
    )

    # Also get standard weighted form for comparison
    recent_sv_pct, recent_avg_saves = calculate_goalie_recent_form(
        goalie_id, games_back=5, use_weighted=True
    )
    ...
    if quality_weighted_sv_pct:
        ...
        recent_avg_saves = (
            goalie.saves / goalie.games_played if goalie.games_played > 0 else 0
        )
    ...
    if home_away_sv_pct and recent_sv_pct and goalie.save_pct:
        weighted_sv_pct = (
            (recent_sv_pct * 0.50)
            + (home_away_sv_pct * 0.25)
            + (goalie.save_pct * 0.25)
        )
    ...
    days_rest, rest_category, sv_pct_adj, shots_adj_rest = check_rest_days(
        goalie, game_date
    )
    ...
    opp_goals_pg, opp_shooting_pct, offensive_impact, opp_xgf = (
        get_opponent_offensive_strength(opponent_team_name)
    )
    ...
    fam_adj, games_vs_team, vs_team_sv_pct, vs_team_trend = (
        get_team_familiarity_adjustment(goalie_id, opponent_team_name)
    )
    ...
    opponent_pp_pct, special_teams_adj, pp_opps_pg = get_special_teams_adjustment(
        opponent_team_name
    )

    # NEW FEATURE 5: Goalie vs. team historical performance
    vs_team_sv_pct, games_vs_team, vs_team_adj = calculate_goalie_vs_team_history(
        goalie_id, opponent_team_name
    )
    ...
    avg_shot_dist, high_danger_rate, shot_quality_adj = (
        calculate_shot_quality_adjustment(opponent_team_name)
    )
    ...
    blocks_pg, def_quality_adj = (
        calculate_defensive_quality_impact(goalie_team_name)
        if goalie_team_name
        else (None, 1.0)
    )
    ...
    season_trend, monthly_perf, trend_adj = analyze_season_long_trends(
        goalie_id, game_date
    )
    ...
    is_division, is_playoff_team, bounce_back_adj, situation_adj = (
        calculate_situational_performance(goalie_id, opponent_team_name, game_date)
    )
    ...
    opp_rolling_goals, opp_rolling_shots, opp_rolling_sh_pct, rolling_form_adj = (
        get_opponent_rolling_form(opponent_team_name)
    )
    ...
    if opp_rolling_shots:
        opponent_shots = (opponent_shots * 0.40) + (
            opp_rolling_shots * 0.60
        )  # Weight recent form 60%
    ...
    base_confidence = calculate_confidence_from_historical_accuracy(
        goalie_id, opponent_team_name, predicted_saves, game_date
    )
    ...
    return {
        ...
        # Opponent quality
        "avg_opponent_strength_L5": (
            round(avg_opp_strength, 1) if avg_opp_strength else None
        ),
        # NEW: Special teams
        "opponent_pp_pct": round(opponent_pp_pct, 3) if opponent_pp_pct else None,
        "special_teams_adjustment": (
            round(special_teams_adj, 3) if special_teams_adj != 1.0 else None
        ),
        # NEW: Goalie vs. team history
        "goalie_career_vs_opponent_sv_pct": (
            round(vs_team_sv_pct, 3) if vs_team_sv_pct else None
        ),
        "career_games_vs_opponent": games_vs_team,
        "vs_team_historical_adjustment": (
            round(vs_team_adj, 3) if vs_team_adj != 1.0 else None
        ),
        # NEW: Shot quality metrics
        "opponent_avg_shot_distance": (
            round(avg_shot_dist, 1) if avg_shot_dist else None
        ),
        "opponent_high_danger_rate": (
            round(high_danger_rate, 2) if high_danger_rate else None
        ),
        "shot_quality_adjustment": (
            round(shot_quality_adj, 3) if shot_quality_adj != 1.0 else None
        ),
        # NEW: Defensive quality
        "team_blocked_shots_pg": round(blocks_pg, 1) if blocks_pg else None,
        "defensive_quality_adjustment": (
            round(def_quality_adj, 3) if def_quality_adj != 1.0 else None
        ),
        ...
        "bounce_back_adjustment": (
            round(bounce_back_adj, 3) if bounce_back_adj != 1.0 else None
        ),
        "situational_adjustment": (
            round(situation_adj, 3) if situation_adj != 1.0 else None
        ),
        # IMPROVEMENT #5: Opponent rolling form
        "opponent_rolling_goals_pg": (
            round(opp_rolling_goals, 2) if opp_rolling_goals else None
        ),
        "opponent_rolling_shots_pg": (
            round(opp_rolling_shots, 1) if opp_rolling_shots else None
        ),
        "opponent_rolling_form_adjustment": (
            round(rolling_form_adj, 3) if rolling_form_adj != 1.0 else None
        ),
    }
...
def backtest_model(days_back=30):
    ...
    recent_actuals = (
        db_session.query(NHLGoalieActuals)
        .filter(NHLGoalieActuals.game_date >= datetime(2025, 9, 1).date())
        .order_by(NHLGoalieActuals.game_date.desc())
        .limit(200)
        .all()
    )
    ...
    for p in sorted_preds[:5]:
        ...
        print(
            f"{result} {p['goalie']:20} vs {p['opponent']:20} Pred: {p['predicted']:.1f} Actual: {p['actual']} (Error: {p['error']:.1f})"
        )
    ...
    if high_conf:
        ...
        print(
            f"High Confidence Picks (>28 or <24 saves): {high_conf_acc:.1f}% accuracy ({len(high_conf)} bets)"
        )
...
def main():
    ...
    total_goalies = (
        db_session.query(NHLGoalie).filter(NHLGoalie.games_played > 0).count()
    )
    ...
    top_goalie = (
        ...
        .filter(NHLGoalie.games_played >= 10)
        .order_by(NHLGoalie.saves.desc())
        .first()
    )
    ...
    if top_goalie:
        ...
        high_shot_team = (
            db_session.query(NHLTeamStats)
            .filter(NHLTeamStats.games_played > 0)
            .order_by(NHLTeamStats.shots_for_per_game.desc())
            .first()
        )
