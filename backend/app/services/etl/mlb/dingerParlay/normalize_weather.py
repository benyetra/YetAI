...
def normalize_weather(raw_csv: str, output_csv: str):
    ...
    for required_col in ("park", "date", "tavg", "wspd"):
        if required_col not in df.columns:
            print(f"Error: raw weather CSV must contain column '{required_col}'.", file=sys.stderr)
            sys.exit(1)
    ...
    if missing_map > 0:
        print(f"Info: {missing_map} parks not found in stadium_to_park mapping. Using original stadium names as park_id.", file=sys.stderr)
