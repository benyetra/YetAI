...
def get_nhl_events_from_odds_api():
    ...
    try:
        ...
        for i, event in enumerate(all_events[:5]):
            logging.info(
                f"  Event {i+1}: {event.get('away_team')} @ {event.get('home_team')} at {event.get('commence_time')}"
            )
        ...
        for bookmaker in data.get("bookmakers", []):
            ...
            for market in bookmaker.get("markets", []):
                ...
                for goalie_name, line_data in outcomes_by_goalie.items():
                    if (
                        "line" in line_data
                        and "over_odds" in line_data
                        and "under_odds" in line_data
                    ):
                        ...
                        logging.info(
                            f"DraftKings line for {goalie_name}: O/U {line_data['line']} ({line_data['over_odds']}/{line_data['under_odds']})"
                        )
...
def fetch_all_fanatics_lines(games):
    ...
    for game in games:
        ...
        if not event_id:
            logging.warning(
                f"Could not match NHL game to Odds API: {nhl_away} @ {nhl_home} (tried: {matchup_key})"
            )
            continue
    ...
    try:
        with open(filename, "w") as f:
            ...
            for name, data in FANATICS_LINES.items():
                f.write(
                    f"{name} | {data['line']} | {data['over_odds']} | {data['under_odds']}\n"
                )
...
def generate_predictions_for_today():
    ...
    for game in games:
        ...
        if home_starter:
            ...
            home_goalies_to_check = [
                {"playerId": g["id"], "name": {"default": g["name"]}}
                for g in home_goalies_list
            ]
            ...
            if idx == 0 or not home_starter:  # Show all if no starter announced
                ...
                if pred:
                    ...
                    if pred.get("home_away_sv_pct"):
                        ...
                        print(
                            f"      Goalie SV% ({location}): {pred['home_away_sv_pct']}"
                        )
                    ...
                    if rest_cat == "back_to_back":
                        ...
                        print(
                            f"      ✅ WELL RESTED ({pred.get('days_rest')} days - performance boost)"
                        )
                    elif rest_cat == "extended_rest":
                        print(
                            f"      ⏸️  EXTENDED REST ({pred.get('days_rest')} days - may be rusty)"
                        )

                    # Show momentum indicators
                    if pred.get("is_hot_streak"):
                        print(
                            f"      🔥 HOT STREAK (3 games above average - SV% boost)"
                        )
                    elif pred.get("is_cold_streak"):
                        print(
                            f"      ❄️  COLD STREAK (3 games below average - SV% penalty)"
                        )
                    ...
                    if opp_gpg > 3.5:
                        ...
                        print(
                            f"      ⚔️  Elite Offense: {opp_gpg} GPG{xg_str}, {opp_sh_pct*100:.1f}% shooting (tougher saves)"
                        )
                    elif opp_gpg < 2.5:
                        xg_str = f" ({opp_xgf:.1f} xG)" if opp_xgf else ""
                        print(
                            f"      🛡️  Weak Offense: {opp_gpg} GPG{xg_str}, {opp_sh_pct*100:.1f}% shooting (easier saves)"
                        )
                    ...
                    if venue_games >= 2 and venue_sv_pct:
                        ...
                        if venue_diff > 1.0:
                            print(
                                f"      🏟️  Strong at this venue: .{int(venue_sv_pct*1000)} SV% in {venue_games}G (+ boost)"
                            )
                        elif venue_diff < -1.0:
                            print(
                                f"      🏟️  Struggles at this venue: .{int(venue_sv_pct*1000)} SV% in {venue_games}G (- penalty)"
                            )
                    ...
                    if games_vs >= 2 and vs_sv_pct:
                        ...
                        if vs_diff > 1.5:
                            ...
                            print(
                                f"      🎯 Dominates this opponent: .{int(vs_sv_pct*1000)} SV% in {games_vs}G{trend_str}"
                            )
                        elif vs_diff < -1.5:
                            trend_str = " 📉" if vs_trend == "declining" else ""
                            print(
                                f"      🎯 Struggles vs this opponent: .{int(vs_sv_pct*1000)} SV% in {games_vs}G{trend_str}"
                            )
                    ...
                    if injury_status == "day-to-day":
                        ...
                        print(
                            f"      🩹 Returning from injury ({games_since_injury} games back)"
                        )
                    ...
                    if is_overworked:
                        print(
                            f"      😓 OVERWORKED ({games_7d} games in 7 days - fatigue risk)"
                        )
                    elif games_7d >= 3:
                    ...
                    if avg_opp_strength:
                        if avg_opp_strength >= 65:
                            print(
                                f"      💎 Faced elite teams recently (L5 avg: {avg_opp_strength:.0f}/100)"
                            )
                        elif avg_opp_strength <= 35:
                            print(
                                f"      ⚠️  Faced weak teams recently (L5 avg: {avg_opp_strength:.0f}/100)"
                            )
                    ...
                    if opp_pp_pct and opp_pp_pct >= 0.25:
                        print(
                            f"      ⚡ Elite PP opponent ({opp_pp_pct:.1%} - expect more shots)"
                        )
                    elif opp_pp_pct and opp_pp_pct < 0.15:
                        print(
                            f"      📉 Weak PP opponent ({opp_pp_pct:.1%} - fewer quality chances)"
                        )
                    ...
                    if vs_team_sv_pct and career_games >= 3:
                        ...
                        if diff >= 3:
                            print(
                                f"      🎯 Dominant vs {pred['opponent_team']} ({vs_team_sv_pct:.3f} SV%, +{diff:.1f}%)"
                            )
                        elif diff <= -3:
                            print(
                                f"      ⚠️  Struggles vs {pred['opponent_team']} ({vs_team_sv_pct:.3f} SV%, {diff:.1f}%)"
                            )
                    ...
                    if avg_shot_dist:
                        if avg_shot_dist < 25:
                            print(
                                f"      🎯 Opponent takes close shots (avg {avg_shot_dist:.1f} ft - tough)"
                            )
                        elif avg_shot_dist > 32:
                            print(
                                f"      🛡️ Opponent shoots from perimeter (avg {avg_shot_dist:.1f} ft - easier)"
                            )
                    ...
                    if blocks_pg and blocks_pg >= 18:
                        print(
                            f"      🛡️ Elite shot blocking team ({blocks_pg:.1f} blocks/game - helps goalie)"
                        )
                    elif blocks_pg and blocks_pg < 13:
                        print(
                            f"      ⚠️  Poor shot blocking ({blocks_pg:.1f} blocks/game - more work for goalie)"
                        )
                    ...
                    if season_trend == "strong_improvement":
                        print(
                            f"      📈 STRONG UPWARD TREND (Getting better as season progresses)"
                        )
                    elif season_trend == "improving":
                        print(
                            f"      📊 Improving trend (Recent form better than early season)"
                        )
                    elif season_trend == "declining":
                        print(f"      📉 DECLINING TREND (Performance dropping off)")
                    elif season_trend == "slight_decline":
                        print(
                            f"      ⚠️  Slight decline (Recent form below early season)"
                        )
                    ...
                    if bounce_back_adj and bounce_back_adj > 1.01:
                        print(
                            f"      💪 BOUNCE-BACK GAME (Strong history after bad performances)"
                        )
                    ...
                    if opp_rolling_goals:
                        if opp_rolling_goals >= 4.0:
                            print(
                                f"      🔥 Opponent HOT STREAK ({opp_rolling_goals:.1f} GPG last 10 - tough test)"
                            )
                        elif opp_rolling_goals <= 2.0:
                            print(
                                f"      ❄️  Opponent COLD ({opp_rolling_goals:.1f} GPG last 10 - favorable)"
                            )
                    ...
                    print(
                        f"      Confidence: {pred['confidence']:.1f}% (Historically calibrated)"
                    )
                    ...
                    if draftkings_line:
                        ...
                        print(
                            f"      📊 DraftKings Line: O/U {line} ({over_odds}/{under_odds})"
                        )
                    ...
                    pred["edge"] = edge
                    pred["edge_saves"] = (
                        edge_saves if "edge_saves" in locals() else None
                    )
                    pred["team"] = "home"
    ...
    if high_edge_bets:
        ...
        for bet in high_edge_bets:
            ...
            print(
                f"  Prediction: {bet['predicted_saves']} saves | {bet['recommendation']:20} | Confidence: {bet['confidence']}%"
            )
...
def save_predictions_to_db(predictions, games_map):
    ...
    for pred in primary_predictions:
        ...
        existing = (
            ...
            .filter_by(game_date=pred["game_date"], goalie_id=pred["goalie_id"])
            .first()
        )
        ...
        pred_data = {
            ...
            "expected_goals_against": pred.get(
                "opponent_xgf_per_game"
            ),  # Using xGF as proxy for xGA
            "opponent_pp_pct": pred.get("opponent_pp_pct"),
            "goalie_career_vs_opponent_sv_pct": pred.get(
    ...
    if primary_predictions:
        ...
        old_predictions = (
            db_session.query(NHLGoaliePredictions).filter_by(game_date=today).all()
        )
        deleted_count = 0
    ...
    if ODDS_API_KEY:
        ...
        if not lines_loaded:
            ...
            if choice == "1":
                ...
                print(
                    "   Set ODDS_API_KEY environment variable to fetch lines automatically"
                )
                print()
