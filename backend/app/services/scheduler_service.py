...
class SchedulerService:
    ...
    def add_task(self, name: str, func: Callable, interval_seconds: int, 
                 max_errors: int = 5, enabled: bool = True):
                ...
                try:
                    ...
                    if games:
                        ...
                        await cache_service.set_odds(
                            sport.value, "us", "h2h,spreads,totals", "american", 
                            result, expire_seconds=7200
                        )
                        
                        updated_count += 1
                        logger.info(f"Updated odds for {sport.value}: {len(games)} games")
                    
                except Exception as e:
                    logger.error(f"Failed to update odds for {sport.value}: {e}")
                    continue
        
        logger.info(f"Completed popular sports odds update: {updated_count} sports updated")
    ...
    async def _update_live_games(self):
        ...
        try:
            ...
            if games:
                ...
                await cache_service.set(live_games_key, result, expire_seconds=1800)  # 30 minutes
            ...
            for sport in sports_to_check:
                try:
                    ...
                    if scores:
                        ...
                        updated_count += 1
                        logger.info(f"Updated scores for {sport.value}: {len(scores)} games")
