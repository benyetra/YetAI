...
class SimplifiedSleeperService:
    ...
    async def connect_sleeper_account(self, user_id: int, sleeper_username: str, db: Session) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            logger.info(f"Connected Sleeper account for user {user_id}: {sleeper_username} -> {sleeper_user_id}")
    ...
    async def sync_all_league_history(self, user_id: int, db: Session) -> Dict[str, Any]:
        """
        ...
        for year in range(2020, current_year + 1):
            try:
                ...
                year_leagues = await self._get_user_leagues_for_season(sleeper_user_id, year)
                
                for league_data in year_leagues:
                    # Check if league already exists
                    existing_league = db.query(SleeperLeague).filter(
                        and_(
                            SleeperLeague.user_id == user_id,
                            SleeperLeague.sleeper_league_id == league_data['league_id'],
                            SleeperLeague.season == year
                        )
                    ).first()
                    
                    if existing_league:
                        logger.debug(f"League {league_data['league_id']} already exists for season {year}")
                        continue
    ...
    async def sync_all_rosters(self, user_id: int, db: Session) -> Dict[str, Any]:
        ...
        user_leagues = db.query(SleeperLeague).filter(SleeperLeague.user_id == user_id).all()
        
        total_rosters_synced = 0
        
        for league in user_leagues:
            try:
                logger.info(f"Syncing rosters for league {league.sleeper_league_id} ({league.name})")
                ...
                for roster_data in rosters_data:
                    ...
                    existing_roster = db.query(SleeperRoster).filter(
                        and_(
                            SleeperRoster.league_id == league.id,
                            SleeperRoster.sleeper_roster_id == str(roster_data['roster_id'])
                        )
                    ).first()
                    
                    if existing_roster:
                        # Update existing roster
                        existing_roster.wins = roster_data.get('settings', {}).get('wins', 0)
                        existing_roster.losses = roster_data.get('settings', {}).get('losses', 0)
                        existing_roster.ties = roster_data.get('settings', {}).get('ties', 0)
                        existing_roster.points_for = float(roster_data.get('settings', {}).get('fpts', 0))
                        existing_roster.points_against = float(roster_data.get('settings', {}).get('fpts_against', 0))
                        existing_roster.waiver_position = roster_data.get('settings', {}).get('waiver_position')
                        ...
                        new_roster = SleeperRoster(
                            ...
                            points_for=float(roster_data.get('settings', {}).get('fpts', 0)),
                            points_against=float(roster_data.get('settings', {}).get('fpts_against', 0)),
                ...
                logger.error(f"Failed to sync rosters for league {league.sleeper_league_id}: {str(e)}")
                continue
        ...
        try:
            ...
            for player_id, player_data in players_data.items():
                try:
                    ...
                    existing_player = db.query(SleeperPlayer).filter(
                        SleeperPlayer.sleeper_player_id == player_id
                    ).first()
                    ...
                    if existing_player:
                        ...
                        existing_player.fantasy_positions = player_data.get('fantasy_positions', [])
                        existing_player.status = player_data.get('status')
                        existing_player.injury_status = player_data.get('injury_status')
                        existing_player.depth_chart_position = player_data.get('depth_chart_position')
                        existing_player.depth_chart_order = player_data.get('depth_chart_order')
                        ...
                        existing_player.fantasy_data_id = str(player_data.get('fantasy_data_id')) if player_data.get('fantasy_data_id') else None
                        existing_player.rotoworld_id = str(player_data.get('rotoworld_id')) if player_data.get('rotoworld_id') else None
                        existing_player.rotowire_id = str(player_data.get('rotowire_id')) if player_data.get('rotowire_id') else None
                        ...
                        new_player = SleeperPlayer(
                            ...
                            rotoworld_id=str(player_data.get('rotoworld_id')) if player_data.get('rotoworld_id') else None,
            ...
            logger.info(f"NFL player sync complete: {synced_count} new, {updated_count} updated")
    ...
    async def full_sync_workflow(self, user_id: int, sleeper_username: str, db: Session) -> Dict[str, Any]:
        """
        Complete workflow: Connect account -> Sync leagues -> Sync rosters -> Sync players
        """
        try:
            logger.info(f"Starting full Sleeper sync for user {user_id} with username {sleeper_username}")
            
            # Step 1: Connect account
            connect_result = await self.connect_sleeper_account(user_id, sleeper_username, db)
    ...
    async def _get_user_leagues_for_season(self, sleeper_user_id: str, season: int) -> List[Dict[str, Any]]:
        """Get all leagues for a user for a specific season"""
        try:
            async with httpx.AsyncClient() as client:
                response = await client.get(f"{self.base_url}/user/{sleeper_user_id}/leagues/nfl/{season}")
                response.raise_for_status()
    ...
    async def _get_all_players(self) -> Dict[str, Any]:
        ...
        if (self._players_cache_time and 
            (now - self._players_cache_time).seconds < 86400 and
            self._players_cache):
        ...
        try:
            async with httpx.AsyncClient(timeout=60.0) as client:
                ...
                if response.status_code == 304:
                    logger.info("NFL players data not modified (304), using cached data")
