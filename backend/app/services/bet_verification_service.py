...
from app.models.database_models import Bet, ParlayBet, Game, BetHistory, BetStatus, BetType, GameStatus
...
class BetVerificationService:
    ...
    async def verify_all_pending_bets(self) -> Dict:
        ...
        try:
            ...
            if not pending_bets and not pending_parlays:
                ...
                return {"success": True, "message": "No pending bets", "verified": 0, "settled": 0}
            
            logger.info(f"Found {len(pending_bets)} pending bets and {len(pending_parlays)} pending parlays")
            ...
            if not game_ids:
                ...
                return {"success": True, "message": "No games to verify", "verified": 0, "settled": 0}
            ...
            if not game_results:
                ...
                return {"success": True, "message": "No completed games", "verified": 0, "settled": 0}
            ...
            for bet in pending_bets:
                if bet.game_id in game_results:
                    try:
                        result = await self._verify_single_bet(bet, game_results[bet.game_id])
                        if result:
                            await self._settle_bet(bet, result)
                            verified_bets += 1
                            if result.status in [BetStatus.WON, BetStatus.LOST, BetStatus.PUSHED]:
                                settled_bets += 1
            ...
            for parlay in pending_parlays:
                try:
                    ...
                    if parlay_result:
                        ...
                        verified_bets += 1
                        if parlay_result.status in [BetStatus.WON, BetStatus.LOST, BetStatus.PUSHED]:
                            settled_bets += 1
                except Exception as e:
                    logger.error(f"Error verifying parlay {parlay.id}: {e}")
            
            logger.info(f"Verification complete: {verified_bets} bets verified, {settled_bets} settled")
    ...
    async def _get_pending_parlays(self) -> List[ParlayBet]:
        ...
        db = SessionLocal()
        try:
            parlays = db.query(ParlayBet).filter(
    ...
    async def _get_game_results(self, game_ids: List[str]) -> Dict[str, GameResult]:
        ...
        try:
            for game_id in game_ids:
                ...
                if game and game.status == GameStatus.FINAL and game.home_score is not None and game.away_score is not None:
                    ...
                    logger.info(f"Found completed game in database: {game.away_team} @ {game.home_team} - {game.away_score}-{game.home_score}")
                else:
        ...
        logger.info(f"Found {len(game_results)} games locally, checking API for {len(remaining_game_ids)} remaining games")
                ...
                for score in scores:
                    if score.id in sport_game_ids and score.completed:
                        game_result = GameResult(
                            ...
                            winner=self._determine_winner(score.home_score, score.away_score),
                            is_final=score.completed,
                            total_score=(score.home_score or 0) + (score.away_score or 0)
                        )
    ...
    async def _verify_single_bet(self, bet: Bet, game_result: GameResult) -> Optional[BetResult]:
        """
    ...
    def _verify_moneyline_bet(self, bet: Bet, game_result: GameResult) -> BetResult:
        ...
        is_home_bet = (game_result.home_team.lower() in selection_lower or 
                      selection_lower in game_result.home_team.lower())
        is_away_bet = (game_result.away_team.lower() in selection_lower or 
                      selection_lower in game_result.away_team.lower())
    ...
    async def _verify_parlay(self, parlay: ParlayBet, game_results: Dict[str, GameResult]) -> Optional[BetResult]:
        ...
        if lost_legs > 0:
            ...
            adjusted_payout = self._calculate_adjusted_parlay_payout(parlay, leg_results)
            return BetResult(
                bet_id=parlay.id,
                status=BetStatus.WON,
            ...
            logger.error(f"Unexpected parlay state: {won_legs} won, {lost_legs} lost, {pushed_legs} pushed, {pending_legs} pending")
            return None
    
    def _calculate_adjusted_parlay_payout(self, parlay: ParlayBet, leg_results: List[BetResult]) -> float:
        """Calculate adjusted payout for parlay with pushed legs removed"""
        winning_legs = [result for result in leg_results if result.status == BetStatus.WON]
    ...
    async def _settle_bet(self, bet: Bet, result: BetResult) -> None:
        ...
        try:
            ...
            logger.info(f"Settled bet {bet.id}: {result.status.value} - ${result.result_amount}")
    ...
    async def _settle_parlay(self, parlay: ParlayBet, result: BetResult) -> None:
        ...
        try:
            ...
            logger.info(f"Settled parlay {parlay.id}: {result.status.value} - ${result.result_amount}")
    ...
    async def _send_parlay_notification(self, parlay: ParlayBet, result: BetResult) -> None:
        ...
        try:
            if result.status == BetStatus.WON:
                ...
                notification_type = "bet_lost"
                title = "😞 Parlay Lost"
                message = f"Your ${parlay.amount} {parlay.leg_count}-leg parlay did not win."
            elif result.status == BetStatus.PUSHED:
                notification_type = "bet_pushed"
