...
def download_slugging_stats_for_year(year: int, out_csv: str):
    ...
    df["TotalBases"] = df["Singles"] * 1 + df["2B"] * 2 + df["3B"] * 3 + df["HR"] * 4
...
def main():
    ...
    p.add_argument(
        "--start_season", type=int, required=True, help="First season (e.g. 2013)"
    )
    p.add_argument(
        "--end_season", type=int, required=True, help="Last season  (e.g. 2025)"
    )
    p.add_argument(
        "--out_dir",
        required=True,
