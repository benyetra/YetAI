...
# Vault pilot sites live on {slug}.yetai.app — allow those origins via regex
# without listing every slug (see LeagueVault-Pilot-PRD §8).
app.add_middleware(
    CORSMiddleware,
    allow_origins=get_cors_origins(),
    allow_origin_regex=r"^https://[a-z0-9-]+\.yetai\.app$",
...
async def get_platform_statistics(db: Session = Depends(get_db)):
    ...
    try:
        data = compute_platform_stats(db)
        logger.info(
            "Platform stats response: users=%s, winnings=%s, 30d_bets=%s, avatars=%s",
            data["total_users"],
            data["total_winnings"],
            data["performance_30d"]["total_bets"],
            len(data["user_avatars"]),
        )
