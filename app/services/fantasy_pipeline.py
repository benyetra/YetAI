...
class FantasyPipeline:
    ...
    async def get_league_roster(self, league_id: str, user_id: int) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            connections_result = await self._fantasy_connection_service.get_user_connections(user_id)
            ...
            logger.info(f"Found Sleeper connection for user {user_id}: platform_user_id={platform_user_id}")

            # Get roster for this user in this league
            roster_player_ids = await self._sleeper_service.get_roster_by_owner(league_id, platform_user_id)

            if not roster_player_ids:
                logger.warning(f"No roster found for user {platform_user_id} in league {league_id}")
                return []
