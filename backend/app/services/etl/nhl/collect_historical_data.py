...
from app.models.predictions_models import (
    NHLTeam, NHLGoalie, NHLTeamStats, NHLPlayer,
    NHLGameStats, NHLGoalieActuals
)
...
def populate_teams():
    ...
    for abbrev, team_info in NHL_TEAMS.items():
        existing_team = db_session.query(NHLTeam).filter_by(team_id=team_info['id']).first()
...
def collect_game_data(season=20252026, max_games_per_team=20):
    ...
    for abbrev, team_info in NHL_TEAMS.items():
        ...
        completed_games = [g for g in schedule['games']
        ...
        for game in games:
            ...
            existing_game = db_session.query(NHLGameStats).filter_by(game_id=game_id).first()
            if existing_game:
                continue
            ...
            if home_goalie:
                ...
                game_stats.home_goalie_shots_against = home_goalie.get('shotsAgainst', 0)
            ...
            if away_goalie:
                ...
                game_stats.away_goalie_shots_against = away_goalie.get('shotsAgainst', 0)
            ...
            if home_goalie:
                add_goalie_actual(game_id, boxscore['gameDate'], home_goalie,
                                boxscore['homeTeam']['placeName']['default'],
                                boxscore['awayTeam']['placeName']['default'])

            if away_goalie:
                add_goalie_actual(game_id, boxscore['gameDate'], away_goalie,
                                boxscore['awayTeam']['placeName']['default'],
                                boxscore['homeTeam']['placeName']['default'])
...
def collect_player_game_stats(boxscore):
    ...
    for team_type in ['homeTeam', 'awayTeam']:
        ...
        opponent_name = boxscore['awayTeam' if team_type == 'homeTeam' else 'homeTeam']['placeName']['default']
        ...
        for position_group in ['forwards', 'defense']:
            ...
            for player_id, player_stats in team_data[position_group].items():
                ...
                player_game_stat = {
                    ...
                    'position': player_stats.get('position', 'F' if position_group == 'forwards' else 'D'),
...
def calculate_goalie_stats():
    ...
    for goalie_id, games in goalie_games.items():
        ...
        goalie.goals_against_avg = (total_shots - total_saves) / len(games) if games else 0
        ...
        goalie.last_10_save_pct = last_10_saves / last_10_shots if last_10_shots > 0 else 0
...
def calculate_team_stats():
    ...
    for team_id, game_lists in team_games.items():
        ...
        team_stats.goals_for_per_game = goals_for / games_played if games_played > 0 else 0
        team_stats.goals_against = goals_against
        team_stats.goals_against_per_game = goals_against / games_played if games_played > 0 else 0
        team_stats.shots_for = shots_for
        team_stats.shots_for_per_game = shots_for / games_played if games_played > 0 else 0
        team_stats.shots_against = shots_against
        team_stats.shots_against_per_game = shots_against / games_played if games_played > 0 else 0

        team_stats.last_10_shots_for_per_game = last_10_shots_for / len(last_10) if last_10 else 0
        team_stats.last_10_shots_against_per_game = last_10_shots_against / len(last_10) if last_10 else 0
        team_stats.last_10_goals_for_per_game = last_10_goals_for / len(last_10) if last_10 else 0
        team_stats.last_10_goals_against_per_game = last_10_goals_against / len(last_10) if last_10 else 0
...
def calculate_team_stats_for_season(season):
    ...
    team_stats = defaultdict(lambda: {
        'games': 0, 'wins': 0, 'losses': 0, 'ot_losses': 0,
    ...
    for team_id, stats in team_stats.items():
        ...
        if team:
            ...
            if games > 0:
                ...
                team.shooting_pct = stats['goals_for'] / stats['shots_for'] if stats['shots_for'] > 0 else 0
...
def calculate_career_goalie_vs_team_stats():
    ...
    for goalie in goalies:
        ...
        actuals = db_session.query(NHLGoalieActuals).filter_by(goalie_id=goalie.player_id).all()
    ...
    for team_data in special_teams_data['data']:
        ...
        if not team_stats:
            ...
            team_stats = NHLTeamStats(
                team_id=team_id,
                team_name=team_data['teamFullName']
            )
            db_session.add(team_stats)
        ...
        if games_played > 0:
            ...
            pp_goals = team_data.get('powerPlayNetPct', 0) * games_played * 3  # Rough estimate
            pp_pct = team_data.get('powerPlayPct', 0.20)
            if pp_pct > 0:
                team_stats.pp_opportunities_per_game = (pp_goals / pp_pct) / games_played if pp_pct > 0 else 3.0
            else:
...
def calculate_shot_quality_metrics(season=20252026, sample_games=50):
    ...
    team_shot_data = defaultdict(lambda: {'total_distance': 0, 'shot_count': 0, 'high_danger': 0})

    # Sample games from the database
    recent_games = db_session.query(NHLGameStats).filter_by(season=season).order_by(
        NHLGameStats.game_date.desc()
    ).limit(sample_games).all()
    ...
    for team_id, shot_data in team_shot_data.items():
        ...
        if team_stats and shot_data['shot_count'] > 0:
            ...
            team_stats.avg_shot_distance = shot_data['total_distance'] / shot_data['shot_count']

            # High danger chances per game
            team_stats.high_danger_chances_against_per_game = shot_data['high_danger'] / games_analyzed if games_analyzed > 0 else 0

            teams_updated += 1

    db_session.commit()
    print(f"  ✓ Updated shot quality metrics for {teams_updated} teams from {games_analyzed} games")
...
def collect_season_player_stats(season=20252026):
    ...
    for player_stats in skater_data['data']:
        ...
        if team_id == 0:
            print(f"  ⚠️  Could not map team '{team_abbrev}' for {player_stats['skaterFullName']}")
            continue
...
def collect_team_offensive_stats(season=20252026):
    ...
    team_offense = defaultdict(lambda: {
        'games': 0, 'goals': 0, 'shots': 0, 'pp_goals': 0,
        'home_goals': 0, 'away_goals': 0,
        'goals_period_1': 0, 'goals_period_2': 0, 'goals_period_3': 0
    })
    ...
    for team_id, stats in team_offense.items():
        ...
        if team and stats['games'] > 0:
            ...
            team.home_goals_per_game = stats['home_goals'] / (stats['games'] / 2) if stats['games'] > 0 else 0
            team.away_goals_per_game = stats['away_goals'] / (stats['games'] / 2) if stats['games'] > 0 else 0
...
def main():
    ...
    parser.add_argument('--backfill', action='store_true',
                       help='Backfill data from 2021-2025 seasons')
    parser.add_argument('--start-season', type=int, default=20212022,
                       help='Starting season for backfill (default: 20212022)')
    parser.add_argument('--end-season', type=int, default=20252026,
