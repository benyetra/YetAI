...
from app.services.yetai_bets_service_db import (
    yetai_bets_service_db as yetai_bets_service,
)
...
from app.services.bet_sharing_service_db import (
    bet_sharing_service_db as bet_sharing_service,
)
from app.services.websocket_manager import (
    manager,
    simulate_odds_updates,
    simulate_score_updates,
)
...
async def get_nfl_odds():
    ...
    try:
        ...
        return {"status": "success", "count": len(odds), "odds": odds}
    except Exception as e:
...
async def get_data_status():
    ...
    status = {"espn_api": "unknown", "odds_api": "unknown", "last_updated": None}
...
async def get_fantasy_projections():
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Error generating projections: {str(e)}"
        )
...
async def get_start_sit_advice(position: str):
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Error getting start/sit advice: {str(e)}"
        )
...
async def check_chat_health():
    ...
    try:
        ...
        health_status["data_sources"]["games_api"] = (
            len(context_data.get("games", [])) > 0
        )
        health_status["data_sources"]["odds_api"] = (
            len(context_data.get("odds", [])) > 0
        )
        health_status["data_sources"]["fantasy_api"] = (
            len(context_data.get("fantasy_projections", [])) > 0
        )

        if health_status["openai_configured"] and any(
...
async def get_best_predictions():
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Error fetching best predictions: {str(e)}"
        )
...
async def simulate_historical_performance():
    ...
    try:
        ...
        return {"status": "success", "message": "Sample historical data generated"}
    except Exception as e:
        raise HTTPException(
...
async def get_current_injury_report():
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Error fetching injury report: {str(e)}"
        )
...
async def get_weather_impact():
    ...
    try:
            ...
            weather_impact.append(
                {
                    ...
                    "fantasy_advice": {
                        "passing": (
                            "reduced"
                            if weather["impact_rating"] == "high"
                            else "normal"
                        ),
                        "kicking": (
                            "difficult" if weather["wind_speed"] > 15 else "normal"
                        ),
                        "running": (
                            "enhanced"
                            if weather["impact_rating"] == "high"
        ...
        raise HTTPException(
            status_code=500, detail=f"Error fetching weather impact: {str(e)}"
        )
...
def get_fantasy_service(db: Session = Depends(get_db)) -> FantasyService:
    ...
    if fantasy_service_instance is None:
        ...
        fantasy_service_instance.register_platform(
            FantasyPlatform.SLEEPER, SleeperFantasyService()
        )
...
async def get_fantasy_accounts(
    ...
    try:
        ...
        return {"status": "success", "accounts": formatted_accounts}
...
async def get_fantasy_leagues(
    ...
    try:
        ...
        formatted_leagues = [
            {
                ...
                "league_id": league[
                    "platform_league_id"
                ],  # Use platform league ID for Trade Analyzer
                "name": league["name"],
                ...
                "last_sync": (
                    league["last_sync"].isoformat() if league["last_sync"] else None
                ),
        ...
        return {"status": "success", "leagues": formatted_leagues}

    except Exception as e:
        logger.error(f"Error getting fantasy leagues: {str(e)}")
        return {"status": "error", "message": str(e), "leagues": []}
        ...
        formatted_teams = [
            {
                ...
                "name": team.get(
                    "name", team.get("team_name", f"Team {team.get('id', 'Unknown')}")
                ),
                "owner_name": team.get(
                    "owner_name", team.get("owner", "Unknown Owner")
                ),
            }
            for team in teams
        ]

        return {"status": "success", "teams": formatted_teams}
...
async def get_league_rules(
    ...
    try:
        ...
        leagues = fantasy_service.get_user_leagues(current_user["id"])
        league = next(
            (l for l in leagues if l["platform_league_id"] == league_id), None
        )

        if not league:
            raise HTTPException(
                status_code=404, detail="League not found or not accessible"
            )
        ...
        rules = {
            ...
            "ai_context": {
                ...
                "position_scarcity": {"qb": 1, "rb": 2, "wr": 2, "te": 1, "flex": 1},
            },
        }
        ...
        return {"status": "error", "message": str(e), "rules": None}
...
async def sync_fantasy_leagues(
    ...
    try:
        ...
        if not any(acc["id"] == fantasy_user_id for acc in accounts):
            raise HTTPException(
                status_code=403, detail="Fantasy account not found or not owned by user"
            )
...
async def sync_fantasy_league_by_id(
    ...
    try:
        ...
        if not target_league:
            raise HTTPException(
                status_code=404, detail="League not found or not owned by user"
            )
        ...
        try:
            league_obj = (
                ...
                .filter(FantasyLeague.id == target_league["id"])
                .first()
            )
            if league_obj:
        ...
        if not fantasy_user_id:
            raise HTTPException(
                status_code=404, detail="League's fantasy account not found"
            )
...
async def get_fantasy_roster(
    ...
    try:
        ...
        try:
            ...
            fantasy_user_obj = (
                db.query(FantasyUser).filter(FantasyUser.id == fantasy_user_id).first()
            )
        ...
        for player_id in user_roster:
            if player_id in players_data:
                ...
                trade_value = round(
                    base_value * age_multiplier * team_multiplier * variance, 1
                )
        ...
        return {"status": "error", "message": str(e), "roster": []}
...
async def disconnect_fantasy_account(
    ...
    try:
        result = fantasy_service.disconnect_fantasy_account(
            current_user["id"], fantasy_user_id
        )
        return result
...
async def disconnect_league(
    ...
    try:
        ...
        result = fantasy_service.disconnect_league(
            current_user["id"], target_league["id"]
        )
        return result
...
async def get_start_sit_recommendations(
    ...
    try:
        ...
        return {"success": True, "week": week, "recommendations": recommendations}
...
async def get_league_standings(
    ...
    try:
        ...
        return {"status": "success", "league_id": league_id, "standings": standings}
...
async def test_sleeper_trending():
    ...
    try:
            ...
            combined_trending.append(
                {
                    ...
                    "first_name": (
                        player.get("name", "").split(" ")[0]
                        if player.get("name")
                        else ""
                    ),
                    "last_name": (
                        " ".join(player.get("name", "").split(" ")[1:])
                        if player.get("name")
                        and len(player.get("name", "").split(" ")) > 1
                        else ""
                    ),
        ...
        return {"status": "success", "trending": combined_trending}
...
async def search_players(
    ...
    try:
        ...
        if league_id and availability != "all":
            try:
                available_players = await sleeper_service.get_available_players(
                    league_id
                )
                availability_lookup = {
                    p.get("player_id"): "available" for p in available_players
                }
            except:
        ...
        for player_id, player_data in all_players.items():
            ...
            if experience_min and (
                not player_experience or player_experience < experience_min
            ):
                continue

            if experience_max and (
                not player_experience or player_experience > experience_max
            ):
                continue

            if injury_status:
                if injury_status == "healthy" and injury_report not in [
                    None,
                    "",
                    "Healthy",
                ]:
                    ...
                    injury_status == "questionable" and injury_report != "Questionable"
                ):
                    continue
            ...
            league_metrics = {}
            if league_context:
                league_metrics = _calculate_league_specific_metrics(
                    player_data, league_context
                )
...
async def compare_players(
    ...
    try:
        ...
        for player_id in player_ids:
            ...
            if league_context:
                league_analysis = _calculate_detailed_league_analysis(
                    player_data, league_context
                )
        ...
        return {
            ...
            "comparison": {
                ...
                "insights": insights,
                "league_context": (
                    league_context["scoring_type"] if league_context else None
                ),
...
def _calculate_detailed_league_analysis(
    player_data: Dict, league_context: Dict
) -> Dict:
    ...
    if position == "WR" and ppr_value > 0:
        ...
        analysis["league_recommendation"] = (
            f"High value in {league_context['scoring_type'].replace('_', ' ')} format"
        )
...
def _generate_comparison_insights(
    players: List[Dict], league_context: Optional[Dict]
) -> List[str]:
    ...
    if len(ages) >= 2:
        ...
        if oldest - youngest >= 3:
            insights.append(
                f"Age gap: {oldest - youngest} years - consider upside vs experience"
            )
    ...
    if len(unique_positions) == 1 and league_context:
        ...
        if pos == "WR" and league_context["ppr_value"] > 0:
            insights.append(
                f"All WRs benefit from {league_context['ppr_value']} PPR scoring"
            )
...
async def sync_league_history(
    ...
    try:
        ...
        if not user or not user.sleeper_user_id:
            raise HTTPException(
                status_code=400, detail="Must connect Sleeper account first"
            )

        result = await simplified_sleeper_service.sync_all_league_history(
            current_user["id"], db
        )
...
async def sync_all_rosters(
    ...
    try:
        result = await simplified_sleeper_service.sync_all_rosters(
            current_user["id"], db
        )
...
async def get_popular_sports_odds():
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Error fetching popular odds: {str(e)}"
        )
...
async def get_live_odds(
    ...
    try:
        ...
        if sport_key not in valid_sports and sport_key not in [
            sport.name.lower() for sport in SportKey
        ]:
...
async def get_game_scores(sport_key: str, days_from: int = 3):
    ...
    try:
        async with OddsAPIService(settings.ODDS_API_KEY) as service:
            scores = await service.get_scores(sport=sport_key, days_from=days_from)
...
async def get_event_odds(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Error fetching event odds: {str(e)}"
        )
...
async def get_live_games():
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Error fetching live games: {str(e)}"
        )
...
async def get_usage_stats():
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Error getting usage stats: {str(e)}"
        )
...
async def login(user_data: UserLogin):
    ...
    try:
        result = await auth_service.authenticate_user(
            email_or_username=user_data.email_or_username, password=user_data.password
        )
...
async def get_google_auth_url():
    ...
    try:
        ...
        return JSONResponse(
            ...
            content={
                "status": "error",
                "message": f"Failed to get authorization URL: {str(e)}",
            },
        )
    ...
    try:
        ...
        if existing_user:
            ...
            if user_info.get("picture"):
                await auth_service.update_user_avatar(
                    existing_user["id"], user_info["picture"]
                )
    ...
    try:
        ...
        if success:
            ...
            update_result = await auth_service.update_user_avatar(
                current_user["id"], result["avatar"], result["thumbnail"]
            )
...
async def update_preferences(
    ...
    try:
        result = await auth_service.update_user_preferences(
            user_id=current_user["id"], preferences=preferences.dict(exclude_unset=True)
        )
...
async def upgrade_subscription(
    upgrade_data: SubscriptionUpgrade, current_user: dict = Depends(get_current_user)
):
    ...
    try:
        ...
        result = await auth_service.upgrade_subscription(
            user_id=current_user["id"], tier=upgrade_data.tier
        )
...
async def create_demo_users():
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to create demo users: {str(e)}"
        )
...
async def get_personalized_predictions(current_user: dict = Depends(get_current_user)):
    ...
    try:
        ...
        for game in games:
            if (
                game.get("home_team") in favorite_teams
                or game.get("away_team") in favorite_teams
            ):
        ...
        raise HTTPException(
            status_code=500, detail=f"Error getting personalized predictions: {str(e)}"
        )
...
async def get_user_performance(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Error getting user performance: {str(e)}"
        )
...
async def place_bet(
    bet_data: PlaceBetRequest, current_user: dict = Depends(get_current_user)
):
    """Place a single bet"""
...
async def place_parlay(
    parlay_data: PlaceParlayRequest, current_user: dict = Depends(get_current_user)
):
    """Place a parlay bet"""
...
async def get_user_parlays(
    ...
    if result["success"]:
        ...
        raise HTTPException(
            status_code=400, detail=result.get("error", "Failed to fetch parlays")
        )
...
async def get_parlay_details(
    ...
    if result["success"]:
        return {"status": "success", "parlay": result["parlay"]}
    else:
        raise HTTPException(
...
async def get_bet_history(
    ...
    if result["success"]:
        ...
        live_bets = live_betting_service.get_user_live_bets(
            current_user["id"], include_settled=True
        )
        ...
        for live_bet in live_bets:
            ...
            if live_bet.bet_type == "moneyline":
                ...
                if live_bet.selection == "over":
                    ...
                    title = (
                        f"Under Total ({live_bet.home_team} vs {live_bet.away_team})"
                    )
            else:
            ...
            bet_data = {
                ...
                "current_score": (
                    f"{live_bet.current_home_score or 0} - {live_bet.current_away_score or 0}"
                    if live_bet.current_home_score is not None
                    else None
                ),
                "game_time": (
                    live_bet.current_game_status.value
                    if live_bet.current_game_status
                    else None
                ),
        ...
        raise HTTPException(
            status_code=400, detail=result.get("error", "Failed to fetch bet history")
        )
...
async def place_live_bet(
    request: PlaceLiveBetRequest, current_user: dict = Depends(get_current_user)
):
...
async def get_cash_out_offer(
    bet_id: str, current_user: dict = Depends(get_current_user)
):
    ...
    try:
        ...
        if not offer:
            raise HTTPException(
                status_code=404, detail="Bet not found or cash out not available"
            )
...
async def execute_cash_out(
    request: CashOutRequest, current_user: dict = Depends(get_current_user)
):
    """Execute cash out for a live bet"""
    try:
        # Get current offer first
        offer = live_betting_service.get_cash_out_offer(
            request.bet_id, current_user["id"]
        )
        ...
        result = live_betting_service.execute_cash_out(
            request.bet_id, current_user["id"], accept_amount
        )
...
async def get_user_live_bets(
    include_settled: bool = False, current_user: dict = Depends(get_current_user)
):
    """Get user's active live bets"""
    try:
        bets = live_betting_service.get_user_live_bets(
            current_user["id"], include_settled
        )

        return {"status": "success", "count": len(bets), "bets": bets}
...
async def get_live_bet_details(
    ...
    try:
        ...
        return {"status": "success", "bet": bet, "cash_out_history": cash_out_history}
...
async def start_live_betting_simulation(current_user: dict = Depends(get_current_user)):
...
async def stop_live_betting_simulation(current_user: dict = Depends(get_current_user)):
    """Stop live betting simulation"""
    try:
        await live_betting_simulator.stop_simulation()

        return {"status": "success", "message": "Live betting simulation stopped"}
...
async def get_yetai_bets(user: dict = Depends(get_current_user_from_auth)):
    ...
    try:
        ...
        return {"status": "success", "bets": bets, "performance_stats": stats}
    except Exception as e:
...
async def create_yetai_parlay(
    ...
    try:
        result = await yetai_bets_service.create_parlay(
...
async def update_yetai_bet(
    bet_id: str,
    update_request: UpdateYetAIBetRequest,
    admin_user: dict = Depends(require_admin),
):
    """Update a YetAI Bet (Admin only)"""
    try:
        result = await yetai_bets_service.update_bet(
            bet_id, update_request, admin_user["id"]
        )
...
async def delete_all_user_bets(user_id: int, admin_user: dict = Depends(require_admin)):
    ...
    try:
        ...
        try:
            ...
            logger.info(
                f"Admin {admin_user['id']} deleted all bets for user {user_id}: {deleted_counts}"
            )
...
async def promote_user_to_admin(
    request: PromoteAdminRequest, current_user: dict = Depends(require_admin)
):
...
async def create_admin_user(
    request: CreateAdminRequest, current_user: dict = Depends(require_admin)
):
...
async def sync_comprehensive_league_history(
    ...
    try:
        logger.info(
            f"Starting comprehensive league history sync for league {league_id}"
        )
...
async def get_league_competitive_insights(
    ...
    try:
        ...
        try:
            league = (
                db_temp.query(FantasyLeague)
                .filter(FantasyLeague.platform_league_id == league_id)
                .first()
            )
            if not league:
                raise HTTPException(
                    status_code=404, detail="League not found or access denied"
                )
        finally:
            db_temp.close()
        ...
        try:
            ...
            historical_records = (
                ...
                .filter(LeagueHistoricalData.league_id == league.id)
                .order_by(LeagueHistoricalData.season.desc())
                .limit(5)
                .all()
            )
            ...
            competitor_analyses = (
                ...
                .filter(CompetitorAnalysis.league_id == league.id)
                .all()
            )
            ...
            insights = {
                "league_summary": {
                    ...
                    "data_last_updated": (
                        max([r.last_updated for r in historical_records]).isoformat()
                ...
                "competitive_landscape": {
                    "league_competitiveness": (
                        "high" if len(competitor_analyses) > 8 else "medium"
                    ),
                    "waiver_wire_activity": (
                        "Very Active"
                        if any(
                            c.avg_waiver_adds_per_season > 15
                            for c in competitor_analyses
                        )
                        else "Moderate"
                    ),
                    "faab_spending_patterns": {
                        "aggressive_spenders": len(
                            [
                                c
                                for c in competitor_analyses
                                if c.faab_conservation_tendency == "aggressive"
                            ]
                        ),
                        "conservative_managers": len(
                            [
                                c
                                for c in competitor_analyses
                                if c.faab_conservation_tendency == "conservative"
                            ]
                        ),
                        "average_faab_per_season": (
                            sum(
                                c.avg_faab_spent_per_season or 0
                                for c in competitor_analyses
                            )
                            / len(competitor_analyses)
            ...
            return {"status": "success", "league_id": league_id, "insights": insights}
...
async def sync_league_waiver_settings(
    ...
    try:
        ...
        try:
            ...
            if not league_data:
                raise HTTPException(
                    status_code=404, detail="League not found on Sleeper"
                )
...
async def enable_2fa(
    request: Enable2FARequest, current_user: dict = Depends(get_current_user)
):
...
async def disable_2fa(
    request: Disable2FARequest, current_user: dict = Depends(get_current_user)
):
    """Disable 2FA with password and 2FA verification"""
    try:
        result = await auth_service.disable_2fa(
            current_user["id"], request.password, request.token
        )
...
async def get_backup_codes_status(current_user: dict = Depends(get_current_user)):
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to get backup codes status: {str(e)}"
        )
...
async def regenerate_backup_codes(current_user: dict = Depends(get_current_user)):
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to regenerate backup codes: {str(e)}"
        )
...
async def email_backup_codes(current_user: dict = Depends(get_current_user)):
    ...
    try:
        ...
        if email_sent:
            return {"status": "success", "message": "Backup codes sent to your email"}
        else:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to email backup codes: {str(e)}"
        )


@app.post("/api/auth/2fa/verify")
async def verify_2fa_token(
    request: Verify2FARequest, current_user: dict = Depends(get_current_user)
):
    """Verify a 2FA token (for testing or validation)"""
    try:
        is_valid = await auth_service.verify_2fa_token(
            current_user["id"], request.token
        )
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to verify 2FA token: {str(e)}"
        )
...
async def update_user(
    user_id: int, update_data: dict, current_user: dict = Depends(get_current_user)
):
    ...
    try:
        ...
        logger.info(
            f"Admin {current_user['id']} updating user {user_id} with data: {update_data}"
        )
...
async def create_user_admin(
    user_data: dict, current_user: dict = Depends(get_current_user)
):
...
async def create_shareable_bet_link(
    ...
    bet_history_result = await bet_service.get_bet_history(
        current_user["id"], BetHistoryQuery(limit=100)
    )
    ...
    if not bet_data:
        parlay_result = await bet_service.get_user_parlays(
    ...
    result = await bet_sharing_service.create_shareable_link(
        current_user["id"], bet_data
    )
...
async def update_verification_config(
    config: dict, current_user: dict = Depends(get_current_user)
):
...
async def get_simple_standings(league_id: str, db: Session = Depends(get_db)):
    ...
    try:
        ...
        sleeper_league = (
            db.query(SleeperLeague)
            .filter(SleeperLeague.sleeper_league_id == league_id)
            .first()
        )
        ...
        logger.info(
            f"Using league season {sleeper_league.season} (may be pre-season with 0-0 records)"
        )
        ...
        rosters = (
            ...
            .order_by(SleeperRoster.wins.desc(), SleeperRoster.points_for.desc())
            .all()
        )

        # Check if we need to fetch real data from Sleeper API (if we have generic team names)
        need_real_data = any(
            roster.owner_name == "Unknown" or roster.team_name.startswith("Team ")
            for roster in rosters
        )
        ...
        if need_real_data:
            try:
                ...
                logger.info(
                    f"Fetching real roster data from Sleeper API for league {league_id}"
                )
                with httpx.Client() as client:
                    # Fetch real roster data from Sleeper
                    rosters_response = client.get(
                    ...
                    logger.info(
                        f"Found {len(sleeper_rosters)} Sleeper rosters and {len(sleeper_users)} users"
                    )
                    for sleeper_roster in sleeper_rosters:
                        roster_id = str(
                            sleeper_roster["roster_id"]
                    ...
                    logger.info(
                        f"Created real_roster_data mapping for {len(real_roster_data)} rosters"
                    )
            ...
            standings.append(
                {
                    ...
                    ),  # For TradeAnalyzer compatibility
                    "wins": roster.wins,
                    "losses": roster.losses,
                    "ties": roster.ties,
                    "points_for": roster.points_for,
                    ...
                    "win_percentage": (
                        ...
                        if (roster.wins + roster.losses + roster.ties) > 0
                        else 0
                    ),
                }
            )
...
def _calculate_team_rank(roster, db) -> int:
    ...
    try:
        ...
        all_rosters = (
            db.query(SleeperRoster)
            .filter(SleeperRoster.league_id == roster.league_id)
            .order_by(SleeperRoster.wins.desc(), SleeperRoster.points_for.desc())
            .all()
        )
...
def _get_tradeable_picks(roster, db) -> list:
    ...
    try:
        ...
        league = (
            db.query(SleeperLeague).filter(SleeperLeague.id == roster.league_id).first()
        )
...
async def get_simple_team_analysis(
    team_id: int, league_id: int = None, db: Session = Depends(get_db)
):
    """Get team analysis using current (2025) season Sleeper data"""
    try:
        logger.info(
            f"Getting team analysis for team_id={team_id}, league_id={league_id}"
        )
        ...
        if league_id:
            source_league = (
                db.query(SleeperLeague).filter(SleeperLeague.id == league_id).first()
            )
        ...
        if not source_league:
            ...
            source_roster = (
                db.query(SleeperRoster)
                .join(SleeperLeague)
                .filter(SleeperRoster.sleeper_roster_id == str(team_id))
        ...
        current_league = (
            db.query(SleeperLeague)
            .filter(SleeperLeague.user_id == user_id, SleeperLeague.season == 2025)
            .first()
        )

        if not current_league:
            logger.warning(
                f"No 2025 league found for user {user_id}, using most recent season"
            )
            current_league = (
                db.query(SleeperLeague)
                .filter(SleeperLeague.user_id == user_id)
                .order_by(SleeperLeague.season.desc())
                .first()
            )
        ...
        if not roster:
            roster = (
                db.query(SleeperRoster)
                .filter(SleeperRoster.league_id == current_league.id)
                .first()
            )
        ...
        if roster and roster.last_synced:
            hours_since_sync = (
                datetime.utcnow() - roster.last_synced
            ).total_seconds() / 3600
            if hours_since_sync > 1:  # Refresh if older than 1 hour
                logger.info(
                    f"Roster data is {hours_since_sync:.1f} hours old, triggering refresh"
                )
                try:
                    from app.services.simplified_sleeper_service import (
                        SimplifiedSleeperService,
                    )
        ...
        if roster:
            logger.info(
                f"Using roster from season {current_league.season}: {roster.team_name}, {roster.wins}-{roster.losses}, {roster.points_for} pts"
            )
            logger.info(
                f"Roster has {len(roster.players) if roster.players else 0} players"
            )
        else:
            logger.error(
                f"No roster found for user {user_id} in league {current_league.id}"
            )
        ...
        if (
            roster.team_name
            and roster.team_name.startswith("Team ")
            or roster.owner_name == "Unknown"
        ):
            try:
                import httpx

                logger.info(
                    f"Fetching real roster data from Sleeper API for league {current_league.sleeper_league_id}"
                )
                with httpx.Client() as client:
                    # Fetch real roster data from Sleeper
                    rosters_response = client.get(
                        f"https://api.sleeper.app/v1/league/{current_league.sleeper_league_id}/rosters"
                    )
                    rosters_response.raise_for_status()
                    sleeper_rosters = rosters_response.json()
                    ...
                    # Fetch league users for owner names
                    users_response = client.get(
                        f"https://api.sleeper.app/v1/league/{current_league.sleeper_league_id}/users"
                    )
                    users_response.raise_for_status()
                    sleeper_users = users_response.json()
        ...
        for player in players:
            ...
            trade_value = calculate_dynamic_player_value(
                player["position"], player["is_starter"]
            )
        ...
        for pos, counts in position_counts.items():
            ...
            if len(starters_at_pos) == 0:
                ...
                position_needs[pos] = max(
                    5, 100 - total_strength
                )  # Inverse relationship

            # Mark as surplus if many players at position
            if len(position_players) >= 5 or (
                pos in ["RB", "WR"] and len(position_players) >= 4
            ):
...
async def simple_trade_analysis(trade_data: Dict[str, Any]):
    ...
    try:
        ...
        if total_value > 0:
            fairness_score = 100 - (
                abs(team1_gives_value - team2_gives_value) / total_value * 100
            )
        else:
            fairness_score = 100
        ...
        if value_diff < 3:
            ...
            winner_grade, loser_grade = (
                ("A-", "B") if team1_gives_value < team2_gives_value else ("B", "A-")
            )
            team1_grade, team2_grade = (
                (loser_grade, winner_grade)
                if team1_gives_value < team2_gives_value
                else (winner_grade, loser_grade)
            )
        else:
            winner_grade, loser_grade = (
                ("A", "C") if team1_gives_value < team2_gives_value else ("C", "A")
            )
            team1_grade, team2_grade = (
                (loser_grade, winner_grade)
                if team1_gives_value < team2_gives_value
        ...
        if fairness_score >= 90:
            ...
            favored_team = (
                "Team 2" if team1_gives_value > team2_gives_value else "Team 1"
            )
                ...
                "key_factors": [
                    {
                        ...
                        "description": "Trade Value Analysis",
                        "impact": "high",
                        "confidence": 85,
                    },
                    {
                        "category": "value_difference",
                        "description": f"Value Difference: {value_diff:.1f}",
                        "impact": "medium",
                        "confidence": 90,
                    },
                ],
...
async def get_dynamic_player_values(
    position: Optional[str] = None, limit: int = 100, search: Optional[str] = None
):
    ...
    try:
        ...
        players = (
            query.order_by(SleeperPlayer.search_rank.nulls_last()).limit(limit).all()
        )
        ...
        return {"success": True, "players": player_values, "count": len(player_values)}
...
async def get_player_analytics(
    ...
    try:
        ...
        analytics = await analytics_service.get_player_analytics(
            player_id, week_list, season
        )

        return {"status": "success", "player_id": player_id, "analytics": analytics}

    except Exception as e:
        logger.error(f"Failed to get player analytics: {str(e)}")
        raise HTTPException(
            status_code=500, detail=f"Failed to get player analytics: {str(e)}"
        )
    finally:
        db.close()
...
async def get_player_trends(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to get player trends: {str(e)}"
        )
    finally:
        db.close()
...
async def get_player_efficiency(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to get player efficiency: {str(e)}"
        )
    finally:
        db.close()
...
async def get_breakout_candidates(
    ...
    try:
        ...
        candidates = await analytics_service.identify_breakout_candidates(
            position, season, min_weeks
        )
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to get breakout candidates: {str(e)}"
        )
    finally:
        db.close()
...
async def get_matchup_analytics(
    ...
    try:
        ...
        matchup_data = await analytics_service.get_matchup_specific_analytics(
            player_id, opponent, season
        )
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to get matchup analytics: {str(e)}"
        )
    finally:
        db.close()
...
async def store_player_analytics(
    ...
    try:
        ...
        analytics = await analytics_service.store_weekly_analytics(
            player_id, week, season, analytics_data
        )
...
async def get_websocket_stats():
    ...
    return {"status": "success", "stats": manager.get_connection_stats()}
...
async def evaluate_trade(
    trade_id: int, force_refresh: bool = False, current_user=Depends(get_current_user)
):
...
async def generate_trade_recommendations(request: Dict[str, Any]):
    ...
    try:
        ...
        if not team:
            ...
            if not roster:
                logger.info(
                    f"No roster found for team_id={request['team_id']}, league_id={request['league_id']}"
                )
        ...
        raise HTTPException(
            status_code=500, detail="Failed to generate trade recommendations"
        )
    finally:
        db.close()
...
async def find_mutual_benefit_trades(
    ...
    try:
        ...
        if target_team_id:
            ...
            if not target_team:
                ...
                if not target_roster:
                    raise HTTPException(
                        status_code=404, detail="Target team not found in league"
                    )
        ...
        raise HTTPException(
            status_code=500, detail="Failed to find mutual benefit trades"
        )
    finally:
        db.close()
...
async def quick_trade_analysis(trade_data: Dict[str, Any]):
    ...
    try:
        ...
        logger.info(
            f"Quick trade analysis - team1_gives: {team1_gives}, team2_gives: {team2_gives}"
        )
        ...
        for player_id in team1_gives:
            ...
            if sleeper_player:
                ...
                logger.info(
                    f"Found team1 player: {sleeper_player.full_name} (ID: {player_id}, value: {value})"
                )
            else:
                logger.warning(f"Player not found in database: {player_id}")

        logger.info(
            f"Team1 gives - Found {len(team1_players)} players, total value: {team1_gives_value}"
        )
        ...
        raise HTTPException(
            status_code=500, detail="Failed to perform quick trade analysis"
        )
    finally:
        db.close()
