...
from app.services.etl.mlb.lineup_utils import lineup_k_rate_vs_hand, lineup_matchup_adjusted_strikeouts, projected_lineup
...
from app.services.etl.mlb.regression_analysis import calculate_performance_metrics, fetch_past_performance_metrics, perform_regression_analysis, project_at_bats_faced, project_innings_pitched

from app.services.etl.mlb.classification_model import load_classifier, park_factor_for_venue
from app.services.etl.mlb.odds_utils import american_to_break_even_prob, normalize_two_sided, expected_value
from app.services.etl.mlb.offsets import apply_shrunk_offsets
...
from app.services.etl.mlb.mlb_matchup_analysis import matchup_adjusted_strikeouts, fetch_last_start_date
...
def get_last_5_games_k_per_9(player_id):
    ...
    start_date = end_date - timedelta(days=30)  # Approximate last 30 days to get last 5 games
...
def get_event_id_for_game(team1, team2):
    ...
    if response.status_code == 200:
        ...
        for event in events:
            if (event['home_team'] == team1 and event['away_team'] == team2) or (event['home_team'] == team2 and event['away_team'] == team1):
                return event['id']
    else:
        print(f"Failed to fetch events: {response.status_code}, response body: {response.text}")
        ...
        for team in ['home', 'away']:
            ...
            if probable_pitcher_key in game:
                ...
                try:    
                    ...
                    continue
                try:
                    pitcher_stats_data = statsapi.player_stat_data(pitcher_id, group="pitching", type="career")
                ...
                try:
                    ...
                    strikeouts = opponent_stats['stats'][0]['splits'][0]['stat']['strikeOuts']
                    if at_bats > 0:
                ...
                mean_absolute_strikeout_error, mean_absolute_innings_error, mean_absolute_at_bats_error = calculate_performance_metrics(past_performance_df)
                ...
                try:
                    ...
                    projected_innings = project_innings_pitched(pitcher_data, mean_absolute_innings_error)
                    ...
                    pitcher_hand_code = 'R' if (pitch_hand or 'R').startswith('R') else 'L'
                    opp_k_rate_lineup = lineup_k_rate_vs_hand(opponent_team_id, pitcher_hand_code)
                    # keep your old team stat as fallback (opponent_k_rate was %); convert to fraction
                    opp_k_rate_frac = (opp_k_rate_lineup if opp_k_rate_lineup is not None else (opponent_k_rate/100.0))

                    # pick any batter id for APIs that require one (regression uses pitcher+batter)
                    batter_id = batter_ids[0] if batter_ids else statsapi.get(
                        "team_roster", {"teamId": opponent_team_id, "rosterType": "active"}
                    )["roster"][0]["person"]["id"]
                    ...
                    projected_at_bats_base = project_at_bats_faced(
                        pitcher_id, recent_data_base, projected_innings, mean_absolute_at_bats_error
                    )
                    if projected_at_bats_base is None:
                        print(f"Not enough data to project AB for {pitcher_name}")
                        continue

                    projected_strikeouts_base = perform_regression_analysis(
                        pitcher_id, batter_id, projected_innings, projected_at_bats_base
                    ) or 0.0
                    ...
                    proj_k_reg = perform_regression_analysis(
                    ...
                    threshold, over_price, under_price = get_book_line(event_id, pitcher_name)

                    # 8) Build features (incl. matchup factor & days rest & distance-to-line)
                    last_start = fetch_last_start_date(pitcher_id)
                    days_rest = (datetime.today().date() - last_start).days if last_start else 7

                    # lineup-weighted matchup factor
                    matchup_factor = lineup_matchup_adjusted_strikeouts(pitcher_id, batter_ids, pitcher_hand_code)
                    ...
                    var = mu * 1.8
                    prob_over_nb = negbin_prob_over(mu, var, threshold) if threshold else prob_over_clf
                    ...
                    from app.services.etl.mlb.odds_utils import (
                        american_to_break_even_prob, normalize_two_sided, expected_value
                    )
                    if over_price is not None and under_price is not None and threshold and threshold > 0:
                        p_over_raw  = american_to_break_even_prob(over_price)
                        p_under_raw = american_to_break_even_prob(under_price)
                        _p_over_fair, _p_under_fair = normalize_two_sided(p_over_raw, p_under_raw)
                        ...
                        flag, price_to_show, edge = ('o' if prob_over >= 0.5 else 'u'), (over_price or 0), abs(prob_over - 0.5)
                    ...
                    pitchers.append({
                        ...
                        'combined_score': round(k_per_9 + (opp_k_rate_frac*100.0), 2),
...
def get_book_line(event_id, pitcher_name):
    ...
    try:
        ...
        params = {'regions':'us','oddsFormat':'american','apiKey':ODDS_API_KEY,'markets':'pitcher_strikeouts'}
        ...
        for bm in data.get('bookmakers', []):
            ...
            for market in bm.get('markets', []):
                ...
                if threshold is not None and over_price is not None and under_price is not None:
                    return threshold, over_price, under_price
...
def fetch_and_update_app_data():
    try:
        ...
        for pitcher_stats_data in pitchers:
            ...
            db_session.commit()
            print(f"Added pitcher {pitcher_stats_data['name']} to the session with projected at-bats {pitcher_stats_data['projected_at_bats']}.")
