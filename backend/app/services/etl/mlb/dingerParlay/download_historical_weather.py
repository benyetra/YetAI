...
def main():
    ...
    p.add_argument("--coords", required=True, help="park_coords.csv (park_id,lat,lon)")
    p.add_argument(
        "--existing", required=True, help="weather_normalized.csv you already have"
    )
    p.add_argument(
        "--output",
        required=True,
    ...
    df_exist = df_exist.rename(
        columns={"game_date": "date", "temp": "tavg", "wind_to_out": "wspd"}
    ...
    grid = pd.MultiIndex.from_product([parks, all_dates], names=["park_id", "date"])
    df_full = pd.DataFrame(index=grid).reset_index()

    # 5) merge in your existing normals
    df_full = df_full.merge(df_exist, on=["park_id", "date"], how="left")
    ...
    if not to_fill.empty:
        ...
        if fetched:
            ...
            df_full = df_full.merge(
                df_hist, on=["park_id", "date"], how="left", suffixes=("", "_hist")
            )
