...
class MockOpenAIClient:
    async def chat_completions_create(self, messages: List[Dict], model: str = "gpt-3.5-turbo"):
        # Simulate AI response based on user input
        user_message = messages[-1]["content"].lower()
        
        if any(word in user_message for word in ["odds", "bet", "line", "spread", "moneyline"]):
            return {
                "choices": [{
                    "message": {
        ...
        elif any(word in user_message for word in ["fantasy", "start", "sit", "lineup", "player"]):
            return {
                "choices": [{
                    "message": {
        ...
        elif any(word in user_message for word in ["game", "schedule", "matchup", "team"]):
            return {
                "choices": [{
                    "message": {
...
class AIChatService:
    ...
    def __init__(self):
        ...
        self.has_openai = bool(
            ...
            not settings.OPENAI_API_KEY.startswith("sk-ant-")
        )
        ...
        if self.has_openai:
            ...
            print("OpenAI API key not configured or invalid format, using mock responses")
            self.openai_client = MockOpenAIClient()
    
    async def get_chat_response(self, user_message: str, conversation_history: List[Dict] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "context_used": {
                    ...
                    "fantasy_projections": len(context_data.get("fantasy_projections", []))
                }
            }
    ...
    async def get_context_data(self) -> Dict[str, Any]:
        ...
        try:
            ...
            games, odds, players = await asyncio.gather(
                games_task, odds_task, fantasy_task, 
            ...
            if not isinstance(players, Exception) and not isinstance(games, Exception):
                try:
                    fantasy_projections = fantasy_pipeline.generate_fantasy_projections(players, games)[:10]
                except Exception as e:
...
"""
    ...
    def _classify_response_type(self, user_message: str) -> str:
        ...
        if any(word in message_lower for word in ["bet", "odds", "line", "spread", "moneyline", "under", "over"]):
            return "betting_advice"
        elif any(word in message_lower for word in ["fantasy", "start", "sit", "lineup", "player", "projection"]):
            return "fantasy_advice"
        elif any(word in message_lower for word in ["game", "schedule", "matchup", "score", "team"]):
