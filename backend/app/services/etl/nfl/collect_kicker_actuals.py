...
def get_current_nfl_week(season=2025):
    ...
    print(f"🏈 Current NFL week: {current_week} (Season {season}, Today: {today}, Season start: {season_start}, Days diff: {days_since_start})")
    return current_week
...
def get_weekly_field_goal_data(week=None, season=2025):
    ...
    try:
        ...
        weekly_stats = fg_data.groupby([
            'kicker_player_id', 'kicker_player_name', 'posteam', 'defteam', 'game_date'
        ]).agg({
            'field_goal_attempt': 'sum',
            'field_goal_result': lambda x: (x == 'made').sum(),  # Count made FGs
        ...
        weekly_stats.columns = [
            'kicker_id', 'kicker_name', 'team', 'opponent', 'game_date',
            'attempts', 'made', 'longest_fg', 'avg_distance', 'temperature', 'wind_speed'
        ]
...
def match_predictions_with_actuals(week=None, season=2025):
    ...
    if week:
        ...
        historical_predictions = db_session.query(KickerPredictions).filter(
            and_(
        ...
        print(f"📅 Found {len(historical_predictions)} historical predictions for week {week}")
    ...
    for _, actual in actuals_df.iterrows():
        ...
        def match_kicker_name(pred, kicker_name_lower):
            ...
            pred_name_attr = 'kicker_player_name' if hasattr(pred, 'kicker_player_name') else 'name'
            ...
            if '.' in kicker_name_lower:
                ...
                if len(actual_parts) >= 2:
                    ...
                    if (len(pred_parts) >= 2 and
                        pred_parts[0][0] == actual_initial and
                        pred_parts[-1] == actual_last):
                        return True
            ...
            if '.' in pred_name:
                ...
                if len(pred_parts) >= 2:
                    ...
                    if (len(actual_parts) >= 2 and
                        actual_parts[0][0] == pred_initial and
                        actual_parts[-1] == pred_last):
                        return True

            # Case 3: Both names are full, check first initial + last name
            pred_parts = pred_name.split()
            actual_parts = kicker_name_lower.split()
            if (len(pred_parts) >= 2 and len(actual_parts) >= 2 and
                pred_parts[0][0] == actual_parts[0][0] and
        ...
        for pred in historical_predictions:
            if match_kicker_name(pred, kicker_name_lower):
                ...
                pred_date = pred.game_date.date() if isinstance(pred.game_date, datetime) else pred.game_date
        ...
        existing = db_session.query(KickerActuals).filter(
            and_(
        ...
        kicker_actual = KickerActuals(
            ...
            actual_longest_fg=int(actual['longest_fg']) if pd.notna(actual['longest_fg']) else None,
            game_temperature=float(actual['temperature']) if pd.notna(actual['temperature']) else None,
            game_wind_speed=float(actual['wind_speed']) if pd.notna(actual['wind_speed']) else None,
        ...
        print(f"✅ {kicker_name}: {actual_made} FGs made, predicted {projected_fgs:.1f} ({our_bet}) [{prediction_source}]")
        print(f"   Over 1.5: {'✅' if hit_over_1_5 else '❌'}, Prediction: {'✅' if correct_prediction else '❌' if correct_prediction is not None else '❓'}")
...
def update_kicker_actuals(week=None):
    ...
    if week is None:
        ...
        if today.month == 9 and today.day >= 19 and today.day <= 25:
            ...
            if week is None:
                print("❌ Cannot determine current NFL week - either it's off-season or an error occurred")
                print("💡 Use --week parameter to specify a specific week: python collect_kicker_actuals.py --week 3")
                return
