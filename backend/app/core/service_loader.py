...
class ServiceAvailability:
    ...
    def load_service(self, name: str, import_path: str, fallback_value: Any = None) -> Any:
        """
...
def initialize_services():
    ...
    service_loader.load_service(
        "auth_service",
        "app.services.auth_service_db.auth_service_db"
    )
    
    # Sports pipeline
    service_loader.load_service(
        "sports_pipeline",
        "app.services.data_pipeline.sports_pipeline"
    )
    
    # AI chat service
    service_loader.load_service(
        "ai_chat_service",
        "app.services.ai_chat_service.ai_chat_service"
    )
    
    # Fantasy services
    service_loader.load_service(
        "fantasy_pipeline",
        "app.services.fantasy_pipeline.fantasy_pipeline"
    )
    ...
    service_loader.load_service(
        "bet_service",
        "app.services.bet_service_db.bet_service_db"
    )
    
    service_loader.load_service(
        "yetai_bets_service",
        "app.services.yetai_bets_service_db.yetai_bets_service_db"
    )
    
    # Analytics services
    service_loader.load_service(
        "performance_tracker",
        "app.services.performance_tracker.performance_tracker"
    )
    ...
    service_loader.load_service(
        ...
        "app.services.google_oauth_service.google_oauth_service"
    )
    
    # Log summary
    available_services = [name for name, available in service_loader.get_status().items() if available]
    unavailable_services = [name for name, available in service_loader.get_status().items() if not available]
    
    logger.info(f"✅ Services available ({len(available_services)}): {', '.join(available_services)}")
    if unavailable_services:
        logger.info(f"⚠️  Services unavailable ({len(unavailable_services)}): {', '.join(unavailable_services)}")
    ...
    def decorator(func):
        def wrapper(*args, **kwargs):
            if not is_service_available(name):
                ...
                raise HTTPException(
                    status_code=503,
                    detail=f"{name} service is currently unavailable"
                )
