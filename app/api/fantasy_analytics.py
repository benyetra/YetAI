...
async def get_player_analytics(
    player_id: str,
    season: int = Query(2024, ge=2020, le=2025),
    weeks: Optional[List[int]] = Query(None),
    db: Session = Depends(get_db)
):
    """Get player analytics data using PlayerAnalyticsService"""
    try:
        from app.services.player_analytics_service import PlayerAnalyticsService
        analytics_service = PlayerAnalyticsService(db)

        # Convert string player_id to int for database lookup
        try:
            player_db_id = int(player_id)
        except ValueError:
            # If string ID provided, try to find player by platform_player_id
            from sqlalchemy import text
            result = db.execute(text("SELECT id FROM fantasy_players WHERE platform_player_id = :pid"), {"pid": player_id})
            row = result.fetchone()
            if not row:
                return {"status": "success", "player_id": player_id, "season": season, "analytics": []}
            player_db_id = row[0]

        analytics = await analytics_service.get_player_analytics(player_db_id, weeks, season)
...
@router.get("/player/{player_id}/efficiency")
async def get_player_efficiency(
    ...
    """Get player efficiency metrics using PlayerAnalyticsService"""
    try:
        from app.services.player_analytics_service import PlayerAnalyticsService
        analytics_service = PlayerAnalyticsService(db)
        ...
        try:
            ...
            row = result.fetchone()
            if not row:
                return {"status": "success", "player_id": player_id, "season": season, "efficiency": {}}
            player_db_id = row[0]

        if not weeks:
            weeks = list(range(1, 13))  # Default to weeks 1-12

        efficiency = await analytics_service.calculate_efficiency_metrics(player_db_id, weeks, season)
...
@router.get("/player/{player_id}/usage-trends")
async def get_player_usage_trends(
    player_id: str,
    season: int = Query(2024, ge=2020, le=2025),
    weeks: Optional[List[int]] = Query(None),
    db: Session = Depends(get_db)
):
    """Get player usage trends using PlayerAnalyticsService"""
    try:
        from app.services.player_analytics_service import PlayerAnalyticsService
        analytics_service = PlayerAnalyticsService(db)
        ...
        try:
            ...
            row = result.fetchone()
            if not row:
                return {"status": "success", "player_id": player_id, "season": season, "trends": {}}
            player_db_id = row[0]

        if not weeks:
            weeks = list(range(1, 13))  # Default to weeks 1-12

        trends = await analytics_service.calculate_usage_trends(player_db_id, weeks, season)
