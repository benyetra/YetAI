...
def test_ats_hit_rate_with_home_picks():
    ...
    rows = [
        (
            _proj(margin=5.0, win_prob=0.7, rec="HOME", market=-3.0),
            _actual(margin=6, home_won=True),
        ),  # covers
        (
            _proj(margin=4.0, win_prob=0.65, rec="HOME", market=-3.0),
            _actual(margin=2, home_won=True),
        ),  # no cover
        (
            _proj(margin=4.5, win_prob=0.66, rec="HOME", market=-3.0),
            _actual(margin=8, home_won=True),
        ),  # covers
    ]
    stats = sat._compute_window(
        _make_db(rows), start=date(2026, 5, 1), end=date(2026, 5, 21)
    )
...
def test_brier_score_perfect_predictions_is_zero():
    rows = [
        (
            _proj(margin=10.0, win_prob=1.0, rec="NO_PLAY"),
            _actual(margin=15, home_won=True),
        ),
        (
            _proj(margin=-10.0, win_prob=0.0, rec="NO_PLAY"),
            _actual(margin=-15, home_won=False),
        ),
    ]
    stats = sat._compute_window(
        _make_db(rows), start=date(2026, 5, 1), end=date(2026, 5, 21)
...
def test_no_play_picks_excluded_from_ats():
    rows = [
        (
            _proj(margin=2.0, win_prob=0.5, rec="NO_PLAY"),
            _actual(margin=1, home_won=True),
        ),
    ]
    stats = sat._compute_window(
        _make_db(rows), start=date(2026, 5, 1), end=date(2026, 5, 21)
...
def test_run_writes_three_rows(monkeypatch):
    db = MagicMock()
    monkeypatch.setattr(
        "app.services.etl.wnba.spreads_accuracy_tracker.SessionLocal", lambda: db
    )
    monkeypatch.setattr(
