...
def normalize_weather(raw_csv: str, out_csv: str):
    ...
    if not {"park", "date", "tavg", "wspd"}.issubset(df.columns):
        sys.exit(
            "Error: weather‐raw must have either {park_id,date,tavg,wspd} or {park,date,tavg,wspd}"
        )
...
if __name__ == "__main__":
    p = argparse.ArgumentParser(description="Normalize raw park-factor & weather CSVs")
    p.add_argument(
        "--park_raw", required=True, help="Raw park-factor CSV (may be s3://...)"
    )
    p.add_argument(
        "--park_out",
        required=True,
    ...
    p.add_argument(
        "--weather_raw", required=True, help="Raw weather CSV (may be s3://...)"
    )
    p.add_argument(
        "--weather_out",
        required=True,
    ...
    normalize_park_factors(args.park_raw, args.park_out, args.year)
    normalize_weather(args.weather_raw, args.weather_out)
