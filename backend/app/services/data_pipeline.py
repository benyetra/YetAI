...
class SportsDataPipeline:
                            ...
                            try:
                                ...
                                if home_team and away_team:
                                    game = {
                                        ...
                                        "home_team_full": home_team["team"][
                                            "displayName"
                                        ],
                                        "away_team_full": away_team["team"][
                                            "displayName"
                                        ],
                                        "date": event["date"],
                                ...
                                logger.warning(
                                    f"Skipping game due to missing data: {e}"
                                )
                                continue

                        logger.info(
                            f"Successfully fetched {len(games)} NFL games with real data"
                        )
                        return games
                    else:
                    ...
                    if response.status == 200:
                        ...
                        for game in odds_data:
                            try:
                                ...
                                logger.warning(
                                    f"Skipping odds for game due to missing data: {e}"
                                )
                                continue
    ...
    def generate_simple_predictions(
        self, games: List[Dict], odds: List[Dict]
    ) -> List[Dict]:
                ...
                for team, data in spread_data.items():
                    if isinstance(data, dict) and "point" in data:
                        ...
                        if abs(line) >= 7:  # Large spread
                            ...
                            recommendation = (
                                f"{team_name} {'+' if line > 0 else ''}{line}"
                            )
