...
from app.services.etl.mlb.umpire_effects import (
    get_umpire_run_adjustment,
...
class HistoricalDataBuilder:
    ...
    def _reconstruct_pitcher_stats(self, pitcher_id, season, game_date):
        ...
        if not logs:
            ...
            if season > 2020:
                prior = self._reconstruct_pitcher_stats(
                    pitcher_id, season - 1, date(season, 1, 1)
                )
    ...
    def _reconstruct_team_hitting(self, team_id, season, game_date):
        ...
        games = cached_api_call(
            "team_schedule",
            {"team": team_id, "season": season, "before": game_date.isoformat()},
            _fetch,
            cache_only=self.cache_only,
        )
        ...
        if n_games < 10:
            ...
            return {
                "ops": round(blended_ops, 3),
                "runs_per_game": round(blended_rpg, 2),
            }
    ...
    def _reconstruct_bullpen_fatigue(self, team_id, game_date):
        ...
        games = cached_api_call(
            "team_7day",
            {"team": team_id, "before": game_date.isoformat()},
            _fetch,
            cache_only=self.cache_only,
        )
    ...
    def _reconstruct_umpire(self, game_id):
        ...
        data = cached_api_call(
            "game_data", {"game_id": game_id}, _fetch, cache_only=self.cache_only
        )
        ...
        try:
            officials = (
                data.get("liveData", {}).get("boxscore", {}).get("officials", [])
            )
    ...
    def _reconstruct_travel(
        self, team_id, team_name, opponent_name, is_home, game_date
    ):
