...
def compute_pitcher_stats_for_season(season: int) -> pd.DataFrame:
    ...
    if _raw_pitching_stats is None:
        print("Error: pybaseball.pitching_stats unavailable; install pybaseball.", file=sys.stderr)
        sys.exit(1)
    ...
    # 5) compute rates
    raw["HR9"] = raw.apply(
        lambda r: (9 * r["HR"] / r["IP"]) if pd.notna(r["IP"]) and r["IP"] > 0 else pd.NA,
        axis=1
    )
    raw["K9"] = raw.apply(
        lambda r: (9 * r["SO"] / r["IP"]) if pd.notna(r["IP"]) and r["IP"] > 0 else pd.NA,
        axis=1
    )
...
def main():
    ...
    p.add_argument("--output", required=True, help="Where to write pitcher_stats_all.csv (local or s3://)")
    args = p.parse_args()
