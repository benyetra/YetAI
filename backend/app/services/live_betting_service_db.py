...
class LiveBettingServiceDB:
    ...
    def place_live_bet(
        self, user_id: int, request: PlaceLiveBetRequest
    ) -> LiveBetResponse:
        ...
        if game_state.status in [
            GameStatus.FINAL,
            GameStatus.CANCELLED,
            GameStatus.POSTPONED,
        ]:
            return LiveBetResponse(
                success=False, error="Game has ended or is not available"
            )
        ...
        if current_odds_data:
            ...
            market_odds = self._get_market_odds(
                current_odds_data, request.bet_type, request.selection
            )
        ...
        try:
            ...
            return LiveBetResponse(success=False, error="Failed to place bet")
    ...
    def get_cash_out_offer(self, bet_id: str, user_id: int) -> Optional[CashOutOffer]:
        ...
        try:
            ...
            try:
                db_bet = (
                    db.query(LiveBetDB)
                    .filter(and_(LiveBetDB.id == bet_id, LiveBetDB.user_id == user_id))
                    .first()
                )
    ...
    def execute_cash_out(
        self, bet_id: str, user_id: int, accept_amount: float
    ) -> Dict[str, Any]:
            ...
            try:
                ...
                for db_bet in active_bets:
                    ...
                    if db_bet.cash_out_available:
                        ...
                        live_bet = self._db_bet_to_model(db_bet)
                        db_bet.cash_out_value = self._calculate_cash_out_value(
                            live_bet, game_update
                        )
    ...
    def update_live_odds(self, odds_update: LiveOddsUpdate):
        ...
        try:
            ...
            try:
                ...
                for db_bet in active_bets:
                    market_odds = self._get_market_odds(
                        odds_update, db_bet.bet_type, db_bet.selection
                    )
                    if market_odds:
                        db_bet.current_odds = market_odds
                        db_bet.current_potential_win = self._calculate_potential_win(
                            db_bet.amount, market_odds
                        )
    ...
    def get_user_live_bets(
        self, user_id: int, include_settled: bool = False
    ) -> List[LiveBet]:
        ...
        try:
            ...
            try:
                ...
                if not include_settled:
                    query = query.filter(LiveBetDB.status.in_([BetStatus.LIVE.value]))
                ...
                for db_bet in db_bets:
                    ...
                    if db_bet.status == BetStatus.LIVE.value:
                        ...
                        if not current_game_state:
                            ...
                            current_game_state = self._create_consistent_game_state(
                                db_bet.game_id, db_bet.sport
                            )
                        ...
                        if current_game_state:
                            ...
                            live_bet.cash_out_value = self._calculate_updated_cash_out(
                                db_bet, current_game_state
                            )
    ...
    def _db_bet_to_model(self, db_bet: LiveBetDB) -> LiveBet:
        ...
        if not getattr(db_bet, "home_team", None) or not getattr(
            db_bet, "away_team", None
        ):
        ...
        return LiveBet(
            ...
            bet_type=(
                db_bet.bet_type.value
                if hasattr(db_bet.bet_type, "value")
                else str(db_bet.bet_type)
            ),
    ...
    def _settle_db_bet(self, db_bet: LiveBetDB, final_game_state: LiveGameUpdate):
        ...
        if db_bet.bet_type == "moneyline":
            if (
                db_bet.selection == "home"
                and final_game_state.home_score > final_game_state.away_score
            ):
                won = True
            elif (
                db_bet.selection == "away"
                and final_game_state.away_score > final_game_state.home_score
            ):
                won = True
    ...
    async def _create_simple_live_market(self, game) -> Optional[LiveBettingMarket]:
        ...
        try:
            ...
            game_statuses = []
            if (
                "baseball" in str(game.sport_key).lower()
                or "mlb" in str(game.sport_key).lower()
            ):
                game_statuses = [
                    GameStatus.FIRST_INNING,
                    GameStatus.SECOND_INNING,
                    GameStatus.THIRD_INNING,
                    GameStatus.FOURTH_INNING,
                    GameStatus.FIFTH_INNING,
                    GameStatus.SIXTH_INNING,
                    GameStatus.SEVENTH_INNING,
                    GameStatus.EIGHTH_INNING,
            ...
            logger.info(
                f"✓ Created market: {market.home_team} vs {market.away_team} with {len(markets_available)} market types"
            )
            return market
    ...
    def _extract_odds_from_game(self, game, market_type: str) -> tuple:
        ...
        bookmaker_name = getattr(
            bookmaker, "title", getattr(bookmaker, "key", "Unknown")
        )
        ...
        for market in markets:
            ...
            market_key = (
                ...
                else getattr(market, "key", None)
            )
            if market_key == market_type:
                outcomes = (
                    market.get("outcomes")
                    if isinstance(market, dict)
                    else getattr(market, "outcomes", [])
                )
                ...
                if market_type == "h2h":
                    ...
                    for outcome in outcomes:
                        outcome_name = (
                            ...
                            else getattr(outcome, "name", "")
                        )
                        outcome_price = (
                            outcome.get("price")
                            if isinstance(outcome, dict)
                            else getattr(outcome, "price", 0)
                        )
    ...
    def _calculate_cash_out_value(
        self, bet: LiveBet, game_state: LiveGameUpdate
    ) -> float:
    ...
    def _create_consistent_game_state(
        self, game_id: str, sport: str, sport_key: str = None
    ):
        """Disabled to prevent fake game data - only use real live data"""
        logger.info(
            f"Skipping fake game state creation for {game_id} to avoid mock data"
        )
        return None
    ...
    def _generate_cash_out_offer(self, bet_id: str) -> CashOutOffer:
        ...
        try:
            ...
            try:
                ...
                live_bet = self._db_bet_to_model(db_bet)
                cash_out_value = (
                    self._calculate_cash_out_value(live_bet, game_state)
                    if game_state
                    else db_bet.cash_out_value
                )
    ...
    def _get_market_odds(
        self, odds_data: LiveOddsUpdate, bet_type: str, selection: str
    ) -> Optional[float]:
