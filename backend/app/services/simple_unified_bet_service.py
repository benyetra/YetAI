...
class SimpleUnifiedBetService:
    ...
    async def place_bet(self, user_id: int, bet_data: PlaceBetRequest) -> Dict:
        ...
        try:
            ...
            try:
                ...
                potential_win = self._calculate_potential_win(bet_data.odds, bet_data.amount)
                ...
                unified_bet = SimpleUnifiedBet(
                    ...
                    team_selection=parsed_selection.get('team_selection', TeamSide.NONE),
                    selected_team_name=parsed_selection.get('selected_team_name'),
                    spread_value=parsed_selection.get('spread_value'),
                    spread_selection=parsed_selection.get('spread_selection', TeamSide.NONE),
                    total_points=parsed_selection.get('total_points'),
                    over_under_selection=parsed_selection.get('over_under_selection', OverUnder.NONE),
                )
    ...
    async def place_parlay(self, user_id: int, parlay_data: PlaceParlayRequest) -> Dict:
        ...
        try:
            ...
            try:
                # Validate bet limits
                if not await self._check_bet_limits(user_id, parlay_data.total_amount, db):
                ...
                logger.info(f"Placed parlay {parlay_id} with {len(parlay_data.legs)} legs for user {user_id}")
    ...
    async def place_live_bet(self, user_id: int, live_bet_data: PlaceLiveBetRequest) -> Dict:
        ...
        try:
            ...
            try:
                ...
                live_bet = SimpleUnifiedBet(
                    ...
                    commence_time=getattr(live_bet_data, 'commence_time', datetime.utcnow()),
                    ...
                    cash_out_available=getattr(live_bet_data, 'cash_out_available', False),
    ...
    async def get_user_bets(self, user_id: int, include_legs: bool = False) -> List[Dict]:
        """Get all bets for a user"""
        try:
            db = SessionLocal()
            try:
                query = db.query(SimpleUnifiedBet).filter(SimpleUnifiedBet.user_id == user_id)
    ...
    async def get_bet_by_id(self, bet_id: str) -> Optional[Dict]:
        ...
        try:
            db = SessionLocal()
            try:
                bet = db.query(SimpleUnifiedBet).filter(SimpleUnifiedBet.id == bet_id).first()
                ...
                if bet.is_parlay:
                    legs = db.query(SimpleUnifiedBet).filter(
                        SimpleUnifiedBet.parent_bet_id == bet_id
                    ).order_by(SimpleUnifiedBet.leg_position).all()
    ...
    async def get_user_stats(self, user_id: int) -> Dict:
        ...
        try:
            ...
            try:
                ...
                bets = db.query(SimpleUnifiedBet).filter(
                    and_(
                ...
                total_won = sum(bet.result_amount or 0 for bet in bets if bet.status == BetStatus.WON)
                ...
                return {
                    ...
                    "straight_bets": len([bet for bet in bets if bet.source == BetSource.STRAIGHT]),
    ...
    def _parse_bet_selection(self, selection: str, bet_type: str, home_team: str, away_team: str) -> Dict:
    ...
    def _format_bet_response(self, bet: SimpleUnifiedBet) -> Dict:
        ...
        return {
            ...
            "spread_selection": bet.spread_selection.value if bet.spread_selection else None,
            "total_points": bet.total_points,
            "over_under_selection": bet.over_under_selection.value if bet.over_under_selection else None,
            # Live betting fields
    ...
    async def _check_bet_limits(self, user_id: int, amount: float, db: Session) -> bool:
        ...
        try:
            ...
            daily_total = db.query(func.sum(SimpleUnifiedBet.amount)).filter(
                and_(
                    SimpleUnifiedBet.user_id == user_id,
                    SimpleUnifiedBet.placed_at >= today,
                    SimpleUnifiedBet.parent_bet_id.is_(None)  # Don't count parlay legs
                )
            ).scalar() or 0
