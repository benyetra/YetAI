...
def extract_feature_names(model):
    ...
    if hasattr(model, "calibrated_classifiers_"):
        for clf in model.calibrated_classifiers_:
            ...
            base = getattr(clf, "estimator", None) or getattr(
                clf, "base_estimator", None
            )
...
def predict_and_store(daily_csv, lineup_csv, model_pkl, top_n):
    ...
    today = date.today()
    db_session.query(DailyHRPredictions).filter(
        DailyHRPredictions.date == today
    ).delete()
    db_session.commit()
...
def main():
    parser = argparse.ArgumentParser(
        description="Load daily features, predict HR, and store top-n"
    )
    ...
    parser.add_argument(
        "--top", type=int, default=10, help="number of top probabilities to store"
    )
