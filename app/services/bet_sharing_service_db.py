...
class BetSharingServiceDB:
        ...
        try:
            ...
            try:
                ...
                shareable_data = {
                    ...
                    "leg_count": bet_data.get("leg_count") or len(bet_data.get("legs", [])),
    ...
    async def get_shared_bet(self, share_id: str, viewer_ip: Optional[str] = None) -> Dict:
        ...
        try:
            db = SessionLocal()
            try:
                shared_bet = db.query(SharedBet).filter(SharedBet.id == share_id).first()
    ...
    async def get_user_shared_bets(self, user_id: int) -> Dict:
        ...
        try:
            db = SessionLocal()
            try:
                shared_bets = db.query(SharedBet).filter(
                    SharedBet.user_id == user_id
                ).order_by(SharedBet.created_at.desc()).all()
    ...
    async def delete_shared_bet(self, user_id: int, share_id: str) -> Dict:
        ...
        try:
            db = SessionLocal()
            try:
                shared_bet = db.query(SharedBet).filter(
                    and_(
                        SharedBet.id == share_id,
                        SharedBet.user_id == user_id
                    )
                ).first()
                
                if not shared_bet:
                    return {"success": False, "error": "Shared bet not found or unauthorized"}
    ...
    async def get_bet_for_sharing(self, user_id: int, bet_id: str, is_parlay: bool = False) -> Dict:
    ...
    async def cleanup_expired_shares(self) -> Dict:
        ...
        try:
            db = SessionLocal()
            try:
                expired_count = db.query(SharedBet).filter(
