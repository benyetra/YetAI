...
async def debug_game_id_mapping():
    ...
    db = SessionLocal()
    try:
        pending_bet_game_ids = db.query(Bet.game_id).filter(
            Bet.status == 'PENDING',
                ...
                try:
                    ...
                    if bet and bet.home_team and bet.away_team:
                        ...
                        for api_score in nfl_scores[:10]:
                            if (bet.home_team.lower() in api_score.home_team.lower() or
                                bet.away_team.lower() in api_score.away_team.lower()):
                                print(f"  🎯 Potential match: {api_score.away_team} @ {api_score.home_team}")
