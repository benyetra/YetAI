...
from app.models.predictions_models import (
    BullpenFatigue, Weather, BlowoutChances, GameProjections
)
...
from app.services.etl.mlb.weather_enhanced import compute_weather_run_adjustment, get_dynamic_park_factor
...
WIN_CALIBRATOR_LOCAL = os.path.join(os.path.dirname(__file__), 'game_model_win_calibrator.pkl')
WIN_CALIBRATOR_S3_KEY = 'mlb/game_model_win_calibrator.pkl'
TRAINING_CACHE_DB = os.path.join(os.path.dirname(__file__), 'training_features_cache.db')
...
VENUE_PARK_ID_MAP = {
    ...
    'Fenway Park': 'bos', 'Wrigley Field': 'chc', 'Guaranteed Rate Field': 'chw',
    ...
    'Sutter Health Park': 'oak', 'Oakland Coliseum': 'oak',
    'Citizens Bank Park': 'phi', 'PNC Park': 'pit', 'Petco Park': 'sd',
    'Oracle Park': 'sf', 'T-Mobile Park': 'sea', 'Busch Stadium': 'stl',
    'Tropicana Field': 'tb', 'Globe Life Field': 'tex', 'Rogers Centre': 'tor',
    'Nationals Park': 'wsh',
}
...
FEATURE_COLS = [
    ...
    'park_factor', 'temperature', 'wind_speed',
    'home_recent_runs_avg', 'away_recent_runs_avg',
    'home_recent_runs_allowed_avg', 'away_recent_runs_allowed_avg',
...
def feature_coverage_report(df):
    ...
    rows.sort(
        key=lambda r: (r['pct_at_neutral_default'] is None, -(r['pct_at_neutral_default'] or 0)),
    )
...
def load_park_factors():
    ...
    try:
        ...
        if sample and all(v > 10 for v in sample):
            ...
            logger.info(f"Loaded {len(PARK_FACTOR_MAP)} park factors (normalized from 100-scale)")
        else:
    ...
    try:
        ...
        if 'stats' in data and data['stats']:
            ...
            if splits:
                ...
                return {'ops': ops, 'runs_per_game': round(runs / games, 2), 'games': games}
    except Exception:
        pass
    return None
            ...
            if prev and prev['games'] >= 50:
                if current and current['games'] >= 3:
                    ...
                    return {
                        ...
                        'runs_per_game': round(w * current['runs_per_game'] + (1 - w) * prev['runs_per_game'], 2),
...
def get_weather_for_venue(venue_name, target_date=None):
    ...
    try:
        weather = db_session.query(Weather).filter(
...
def build_game_features(game_info):
    ...
    try:
        home_travel = get_travel_run_adjustment(
            home_id, game_info.get('home_name', ''),
            game_info.get('away_name', ''), is_home=True
        )
        away_travel = get_travel_run_adjustment(
            away_id, game_info.get('away_name', ''),
            game_info.get('home_name', ''), is_home=False
        )
    except Exception:
        home_travel = 0.0
        away_travel = 0.0
...
def predict_total_runs_heuristic(features):
    ...
    home_pitcher_adj = (4.50 - features['away_starter_era']) * 0.3  # Away pitcher faces home team
...
def _fetch_team_game_log(conn, team_id, season):
    ...
    if not team_id:
        return
    row = conn.execute(
        'SELECT 1 FROM team_log_fetched WHERE team_id=? AND season=?',
        (team_id, season)
    ).fetchone()
    if row:
        return
    ...
    try:
        ...
        h_splits = (h_resp.get('stats') or [{}])[0].get('splits', []) if h_resp.get('stats') else []
        ...
        p_splits = (p_resp.get('stats') or [{}])[0].get('splits', []) if p_resp.get('stats') else []
...
def tune_ensemble_weights(ensemble, val_df, target_col, classification=True):
    ...
    w0 = np.array([float(default_weights.get(n, 1.0 / len(names))) for n in names], dtype=float)
    w0 = w0 / w0.sum()
    ...
    result = minimize(
        _metric, w0, method='SLSQP', bounds=bounds, constraints=constraints,
...
def build_historical_training_data(
    ...
    for season in seasons:
        ...
        team_ids = {g.get('home_id') for g in finals} | {g.get('away_id') for g in finals}
...
def train_game_models(df, fast=False, tune_weights=False, val_fraction=0.15):
    ...
    if fast:
        win_pipeline.set_params(**{f'clf__{k}': v for k, v in FAST_XGB_WIN_PARAMS.items()})
        ...
        win_search = RandomizedSearchCV(
            win_pipeline, win_params, n_iter=20,
    ...
    win_ensemble = {
        ...
        'weights': {'xgboost': 0.40, 'random_forest': 0.25,
                    'logistic_regression': 0.15, 'catboost': 0.20},
    }
    ...
    if fast:
        total_pipeline.set_params(**{f'reg__{k}': v for k, v in FAST_XGB_TOTAL_PARAMS.items()})
        ...
        total_search = RandomizedSearchCV(
            total_pipeline, total_params, n_iter=20,
    ...
    total_ensemble = {
        ...
        'catboost': cb_total,
        'weights': {'xgboost': 0.40, 'random_forest': 0.25,
                    'elastic_net': 0.15, 'catboost': 0.20},
    }

    if val_df is not None:
        win_ensemble = tune_ensemble_weights(
            win_ensemble, val_df, 'home_win', classification=True,
        )
        total_ensemble = tune_ensemble_weights(
            total_ensemble, val_df, 'total_runs', classification=False,
        )
...
def fit_and_save_win_calibrator(
    ...
    train_core, cal_df = split_calibration_holdout(
        train_df, val_fraction, min_cal_rows,
    )
    ...
    if HAS_BOTO3:
        try:
            ...
            logger.info('Saved win calibrator to s3://%s/%s', S3_BUCKET, WIN_CALIBRATOR_S3_KEY)
...
def predict_games(games):
    ...
    if use_ml:
        logger.info(f"Using {'ensemble' if use_ensemble else 'single'} ML models for predictions")
    else:
    ...
    for game in games:
        ...
        home_runs = round(projected_total * home_win_prob / max(home_win_prob + away_win_prob, 0.01), 1)
...
def main():
    ...
    parser.add_argument('--no-tune-weights', action='store_true',
    ...
    parser.add_argument('--accurate-bullpen', action='store_true',
                        help='With --train: boxscore-based bullpen fatigue (much slower)')
    parser.add_argument('--cal-train-full', action='store_true',
                        help='With --evaluate: train on full season, calibrate on tail only')
    parser.add_argument('--compare-cal-train-modes', action='store_true',
    ...
    with app.app_context():
        ...
        if args.train:
            ...
            logger.info(f"Training complete. Ensemble models saved "
            ...
            seasons = args.seasons if args.seasons else [CURRENT_SEASON - 2, CURRENT_SEASON - 1]
            if len(set(seasons)) < 2:
                logger.error('--evaluate needs at least two distinct years in --seasons')
                return
