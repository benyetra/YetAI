...
class FantasyTeam(Base):
    ...
    )  # True if this is the connected user's team
...
class FantasyRecommendation(Base):
    ...
    secondary_player_id = Column(
        Integer, ForeignKey("fantasy_players.id")
    )  # For trades
...
class CompetitorAnalysis(Base):
    ...
    faab_conservation_tendency = Column(
        String(20)
...
class DraftPick(Base):
    ...
    id = Column(Integer, primary_key=True, index=True)
    league_id = Column(Integer, ForeignKey("fantasy_leagues.id"), nullable=False)
    current_owner_team_id = Column(
        Integer, ForeignKey("fantasy_teams.id"), nullable=False
    )
    original_owner_team_id = Column(
        Integer, ForeignKey("fantasy_teams.id"), nullable=False
    )
...
class Trade(Base):
    ...
    team1_gives = Column(
        JSON
    )  # {"players": [player_ids], "picks": [pick_ids], "faab": amount}
    team2_gives = Column(
        JSON
    )  # {"players": [player_ids], "picks": [pick_ids], "faab": amount}
...
class TradeRecommendation(Base):
    ...
    trade_id = Column(
        Integer, ForeignKey("trades.id")
    )  # If trade was actually proposed

    # Recommendation details
    recommendation_type = Column(
        String(50)
    )  # "position_need", "buy_low", "sell_high", etc.
...
Index(
    "idx_projections_player_week",
    PlayerProjection.player_id,
    PlayerProjection.week,
    PlayerProjection.season,
)
Index(
    "idx_recommendations_user_active",
    FantasyRecommendation.user_id,
    FantasyRecommendation.is_active,
)
Index(
    "idx_waiver_targets_league_week", WaiverWireTarget.league_id, WaiverWireTarget.week
)
Index(
    "idx_historical_data_league_season",
    LeagueHistoricalData.league_id,
    LeagueHistoricalData.season,
)
Index(
    "idx_competitor_analysis_league_team",
    CompetitorAnalysis.league_id,
    CompetitorAnalysis.team_id,
)
Index(
    "idx_player_analytics_player_week",
...
Index(
    "idx_player_analytics_snap_share",
    PlayerAnalytics.player_id,
    PlayerAnalytics.snap_percentage,
)
Index(
    "idx_player_analytics_target_share",
    PlayerAnalytics.player_id,
...
Index(
    "idx_trade_recommendations_active",
    TradeRecommendation.league_id,
    TradeRecommendation.is_active,
)
Index(
    "idx_player_values_league_week",
    PlayerValue.league_id,
    PlayerValue.week,
    PlayerValue.season,
)
Index(
    "idx_team_needs_analysis_team_week",
    TeamNeedsAnalysis.team_id,
    TeamNeedsAnalysis.week,
)
