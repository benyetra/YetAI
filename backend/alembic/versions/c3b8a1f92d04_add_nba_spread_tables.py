...
def upgrade() -> None:
    if not _has_table("pred_nba_spread_projections"):
        op.create_table(
    ...
    if not _has_index("pred_nba_spread_projections", "idx_nba_spread_projections_date"):
        op.create_index(
            "idx_nba_spread_projections_date",
            "pred_nba_spread_projections",
            ["game_date"],
        )
    if not _has_index("pred_nba_spread_projections", "idx_nba_spread_projections_edge"):
        op.create_index(
    ...
    if not _has_table("pred_nba_spread_actuals"):
        op.create_table(
    ...
    if not _has_index("pred_nba_spread_actuals", "idx_nba_spread_actuals_date"):
        op.create_index(
            "idx_nba_spread_actuals_date", "pred_nba_spread_actuals", ["game_date"]
        )

    if not _has_table("pred_nba_spread_accuracy"):
        op.create_table(
...
def downgrade() -> None:
    if _has_table("pred_nba_spread_accuracy"):
        op.drop_table("pred_nba_spread_accuracy")
    if _has_index("pred_nba_spread_actuals", "idx_nba_spread_actuals_date"):
        op.drop_index(
            "idx_nba_spread_actuals_date", table_name="pred_nba_spread_actuals"
        )
    if _has_table("pred_nba_spread_actuals"):
        op.drop_table("pred_nba_spread_actuals")
    if _has_index("pred_nba_spread_projections", "idx_nba_spread_projections_edge"):
        op.drop_index(
            "idx_nba_spread_projections_edge",
            table_name="pred_nba_spread_projections",
        )
    if _has_index("pred_nba_spread_projections", "idx_nba_spread_projections_date"):
        op.drop_index(
            "idx_nba_spread_projections_date",
            table_name="pred_nba_spread_projections",
        )
    if _has_table("pred_nba_spread_projections"):
