    ...
    async def audit_all_endpoints(self):
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if games:
                    ...
                    print(f"   Sample: {sample_game.get('away_team_full')} @ {sample_game.get('home_team_full')}")
                    print(f"   Date: {sample_game.get('date')}")
                    print(f"   Venue: {sample_game.get('venue')}")
                    
                    # Check if this looks like real data
                    has_real_teams = any(team in str(sample_game) for team in ['Chiefs', 'Bills', 'Cowboys', 'Patriots'])
                    has_real_venue = sample_game.get('venue') and len(sample_game.get('venue', '')) > 10
                    
                    results['games'] = {
                        'status': 'real' if has_real_teams and has_real_venue else 'questionable',
                        'count': len(games),
                        'sample': sample_game
                    }
                else:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if odds:
                    ...
                    print(f"   Sample: {sample_odds.get('away_team')} @ {sample_odds.get('home_team')}")
                    ...
                    if sample_odds.get('bookmakers'):
                        ...
                        real_books = ['DraftKings', 'FanDuel', 'BetMGM', 'Caesars', 'PointsBet']
                        has_real_bookmaker = any(book in bookmaker.get('name', '') for book in real_books)
                        ...
                        results['odds'] = {'status': 'no_bookmakers', 'count': len(odds)}
                else:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if projections:
                    ...
                    print(f"✅ Basic Fantasy API Working: {len(projections)} projections")
                    print(f"   Sample: {sample_proj.get('name')} ({sample_proj.get('position')})")
                    ...
                    has_generic_reasoning = any(phrase in sample_proj.get('reasoning', '').lower() 
                                              for phrase in ['strong matchup', 'good volume', 'solid projection'])
                    
                    results['fantasy_basic'] = {
                        'status': 'likely_mock' if has_generic_reasoning else 'questionable',
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                print(f"✅ Enhanced Fantasy API Working: {len(projections)} projections")
                ...
                if projections:
                    ...
                    print(f"   Injury Status: {sample_proj.get('injury_status', 'unknown')}")
                    print(f"   Weather Impact: {sample_proj.get('weather_impact', 'unknown')}")
                    
                    # Check if this looks like real data
                    has_stats_based = sample_proj.get('stats_based', False)
                    has_injury_data = sample_proj.get('injury_status', 'unknown') != 'unknown'
                    has_weather_data = sample_proj.get('weather_impact', 'unknown') != 'unknown'
                    
                    real_score = sum([has_stats_based, has_injury_data, has_weather_data])
                    
                    results['fantasy_enhanced'] = {
                        'status': 'real' if real_score >= 2 else 'partially_real' if real_score >= 1 else 'mock',
                        'count': len(projections),
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                results['performance'] = {
                    'status': 'simulated_realistic' if 50 <= accuracy <= 90 and total_preds > 0 else 'questionable',
                    'metrics': metrics
                }
            else:
