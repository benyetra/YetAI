...
def store_game_projections(predictions, target_date=None):
    ...
    for pred in predictions:
        existing = db_session.query(GameProjections).filter_by(
            date=target_date,
...
def store_game_actuals(target_date):
    ...
    for game in schedule:
        ...
        proj = db_session.query(GameProjections).filter_by(
            date=target_date,
            game_id=game_id
        ).first()
        ...
        if proj:
            ...
            if proj.run_line and proj.market_spread:
                ...
                spread_correct = (actual_margin > proj.market_spread) if proj.ml_recommendation == 'HOME' else None

            projection_error = round(total_runs - projected_total, 1) if projected_total else None

        # Upsert
        existing = db_session.query(GameActuals).filter_by(
            date=target_date,
            game_id=game_id
        ).first()
...
def run_game_projection_pipeline(target_date=None):
    ...
    logger.info("Step 2: Generating game predictions (with Technical Playbook features)...")
    from app.services.etl.mlb.game_model import get_todays_games, predict_games, load_park_factors
    load_park_factors()
    ...
    for pred in predictions:
        ...
        if odds_data:
            ...
            pred['ml_recommendation'] = 'HOME' if pred['home_win_prob'] > 0.55 else ('AWAY' if pred['home_win_prob'] < 0.45 else 'NO_PLAY')
    ...
    try:
        ...
        blowouts = {b.game_id: b.run_differential for b in db_session.query(BlowoutChances).all()}
        for pred in predictions:
...
def main():
    ...
    parser.add_argument('--store-actuals', action='store_true', help='Store game actuals (post-game)')
