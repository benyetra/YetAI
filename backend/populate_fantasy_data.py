...
def populate_fantasy_data():
    ...
    try:
        ...
        for sleeper_player in sleeper_players:
            ...
            existing_fp = db.query(FantasyPlayer).filter(
        ...
        for fantasy_team in fantasy_teams:
            ...
            sleeper_roster = db.query(SleeperRoster).filter(
            ...
            if sleeper_roster and sleeper_roster.players:
                for player_id in sleeper_roster.players:
                    ...
                    if fantasy_player:
                        ...
                        existing_spot = db.query(FantasyRosterSpot).filter(
        ...
        if sample_team:
            roster_spots = db.query(FantasyRosterSpot).filter(
                FantasyRosterSpot.team_id == sample_team.id
            ).limit(5).all()
            
            print(f"\nSample roster for team '{sample_team.name}':")
            for spot in roster_spots:
                print(f"  {spot.player.name} ({spot.position}) - Starter: {spot.is_starter}")
