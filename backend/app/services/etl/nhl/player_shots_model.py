...
from app.models.predictions_models import NHLPlayer, NHLTeamStats, NHLPlayerShotsPredictions
...
def generate_daily_player_predictions(game_date=None):
    ...
    top_players = db_session.query(NHLPlayer).filter(
        NHLPlayer.shots > 100
    ).order_by(NHLPlayer.shots.desc()).limit(20).all()
    ...
    for i, player in enumerate(top_players[:10]):  # Top 10 for demo
        ...
        if 'error' not in pred:
            ...
            print(f"  {'Home' if pred['is_home'] else 'Away':4} | "
            ...
            if pred['ice_time_adjustment'] != 1.0:
                print(f"  📊 Ice time: {pred['ice_time_adjustment']:.2f}x "
                      f"({pred['player_toi_per_game']:.0f} sec/game)")

            if pred['opponent_shots_adjustment'] != 1.0:
                print(f"  🎯 Opponent allows: {pred['opponent_shots_against_pg']:.1f} shots/game "
                      f"({pred['opponent_shots_adjustment']:.2f}x)")

            if pred['blocks_adjustment'] != 1.0:
                print(f"  🛡️  Opponent blocks: {pred['opponent_blocks_pg']:.1f}/game "
...
if __name__ == "__main__":
        ...
        if player:
            ...
            print(f"Season stats: {player.shots} shots in {player.games_played} GP ({player.shots_per_game:.2f}/game)\n")
