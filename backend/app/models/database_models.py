...
class User(Base):
    ...
    shared_bets = relationship("SharedBet", back_populates="user", cascade="all, delete-orphan")
    yetai_bets = relationship("YetAIBet", back_populates="user", cascade="all, delete-orphan")
    live_bets = relationship("LiveBet", back_populates="user", cascade="all, delete-orphan")
...
class Game(Base):
    ...
    weather = Column(JSON)
    odds_data = Column(JSON)  # Store latest odds
...
class YetAIBet(Base):
    ...
    id = Column(String(255), primary_key=True, index=True)  # UUID
    user_id = Column(Integer, ForeignKey("users.id"), nullable=True)  # null for admin-created
    game_id = Column(String(255), ForeignKey("games.id"), nullable=True)
...
class BetHistory(Base):
    ...
    action = Column(String(50), nullable=False)  # placed, settled, cancelled, cashed_out
