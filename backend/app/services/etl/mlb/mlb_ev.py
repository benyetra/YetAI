...
def get_todays_games():
    ...
    games = []
    for g in sched:
        home_pid = g.get("home_probable_pitcher_id") or get_player_id_by_name(
            g.get("home_probable_pitcher")
        )
        away_pid = g.get("away_probable_pitcher_id") or get_player_id_by_name(
            g.get("away_probable_pitcher")
        )
        if not home_pid or not away_pid:
            logging.warning(
                f"Skipping {g['away_name']} @ {g['home_name']}—missing pitcher ID"
            )
            continue
        try:
...
def weighted_lineup_ops(batter_ids):
    ...
    for pid in batter_ids:
        try:
            res = mlbstatsapi.player_stat_data(
                pid, group="hitting", type="season", season=str(CURRENT_SEASON)
            )
            stats = {}
            if isinstance(res, list):
                entry = next(
                    (r for r in res if r.get("season") == str(CURRENT_SEASON)), None
                )
...
def evaluate_pitcher(pid):
    ...
    try:
        ...
        if isinstance(ystats, list):
            rec = next(
                (r for r in ystats if r.get("season") == str(CURRENT_SEASON)), None
            )
        ...
        pass
    try:
        sstats = mlbstatsapi.player_stat_data(
            pid, group="pitching", type="season", season=str(CURRENT_SEASON)
        )
        stats = {}
        if isinstance(sstats, list):
            rec = next(
                (r for r in sstats if r.get("season") == str(CURRENT_SEASON)), None
            )
            stats = rec.get("stats", {}) if rec else {}
    ...
    for g in schedule:
        ...
        h_prob = max(
            ...
            min(
                1,
                h_prob
                + bullpen_adjustment(get_bullpen_stats(g["home_id"])["era"])
