    ...
    try:
        ...
        if 'stats' in data and data['stats']:
            ...
            if splits:
                ...
                games_played = team_stats.get('gamesPlayed', 1)  # Default to 1 to avoid division by zero
                runs = team_stats.get('runs', 0)
                runs_allowed = team_stats.get('runsAgainst', 0)  # Assuming this field exists similarly
...
def evaluate_blowout_chances(pitchers, batters):
    ...
    for game in get_todays_games():
        ...
        home_avg_runs, home_avg_runs_allowed = get_team_performance_metrics(home_team_id)
        away_avg_runs, away_avg_runs_allowed = get_team_performance_metrics(away_team_id)

        home_pitcher = next((pitcher for pitcher in pitchers if pitcher['team'] == game['home_name']), None)
        away_pitcher = next((pitcher for pitcher in pitchers if pitcher['team'] == game['away_name']), None)

        home_batters = [batter for batter in batters if batter['team'] == game['home_name']]
        away_batters = [batter for batter in batters if batter['team'] == game['away_name']]

        if home_pitcher:
            home_pitcher_effectiveness = calculate_pitcher_effectiveness(home_pitcher, away_batters)
        else:
            home_pitcher_effectiveness = 0

        if away_pitcher:
            away_pitcher_effectiveness = calculate_pitcher_effectiveness(away_pitcher, home_batters)
        else:
            away_pitcher_effectiveness = 0

        home_expected_runs = home_avg_runs + away_pitcher_effectiveness - away_avg_runs_allowed
        away_expected_runs = away_avg_runs + home_pitcher_effectiveness - home_avg_runs_allowed
...
def calculate_pitcher_effectiveness(pitcher, opposing_batters):
    ...
    for batter in opposing_batters:
        batter_avg = batter.get('batting_average_vs_pitcher') or batter.get('season_avg_vs_handed', 0)
        total_effectiveness += batter_avg

    average_effectiveness = total_effectiveness / len(opposing_batters) if opposing_batters else 0
