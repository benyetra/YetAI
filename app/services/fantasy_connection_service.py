...
class FantasyConnectionService:
    ...
    async def _connect_sleeper(
        self, user_id: int, credentials: Dict[str, Any]
    ) -> Dict[str, Any]:
    ...
    async def get_user_connections(self, user_id: int) -> Dict[str, Any]:
        ...
        try:
            ...
            connections = (
                db.query(FantasyUser)
                .filter(FantasyUser.user_id == user_id, FantasyUser.is_active == True)
                .all()
            )
                ...
                result.append(
                    {
                        ...
                        "last_sync": (
                            conn.last_sync.isoformat() if conn.last_sync else None
                        ),
                        "status": "active",
                    }
                )
            ...
            for connection in connections:
                if connection.platform == FantasyPlatform.SLEEPER:
                    try:
                        ...
                        leagues = await self.sleeper_service.get_user_leagues(
                        ...
                        logger.info(
                            f"Found {len(leagues)} leagues for Sleeper user {connection.platform_username}"
                        )

                    except Exception as e:
                        logger.error(
                            f"Error fetching leagues for Sleeper user {connection.platform_user_id}: {e}"
                        )
                        continue
    ...
    async def disconnect_platform(
        ...
        try:
            ...
            logger.info(
                f"Disconnected {connection.platform.value} connection for user {user_id}"
            )
