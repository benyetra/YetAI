...
def upgrade() -> None:
    ...
    op.add_column('fantasy_leagues', sa.Column('waiver_type', sa.String(length=50), nullable=True))
    op.add_column('fantasy_leagues', sa.Column('waiver_budget', sa.Integer(), nullable=True))
    op.add_column('fantasy_leagues', sa.Column('waiver_clear_days', sa.Integer(), nullable=True))
    op.add_column('fantasy_leagues', sa.Column('roster_positions', sa.JSON(), nullable=True))
    ...
    op.create_index('idx_historical_data_league_season', 'league_historical_data', ['league_id', 'season'], unique=False)
    ...
    op.create_index('idx_competitor_analysis_league_team', 'competitor_analysis', ['league_id', 'team_id'], unique=False)
...
def downgrade() -> None:
    ...
    op.drop_index('idx_competitor_analysis_league_team', table_name='competitor_analysis')
    op.drop_table('competitor_analysis')
    
    op.drop_index(op.f('ix_league_historical_data_id'), table_name='league_historical_data')
    op.drop_index('idx_historical_data_league_season', table_name='league_historical_data')
