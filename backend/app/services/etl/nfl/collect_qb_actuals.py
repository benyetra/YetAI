...
def collect_qb_game_stats(season: int, week: int) -> List[Dict]:
    ...
    try:
        ...
        # Aggregate QB stats
        qb_stats = week_data.groupby(['passer_player_id', 'passer_player_name', 'posteam']).agg({
            'passing_yards': 'sum',
            'complete_pass': 'sum',
            'incomplete_pass': 'sum',
            'pass_attempt': 'sum',
        ...
        qb_stats['completion_pct'] = (qb_stats['complete_pass'] / qb_stats['pass_attempt']) * 100
        ...
        for _, qb_row in qb_stats.iterrows():
            ...
            if not team_games.empty:
                ...
                opponent = game['away_team'] if game['home_team'] == qb_row['posteam'] else game['home_team']
...
def match_predictions_with_actuals(qb_actuals: List[Dict], season: int, week: int) -> List[Dict]:
    """
    ...
    for actual in qb_actuals:
        try:
            ...
            prediction = db_session.query(QBPredictions).filter_by(
                qb_player_name=full_name,
                season=season,
                week=week
            ).first()
            ...
            if prediction:
                ...
                prediction_accuracy = abs(prediction_error) / actual_yards if actual_yards > 0 else 0
                ...
                if prediction.betting_recommendation and prediction.ou_line:
                    ...
                    accuracy_threshold = min(0.15 * actual_yards, 20)  # 15% or 20 yards, whichever is smaller
                ...
                actual.update({
                    ...
                    'prediction_confidence': float(prediction.model_confidence or 0.5),
                    'prediction_method': prediction.prediction_method or 'unknown',
                    'betting_recommendation': prediction.betting_recommendation,
                    'ou_line': float(prediction.ou_line) if prediction.ou_line else None
                })
                ...
                print(f"  ✅ {actual['qb_player_name']}: {actual_yards} yards "
...
def save_qb_actuals(qb_results: List[Dict]) -> Dict:
    ...
    for qb_data in qb_results:
        try:
            ...
            existing_actual = db_session.query(QBActuals).filter_by(
            ...
            if existing_actual:
                print(f"  ⚠️ Actual result already exists for {qb_data['qb_player_name']} Week {qb_data['week']}")
                continue
