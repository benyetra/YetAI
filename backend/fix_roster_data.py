...
def fix_roster_data():
    ...
    try:
        ...
        actual_roster = [
            ...
            {"name": "Brian Robinson Jr.", "position": "RB", "team": "ATL"},  # Listed as B Robinson
            {"name": "Isaiah Guerendo", "position": "RB", "team": "SF"},
            {"name": "Deebo Samuel", "position": "WR", "team": "WAS"},  # Listed as D Samuel
            {"name": "Mike Evans", "position": "WR", "team": "TB"},  # Listed as M Evans
            {"name": "Evan Engram", "position": "TE", "team": "DEN"},  # Listed as E Engram
            {"name": "Joe Mixon", "position": "RB", "team": "HOU"},  # Listed as J Mixon
            {"name": "Minnesota Vikings", "position": "DEF", "team": "MIN"},  # Listed as MIN DEF
            {"name": "Jared Goff", "position": "QB", "team": "DET"},  # Listed as J Goff
            {"name": "C.J. Stroud", "position": "QB", "team": "HOU"},  # Listed as C Stroud
            {"name": "Chuba Hubbard", "position": "RB", "team": "CAR"},  # Listed as C Hubbard
            {"name": "Jaylen Waddle", "position": "WR", "team": "MIA"},  # Listed as J Waddle
            {"name": "Chris Olave", "position": "WR", "team": "NO"},  # Listed as C Olave
        ...
        existing_spots = db.query(FantasyRosterSpot).filter(FantasyRosterSpot.team_id == team.id).all()
        ...
        for player_info in actual_roster:
            ...
            fantasy_player = db.query(FantasyPlayer).filter(
                FantasyPlayer.name.ilike(f"%{player_info['name']}%")
            ).first()
        ...
        # Verify the update
        updated_spots = db.query(FantasyRosterSpot).filter(FantasyRosterSpot.team_id == team.id).all()
