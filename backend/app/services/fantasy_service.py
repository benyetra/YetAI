...
class FantasyPlatformInterface(ABC):
    ...
    async def get_team_roster(
        self, team_id: str, week: Optional[int] = None
    ) -> List[Dict[str, Any]]:
...
class FantasyService:
    ...
    def register_platform(
        self, platform: FantasyPlatform, interface: FantasyPlatformInterface
    ):
    ...
    async def connect_user_account(
        self, user_id: int, platform: FantasyPlatform, credentials: Dict[str, Any]
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            existing_fantasy_user = (
                self.db.query(FantasyUser)
                .filter(
                    and_(
    ...
    async def sync_user_leagues(self, fantasy_user_id: int) -> Dict[str, Any]:
        ...
        try:
            fantasy_user = (
                self.db.query(FantasyUser)
                .filter(FantasyUser.id == fantasy_user_id)
                .first()
            )
            if not fantasy_user:
            ...
            leagues_data = await platform_interface.get_user_leagues(
                fantasy_user.platform_user_id
            )
            ...
            self.db.rollback()
            logger.error(
                f"Failed to sync leagues for fantasy user {fantasy_user_id}: {str(e)}"
            )
            return {"success": False, "error": str(e)}

    async def _sync_league(
        self, fantasy_user: FantasyUser, league_data: Dict[str, Any]
        ...
        try:
            ...
            if existing_league:
                ...
                league.scoring_type = league_data.get(
                    "scoring_type", league.scoring_type
                )
                # Update waiver settings
                league.waiver_type = waiver_settings.get(
                    "waiver_type", league.waiver_type
                )
                league.waiver_budget = waiver_settings.get(
                    "waiver_budget", league.waiver_budget
                )
                league.waiver_clear_days = waiver_settings.get(
                    "waiver_clear_days", league.waiver_clear_days
                )
            else:
                # Create new league
            ...
            # Sync teams
            await self._sync_league_teams(
                league, league_data.get("teams", []), fantasy_user
            )
            ...
            logger.error(
                f"Failed to sync league {league_data.get('league_id')}: {str(e)}"
            )
            return None
    ...
    async def _sync_league_teams(
        ...
        try:
            ...
            for team_data in teams_data:
                ...
                is_user_team = (
                    team_data.get("owner_id") == fantasy_user.platform_user_id
                )
                ...
                if existing_team:
                    ...
                    team.points_against = team_data.get(
                        "points_against", team.points_against
                    )
                    team.waiver_position = team_data.get(
    ...
    def get_user_fantasy_accounts(self, user_id: int) -> List[Dict[str, Any]]:
        ...
        fantasy_users = (
            self.db.query(FantasyUser)
            .filter(and_(FantasyUser.user_id == user_id, FantasyUser.is_active == True))
            .all()
        )
    ...
    def _get_user_team(self, league: FantasyLeague) -> Optional[Dict[str, Any]]:
        ...
        user_team = (
            ...
            .filter(
                and_(
                    FantasyTeam.league_id == league.id, FantasyTeam.is_user_team == True
                )
            )
            .first()
        )
    ...
    async def generate_start_sit_recommendations(
        self, user_id: int, week: int
    ) -> List[Dict[str, Any]]:
        ...
        for league_data in user_leagues:
            ...
            league = (
                self.db.query(FantasyLeague)
                .filter(FantasyLeague.id == league_id)
                .first()
            )
            ...
            try:
                ...
                logger.error(
                    f"Failed to generate start/sit recommendations for league {league_id}: {str(e)}"
                )
                continue

        # Sort by confidence and projected impact
        recommendations.sort(
            key=lambda x: (x.get("confidence", 0), x.get("projected_points", 0)),
            reverse=True,
        )
        return recommendations[:15]  # Return top 15 recommendations

    async def _generate_start_sit_for_league(
        self, league_id: int, league_name: str, roster_data: List[Dict], week: int
    ) -> List[Dict[str, Any]]:
    ...
    async def _rank_players_by_performance(
        self, players: List[Dict], week: int
    ) -> List[Dict]:
    ...
    def _generate_basic_projection(self, player: Dict, week: int) -> Dict:
        ...
        if recent_analytics and len(recent_analytics) >= 2:
            ...
            return self._generate_analytics_based_projection(
                player, recent_analytics, week
            )
        else:
            # Fallback to enhanced baseline with league context
            return self._generate_enhanced_baseline_projection(player, week)

    def _generate_analytics_based_projection(
        self, player: Dict, analytics_data, week: int
    ) -> Dict:
        ...
        if len(recent_points) >= 3:
            trend_score = (recent_points[0] + recent_points[1]) / 2 - (
                recent_points[-2] + recent_points[-1]
            ) / 2
        else:
            trend_score = 0
        ...
        avg_snap_pct = sum(
            a.snap_percentage for a in analytics_data if a.snap_percentage
        ) / len(analytics_data)
        avg_target_share = (
            sum(a.target_share for a in analytics_data if a.target_share)
            / len(analytics_data)
        ...
        consistency_score = (
            100 - (max(recent_points) - min(recent_points)) * 2
        ...
        return {
            ...
            "analytics_data": {
                ...
                "target_share": (
                    round(avg_target_share, 3) if avg_target_share else None
                ),
    ...
    def _generate_enhanced_baseline_projection(self, player: Dict, week: int) -> Dict:
        ...
        baseline = baseline_projections.get(
            position, {"points": 8, "floor": 0, "ceiling": 15, "variance": 0.3}
        )

        # Add realistic variance instead of random
        variance_factor = baseline["variance"]
        projected_points = baseline["points"] * (
            1 + ((week % 3 - 1) * variance_factor * 0.1)
        )  # Week-based variance
        projected_points = max(
            baseline["floor"], min(baseline["ceiling"], projected_points)
        )
    ...
    def _create_position_recommendations(
        self,
        league_id: int,
        league_name: str,
        position: str,
        ...
        for i, player in enumerate(sorted_players):
            ...
            if is_starter:
                ...
                if projected_points < 5:
                    reasoning = (
                        f"Low projection ({projected_points} pts), consider benching"
                    )
                else:
    ...
    def disconnect_fantasy_account(
        self, user_id: int, fantasy_user_id: int
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            self.db.rollback()
            logger.error(
                f"Failed to disconnect fantasy account {fantasy_user_id}: {str(e)}"
            )
            return {"success": False, "error": str(e)}
    ...
    def disconnect_league(self, user_id: int, league_id: int) -> Dict[str, Any]:
        ...
        try:
            ...
            league = (
                ...
                .join(FantasyUser)
                .filter(
                    and_(FantasyLeague.id == league_id, FantasyUser.user_id == user_id)
                )
                .first()
            )
            ...
            if not league:
                return {
                    "success": False,
                    "error": "League not found or doesn't belong to user",
                }
            ...
            self.db.query(FantasyRecommendation).filter(
                FantasyRecommendation.league_id == league_id
            ).delete()

            # 2. Waiver wire targets
            self.db.query(WaiverWireTarget).filter(
                WaiverWireTarget.league_id == league_id
            ).delete()

            # 3. Fantasy matchups
            self.db.query(FantasyMatchup).filter(
                FantasyMatchup.league_id == league_id
            ).delete()

            # 4. Fantasy transactions
            self.db.query(FantasyTransaction).filter(
                FantasyTransaction.league_id == league_id
            ).delete()

            # 5. League historical data
            self.db.query(LeagueHistoricalData).filter(
                LeagueHistoricalData.league_id == league_id
            ).delete()

            # 6. Competitor analysis
            self.db.query(CompetitorAnalysis).filter(
            ...
            for team in teams:
                ...
                self.db.query(FantasyRosterSpot).filter(
                    FantasyRosterSpot.team_id == team.id
                ).delete()
            # Delete teams
            self.db.query(FantasyTeam).filter(
            ...
            logger.info(
                f"Successfully disconnected and removed league '{league_name}' (ID: {league_id}) for user {user_id}"
            )
            return {
                "success": True,
    ...
    def get_league_standings(self, league_id: int) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            for team in teams:
                ...
                points_per_game = (
                    team.points_for / total_games if total_games > 0 else 0
                )
                points_against_per_game = (
                    team.points_against / total_games if total_games > 0 else 0
                )
                ...
                team_data = {
                    ...
                    "points_against": (
                        float(team.points_against) if team.points_against else 0
                    ),
                    "points_per_game": round(points_per_game, 2),
                    "points_against_per_game": round(points_against_per_game, 2),
                    "point_differential": (
                        float(team.points_for - team.points_against)
                        if team.points_for and team.points_against
                        else 0
                    ),
                    "waiver_position": team.waiver_position,
                }
            ...
            logger.error(
                f"Failed to get league standings for league {league_id}: {str(e)}"
            )
            return []

    async def get_league_matchups(
        self, league_id: int, week: int
    ) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            matchups_data = await platform_interface.get_league_matchups(
            ...
            for matchup in matchups_data:
                ...
                if team1 and team2:
                    processed_matchup = {
                        ...
                        "status": self._determine_matchup_status(
            ...
            processed_matchups.sort(
                key=lambda x: (not x["user_involved"], x["matchup_id"])
            )
            ...
            logger.error(
                f"Failed to get league matchups for league {league_id}, week {week}: {str(e)}"
            )
            return []
            ...
            try:
                ...
                for position, players in position_groups.items():
                    ...
                    position_recommendations = (
                        self._create_enhanced_position_recommendations(
                            league_id,
                            league_data["name"],
                            position,
                            sorted_players,
                ...
                logger.error(
                    f"Failed to generate start/sit for league {league_id}: {str(e)}"
                )
                continue
    ...
    async def _detect_superflex_league(self, league_id: int) -> Dict[str, Any]:
        ...
        try:
            ...
            roster_positions = (
                self.db.query(FantasyRosterSpot.position)
                .filter(
                    FantasyRosterSpot.team_id.in_(
                        self.db.query(FantasyTeam.id).filter(
                            FantasyTeam.league_id == league_id
                        )
                    )
                )
            ...
            superflex_slots = positions_list.count("SUPER_FLEX") + positions_list.count(
                "SUPERFLEX"
            )
            ...
            return {
                ...
                "league_type": (
                    "superflex" if has_superflex else ("2qb" if is_2qb else "standard")
                ),
            }

        except Exception as e:
            logger.error(
                f"Failed to detect superflex configuration for league {league_id}: {str(e)}"
            )
    ...
    async def _get_league_rules_context(
        ...
        return {
            ...
            "ai_context": {
                ...
                "qb_premium": superflex_info["has_superflex"]
                or superflex_info["qb_slots"] >= 2,  # QB premium in superflex/2QB
                "superflex_multiplier": superflex_info[
    ...
    def _get_enhanced_player_projection(
        self, player: Dict, week: int, league_rules: Dict
    ) -> Dict[str, Any]:
        ...
        baseline = base_projections.get(
            position, {"points": 8, "floor": 0, "ceiling": 15}
        )
    ...
    def _create_enhanced_position_recommendations(
        self,
        league_id: int,
        league_name: str,
        position: str,
    ...
    async def generate_waiver_wire_recommendations(
        self, user_id: int, week: int
    ) -> List[Dict[str, Any]]:
        ...
        for league_data in user_leagues:
            ...
            try:
                ...
                available_players = await platform_interface.get_available_players(
                ...
                logger.error(
                    f"Failed to generate waiver recommendations for league {league_id}: {str(e)}"
                )
                continue
    ...
    async def _analyze_enhanced_roster_needs(
        self, league_id: int, team_id: int, league_rules: Dict[str, Any]
    ) -> Dict[str, int]:
        """Enhanced roster needs analysis with league context"""
        try:
    ...
    def _generate_enhanced_league_recommendations(
        self,
        league_id: int,
        league_name: str,
        trending_adds: List[Dict],
        ...
        for trending_player in trending_adds[:15]:  # Top 15 trending adds
            ...
            if priority_score > 0:
                ...
                waiver_suggestion = self._generate_waiver_suggestion(
                    priority_score, league_rules, player
                )
                ...
                recommendation = {
                    ...
                    "league_context": {
                        "scoring_type": league_rules["scoring_type"],
                        "ppr_value": league_rules["ppr_value"],
                        "position_value": self._get_position_value_in_league(
                            position, league_rules
                        ),
                    },
                    "age": player.get("age"),
    ...
    def _calculate_enhanced_priority_score(
        self,
        trending_player: Dict,
        player: Dict,
        roster_needs: Dict[str, int],
        league_rules: Dict[str, Any],
    ) -> float:
    ...
    def _generate_waiver_suggestion(
        self,
        ...
        is_faab_league = (
            league
            and league.waiver_type == "FAAB"
            and league.waiver_budget
            and league.waiver_budget > 0
        )

        if is_faab_league:
            return self._generate_faab_suggestion(
                priority_score, league_rules, league.waiver_budget
            )
        else:
            return self._generate_waiver_priority_suggestion(
                priority_score, league_rules, player
            )

    def _generate_faab_suggestion(
        self, priority_score: float, league_rules: Dict[str, Any], waiver_budget: int
    ) -> Dict[str, Any]:
        ...
        if waiver_budget:
            ...
            max_amount = min(
                waiver_budget, int(waiver_budget * (base_percentage + 3) / 100)
            )
    ...
    def _generate_waiver_priority_suggestion(
        self,
        priority_score: float,
        league_rules: Dict[str, Any],
        player: Dict[str, Any],
    ...
    def _generate_enhanced_recommendation_reason(
        self,
        player: Dict,
        roster_needs: Dict[str, int],
        trending_player: Dict,
    ...
    def _get_position_value_in_league(
        self, position: str, league_rules: Dict[str, Any]
    ) -> str:
    ...
    def _analyze_league_competition(
        self, league_id: int, position: str
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            for analysis in competitor_analyses:
                if (
                    analysis.waiver_aggressiveness_score
                    and analysis.waiver_aggressiveness_score > 30
                ):
                    active_managers += 1
            ...
            avg_position_interest = (
                sum(position_demand_scores) / len(position_demand_scores)
                if position_demand_scores
                else 0
            )
            active_manager_ratio = (
                active_managers / total_managers if total_managers > 0 else 0
            )
            ...
            logger.error(
                f"Failed to analyze league competition for league {league_id}: {str(e)}"
            )
            return {
                "threat_level": "unknown",
                "position_insights": {"high_demand": False},
    ...
    async def sync_historical_league_data(
        self, league_id: int, seasons: List[int]
    ) -> bool:
        ...
        try:
            ...
            for season in seasons:
                logger.info(
                    f"Syncing historical data for league {league_id}, season {season}"
                )
                ...
                await self._analyze_and_store_competitor_data(
                    league_id, season, historical_data
                )

            logger.info(
                f"Successfully synced historical data for {len(seasons)} seasons"
            )
            return True

        except Exception as e:
            logger.error(
                f"Failed to sync historical data for league {league_id}: {str(e)}"
            )
            return False

    async def _store_historical_data(
        self, league_id: int, season: int, historical_data: Dict[str, Any]
    ):
        """Store historical league data in database"""
        try:
            # Check if data already exists
            existing = (
                self.db.query(LeagueHistoricalData)
                .filter(
                    and_(
                        LeagueHistoricalData.league_id == league_id,
            ...
            if existing:
                ...
                existing.transactions_data = historical_data.get(
                    "transactions_data", []
                )
                existing.standings_data = historical_data.get("standings_data", [])
                existing.waiver_type = historical_data.get("waiver_settings", {}).get(
                    "waiver_type"
                )
                existing.waiver_budget = historical_data.get("waiver_settings", {}).get(
                    "waiver_budget"
                )
                ...
                historical_record = LeagueHistoricalData(
                    ...
                    waiver_type=historical_data.get("waiver_settings", {}).get(
                        "waiver_type"
                    ),
                    waiver_budget=historical_data.get("waiver_settings", {}).get(
                        "waiver_budget"
                    ),
                    scoring_type=self._determine_scoring_from_settings(
                        league_info.get("scoring_settings", {})
                    ),
            ...
            self.db.commit()
            logger.info(
                f"Stored historical data for league {league_id}, season {season}"
            )
    ...
    async def _analyze_and_store_competitor_data(
        self, league_id: int, season: int, historical_data: Dict[str, Any]
    ):
        ...
        try:
            ...
            for team in teams_data:
                ...
                analysis = self._analyze_team_patterns(
                    team_transactions_list, waiver_settings
                )
            ...
            self.db.commit()
            logger.info(
                f"Analyzed competitor data for league {league_id}, season {season}"
            )
    ...
    def _analyze_team_patterns(
        self, transactions: List[Dict], waiver_settings: Dict
    ) -> Dict[str, Any]:
        """Analyze individual team's transaction patterns"""
        waiver_adds = [
            t for t in transactions if t.get("type") == "waiver" and t.get("adds")
        ]
        free_agent_adds = [
            t for t in transactions if t.get("type") == "free_agent" and t.get("adds")
        ]
        ...
        if waiver_settings.get("waiver_type") == "FAAB":
            ...
            if faab_bids:
                faab_data = {
                    ...
                    "faab_conservation_tendency": self._determine_faab_tendency(
                        faab_bids, waiver_settings.get("waiver_budget", 100)
                    ),
                }
    ...
    def _merge_competitor_analysis(
        self, existing: CompetitorAnalysis, new_analysis: Dict[str, Any]
    ):
        ...
        if existing.avg_waiver_adds_per_season:
            existing.avg_waiver_adds_per_season = (
                existing.avg_waiver_adds_per_season * (seasons_count - 1)
                + new_analysis["avg_waiver_adds_per_season"]
            ) / seasons_count
        else:
            existing.avg_waiver_adds_per_season = new_analysis[
                "avg_waiver_adds_per_season"
            ]
    ...
    async def get_team_analytics(
        self, user_id: int, league_id: str, season: int = 2025
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            league = (
                self.db.query(FantasyLeague)
                .filter(FantasyLeague.sleeper_league_id == league_id)
                .first()
            )
            ...
            user_team = (
                self.db.query(FantasyTeam)
                .filter(
                    FantasyTeam.league_id == league.id, FantasyTeam.user_id == user_id
                )
                .first()
            )
    ...
    def _calculate_league_insights(
        self, league_analytics: List[Dict]
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            for team in league_analytics:
                ...
                # Consistency scores
                consistency = team.get("team_consistency", {}).get(
                    "overall_consistency", 0
                )
                consistency_scores.append(consistency)

                # Efficiency grades
                efficiency = team.get("efficiency_benchmarks", {}).get(
                    "overall_efficiency_grade", "F"
                )
                efficiency_grades.append(efficiency)

            # Calculate league averages and distributions
            league_insights = {
                "league_strength_distribution": self._analyze_grade_distribution(
                    overall_grades
                ),
                "position_competitiveness": self._analyze_position_competitiveness(
                    position_grades
                ),
                "league_consistency_average": (
                    sum(consistency_scores) / len(consistency_scores)
                    if consistency_scores
                    else 0
                ),
                "most_competitive_positions": self._identify_competitive_positions(
                    position_grades
                ),
                "league_efficiency_trends": self._analyze_efficiency_trends(
                    efficiency_grades
                ),
                "championship_contenders": self._identify_championship_contenders(
                    league_analytics
                ),
    ...
    def _analyze_competitive_position(
        self, user_team_analytics: Dict, league_analytics: List[Dict]
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            user_consistency = user_team_analytics.get("team_consistency", {}).get(
                "overall_consistency", 0
            )

            # Compare to league
            league_grades = [
                grade_values.get(team.get("overall_team_grade", "F"), 50)
                for team in league_analytics
            ]
            league_consistencies = [
                team.get("team_consistency", {}).get("overall_consistency", 0)
                for team in league_analytics
            ]
            ...
            for position, analytics in user_positions.items():
                user_pos_grade = grade_values.get(
                    analytics.get("position_grade", "F"), 50
                )
                ...
                for team in league_analytics:
                    ...
                    league_pos_grades.append(
                        grade_values.get(team_pos.get("position_grade", "F"), 50)
                    )
                ...
                if league_pos_grades:
                    ...
                    if user_pos_grade > league_pos_avg + 10:
                        relative_strengths.append(
                            f"{position} depth above league average"
                        )
                    elif user_pos_grade < league_pos_avg - 10:
                        relative_weaknesses.append(
                            f"{position} depth below league average"
                        )
            ...
            return {
                ...
                "percentile": round(
                    (len(league_analytics) - league_rank + 1)
                    / len(league_analytics)
                    * 100,
                    1,
                ),
                "competitive_tier": self._determine_competitive_tier(
                    league_rank, len(league_analytics)
                ),
                "relative_strengths": relative_strengths,
                "relative_weaknesses": relative_weaknesses,
                "championship_odds": self._estimate_championship_odds_from_rank(
                    league_rank, len(league_analytics)
                ),
                "playoff_probability": self._estimate_playoff_probability(
                    league_rank, len(league_analytics)
                ),
            }
    ...
    def _generate_team_recommendations(
        ...
        try:
            ...
            weaknesses = user_team_analytics.get("strengths_weaknesses", {}).get(
                "key_weaknesses", []
            )
            ...
            consistency = user_team_analytics.get("team_consistency", {}).get(
                "overall_consistency", 0
            )
            if consistency < 60:
                recommendations.append(
    ...
    def _analyze_grade_distribution(self, grades: List[str]) -> Dict[str, Any]:
        ...
        return {
            ...
            "most_common_grade": (
                max(grade_counts.items(), key=lambda x: x[1])[0]
                if grade_counts
                else "F"
            ),
            "league_strength": (
                "High"
                if sum(1 for g in grades if g in ["A+", "A", "A-"]) > len(grades) * 0.4
                else "Average"
            ),
        }

    def _analyze_position_competitiveness(
        self, position_grades: Dict[str, List[str]]
    ) -> Dict[str, str]:
    ...
    def _identify_competitive_positions(
        self, position_grades: Dict[str, List[str]]
    ) -> List[str]:
    ...
    def _analyze_efficiency_trends(
        ...
        high_efficiency_count = sum(
            1 for grade in efficiency_grades if grade in ["A", "B"]
        )

        return {
            "league_efficiency_level": (
                "High"
                if high_efficiency_count > len(efficiency_grades) * 0.5
                else "Moderate"
            ),
            "efficient_teams_count": high_efficiency_count,
            "efficiency_distribution": {
                grade: efficiency_grades.count(grade)
                for grade in set(efficiency_grades)
            },
        }

    def _identify_championship_contenders(
        self, league_analytics: List[Dict]
    ) -> List[Dict[str, str]]:
    ...
    def _identify_rebuilding_teams(
        self, league_analytics: List[Dict]
    ) -> List[Dict[str, str]]:
    ...
    def _estimate_championship_odds_from_rank(
        self, rank: int, total_teams: int
    ) -> float:
