...
def fetch_team_roster_il(team_id):
    ...
    il_players = []
    try:
        data = mlbstatsapi.get(
            "team_roster", {"teamId": team_id, "rosterType": "40Man"}
        )
    ...
    try:
        ...
        for txn in data.get("transactions", []):
            ...
            if txn_type not in ("DL", "ASG", "SC", "REL", "DFA", "FA"):
                ...
                if (
                    "injured list" not in description.lower()
                    and "disabled list" not in description.lower()
                ):
                    continue
...
if __name__ == "__main__":
    ...
    for team_id, data in sorted(impacts.items(), key=lambda x: -x[1]["impact"]):
        if data["il_count"] > 0:
            print(
                f"{data['team_name']}: impact={data['impact']:.3f}, "
