...
def compute_power_profile(statcast_csv: str, slugging_csv: str) -> pd.DataFrame:
    ...
    df["PowerScore"] = 0.4 * df["Z_AEV"] + 0.3 * df["Z_HardHitRate"] + 0.3 * df["Z_SLG"]
...
def main():
    p = argparse.ArgumentParser(
        description="Compute multi-year PowerScore (S3/local support)"
    )
    ...
    p.add_argument(
        "--statcast_dir",
        required=True,
        help="directory with statcast_bb_{year}.csv files (local or s3)",
    )
    p.add_argument(
        "--slugging_dir",
        required=True,
        help="directory with slugging_{year}.csv files (local or s3)",
    )
    p.add_argument(
        "--output",
        required=True,
        help="where to write the combined power_scores_all.csv (local or s3)",
    )
    args = p.parse_args()
    ...
    for year in range(args.start, args.end + 1):
        sc = (
            os.path.join(args.statcast_dir, f"statcast_bb_{year}.csv")
            if not args.statcast_dir.startswith("s3://")
            else f"{args.statcast_dir.rstrip('/')}/statcast_bb_{year}.csv"
        )
        sl = (
            os.path.join(args.slugging_dir, f"slugging_{year}.csv")
            if not args.slugging_dir.startswith("s3://")
            else f"{args.slugging_dir.rstrip('/')}/slugging_{year}.csv"
        )
