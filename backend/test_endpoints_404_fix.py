...
class EndpointTester:
    ...
    def test_endpoint(
        ...
        try:
            ...
            if not endpoint_exists:
                self.log_result(
                    endpoint, method, response.status_code, False, "404 Not Found"
                )
                return False
            ...
            self.log_result(
                endpoint, method, 0, False, "Connection Error - Is server running?"
            )
            return False
    ...
    def test_sports_betting_endpoints(self):
        ...
        self.test_endpoint(
            "POST", "/api/bets/history", {"status": "pending", "limit": 10}
        )
        ...
        self.test_endpoint(
            "POST",
            "/api/bets/share",
            {"bet_id": "test-bet-id", "message": "Great bet!"},
        )
    ...
    def test_fantasy_endpoints(self):
        ...
        self.test_endpoint(
            "POST",
            "/api/fantasy/connect",
            {"platform": "sleeper", "credentials": {"username": "test_user"}},
        )
    ...
    def save_results(self):
        ...
        filename = (
            f"endpoint_test_results_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
        )
