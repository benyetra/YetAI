...
class FantasyAnalyticsService:
    ...
    def get_player_trend_analysis(self, player_id: int, weeks_back: int = 4) -> Dict[str, Any]:
        ...
        try:
            ...
            recent_games = self.db.query(PlayerAnalytics).filter(
                ...
                PlayerAnalytics.season.desc(),
                PlayerAnalytics.week.desc()
            ).limit(weeks_back).all()
            ...
            snaps = [g.snap_percentage for g in recent_games if g.snap_percentage is not None]
            ...
            return {
                ...
                "trends": {
                    "points": {
                        "current_avg": round(np.mean(points[:2]), 2) if len(points) >= 2 else 0,
                        "previous_avg": round(np.mean(points[2:]), 2) if len(points) > 2 else 0,
                        "trend": points_trend,
                        "raw_data": points
                    },
                    "targets": {
                        "current_avg": round(np.mean(targets[:2]), 2) if len(targets) >= 2 else 0,
                        "previous_avg": round(np.mean(targets[2:]), 2) if len(targets) > 2 else 0,
                        "trend": targets_trend,
                        "raw_data": targets
                    },
                    "snap_share": {
                        "current_avg": round(np.mean(snaps[:2]), 2) if len(snaps) >= 2 else 0,
                        "previous_avg": round(np.mean(snaps[2:]), 2) if len(snaps) > 2 else 0,
                        "trend": snap_trend,
                        "raw_data": snaps
                    }
                },
    ...
    def get_matchup_analysis(self, player_id: int, opponent: str, week: int) -> Dict[str, Any]:
        """Analyze player matchup using historical data"""
        try:
            # Get player's historical performance
            player_history = self.db.query(PlayerAnalytics).filter(
                PlayerAnalytics.player_id == player_id
            ).order_by(
                PlayerAnalytics.season.desc(),
                PlayerAnalytics.week.desc()
            ...
            vs_opponent = self.db.query(PlayerAnalytics).filter(
                and_(
                    PlayerAnalytics.player_id == player_id,
            ...
            avg_points = np.mean([g.ppr_points for g in player_history if g.ppr_points]) if player_history else 0
            ...
            return {
                ...
                "recent_form": {
                    "last_3_avg": round(np.mean([g.ppr_points for g in player_history[:3] if g.ppr_points]), 2),
                    "trend": self._calculate_trend([g.ppr_points for g in player_history if g.ppr_points])
                }
            }
    ...
    def get_breakout_candidates(self, position: Optional[str] = None, limit: int = 10) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            for result in query.all():
                ...
                recent = self.db.query(PlayerAnalytics).filter(
                    and_(
                        PlayerAnalytics.player_id == player_id,
                        # Use data from recent seasons, not just current
                PlayerAnalytics.season >= (self.current_season - 1),
                ...
                older = self.db.query(PlayerAnalytics).filter(
                    and_(
                        PlayerAnalytics.player_id == player_id,
                        # Use data from recent seasons, not just current
                PlayerAnalytics.season >= (self.current_season - 1),
                ...
                if len(recent) >= 2 and len(older) >= 3:
                    recent_snaps = np.mean([g.snap_percentage for g in recent if g.snap_percentage])
                    older_snaps = np.mean([g.snap_percentage for g in older if g.snap_percentage])
                    
                    recent_targets = np.mean([g.target_share for g in recent if g.target_share])
                    older_targets = np.mean([g.target_share for g in older if g.target_share])
                    ...
                    if snap_increase > 10 or target_increase > 0.05:
                        ...
                        if player and (not position or player.position == position):
                            breakout_score = (snap_increase * 0.3) + (target_increase * 100 * 0.7)
                            ...
                            candidates.append({
                                ...
                                "target_share_increase": round(target_increase * 100, 1),
                                "recent_avg_points": round(np.mean([g.ppr_points for g in recent if g.ppr_points]), 2),
                                "reasons": self._generate_breakout_reasons(snap_increase, target_increase)
                            })
    ...
    def _generate_breakout_reasons(self, snap_increase: float, target_increase: float) -> List[str]:
        ...
        if target_increase > 0.1:
            reasons.append(f"Significant target share increase (+{target_increase*100:.1f}%)")
    ...
    def get_regression_candidates(self, limit: int = 10) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            query = self.db.query(
                ...
                func.avg(PlayerAnalytics.ppr_points) > 10  # Focus on relevant players
            )
            ...
            for result in query.all():
                ...
                PlayerAnalytics.season >= (self.current_season - 1)
                    ...
                    candidates.append({
                        ...
                        "reasons": self._generate_regression_reasons(avg_efficiency, threshold, td_dependent)
                    })
    ...
    def _generate_regression_reasons(self, efficiency: float, expected: float, td_dependent: bool) -> List[str]:
        """Generate regression risk reasons"""
        reasons = []
        
        efficiency_diff = ((efficiency / expected) - 1) * 100
        if efficiency_diff > 50:
            reasons.append(f"Unsustainably high efficiency ({efficiency_diff:.0f}% above normal)")
        elif efficiency_diff > 30:
            reasons.append(f"Above normal efficiency ({efficiency_diff:.0f}% above expected)")
    ...
    def get_consistency_rankings(self, position: str, scoring_type: str = "ppr") -> List[Dict[str, Any]]:
        ...
        try:
            ...
            players = self.db.query(
                ...
                func.count(PlayerAnalytics.id) >= 8
            ).all()
            ...
            for player_id, games, avg_points, std_dev in players:
                if avg_points and avg_points > 5:  # Minimum relevance threshold
                    ...
                    analytics = self.db.query(PlayerAnalytics).filter(
                        PlayerAnalytics.player_id == player_id
                    ).order_by(PlayerAnalytics.season.desc(), PlayerAnalytics.week.desc()).limit(16).all()
                    
                    boom_rate = 0
                    bust_rate = 0
                    if analytics:
                        recent_boom = analytics[0].boom_rate if analytics[0].boom_rate else 0
                        recent_bust = analytics[0].bust_rate if analytics[0].bust_rate else 0
    ...
    def get_advanced_player_projection(self, player_id: int, week: int, league_settings: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            historical = self.db.query(PlayerAnalytics).filter(
                PlayerAnalytics.player_id == player_id
            ).order_by(
                PlayerAnalytics.season.desc(),
                PlayerAnalytics.week.desc()
            ...
            if scoring_type == "standard":
                points_col = [g.standard_points for g in recent_games if g.standard_points]
            elif scoring_type == "half_ppr":
                points_col = [g.half_ppr_points for g in recent_games if g.half_ppr_points]
            else:
            ...
            consistency = self._calculate_consistency([g.ppr_points for g in historical if g.ppr_points])
    ...
    def get_waiver_wire_analytics(self, league_id: int, position: Optional[str] = None) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            trending_query = self.db.query(
                ...
                func.avg(PlayerAnalytics.snap_percentage) > 40
            )
            ...
            for result in trending_query.all():
                ...
                if ownership_pct < 50:  # Available in most leagues
                    ...
                    opportunity_score = self._calculate_opportunity_score(player_id, avg_snaps)
                ...
                PlayerAnalytics.season >= (self.current_season - 1)
                    ...
                    recent_avg = np.mean([g.ppr_points for g in recent_games if g.ppr_points])
                    ...
                    recommendations.append({
                        ...
                        "priority": self._determine_waiver_priority(opportunity_score, ownership_pct),
                        "recommendation": self._generate_waiver_recommendation(opportunity_score, player.position)
                    })
    ...
    def _calculate_opportunity_score(self, player_id: int, avg_snaps: float) -> float:
        ...
        recent_targets = self.db.query(
            func.avg(PlayerAnalytics.target_share)
        ).filter(
            and_(
                PlayerAnalytics.player_id == player_id,
                # Use data from recent seasons, not just current
        ...
        # Red zone usage
        rz_usage = self.db.query(
            func.avg(PlayerAnalytics.red_zone_touches)
        ).filter(
            and_(
                PlayerAnalytics.player_id == player_id,
                # Use data from recent seasons, not just current
    ...
    def _determine_waiver_priority(self, opportunity_score: float, ownership: float) -> str:
    ...
    def _generate_waiver_recommendation(self, opportunity_score: float, position: str) -> str:
    ...
    def get_trade_value_analysis(self, player_ids: List[int], league_settings: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "recommendation": self._get_trade_recommendation(fairness_pct, total_value_team1, total_value_team2)
            }
    ...
    def _calculate_player_trade_value(self, player_id: int, league_settings: Dict[str, Any]) -> Dict[str, Any]:
        ...
        position = self.db.query(FantasyPlayer.position).filter(
    ...
    def _get_trade_recommendation(self, fairness: float, value1: float, value2: float) -> str:
