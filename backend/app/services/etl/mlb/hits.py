...
def filter_hitters(hitters, min_combined_score=3):
    filtered_hitters = [
        ...
        if hitter['combined_score'] is not None and hitter['combined_score'] >= min_combined_score
    ]
    print(f"Filtered {len(hitters)} to {len(filtered_hitters)} hitters with a combined score of {min_combined_score} or more.")
