...
required_fields = {'game_id', 'game_date', 'home_name', 'away_name', 'home_id', 'away_id',
...
check("REQ-BT-005a: sample_full_season()", hasattr(BacktestSampler, 'sample_full_season'))
check("REQ-BT-005b: sample_random_dates()", hasattr(BacktestSampler, 'sample_random_dates'))
...
check("REQ-BT-006: pitcher stats reconstruction", '_reconstruct_pitcher_stats' in hdb_src)

# REQ-BT-007: game log filtering by date
check("REQ-BT-007: date filtering in pitcher stats",
...
check("REQ-BT-009b: team pitching reconstruction", '_reconstruct_team_pitching' in hdb_src)

# REQ-BT-010: early-season blending
check("REQ-BT-010: early-season blending formula",
...
check("REQ-BT-012: batter stats in lineup", 'ops' in hdb_src.lower() or 'OPS' in hdb_src)

# REQ-BT-013: bullpen fatigue
check("REQ-BT-013: bullpen fatigue reconstruction", '_reconstruct_bullpen_fatigue' in hdb_src)

# REQ-BT-014: domed stadium weather
check("REQ-BT-014: domed stadium detection",
...
check("REQ-BT-017: park factor lookup",
      'park_factor' in hdb_src or 'VENUE_PARK_FACTORS' in hdb_src)

# REQ-BT-018: historical odds
check("REQ-BT-018: historical odds fetch", '_fetch_historical_odds' in hdb_src or 'historical' in hdb_src.lower())

# REQ-BT-019: odds unavailable handling
check("REQ-BT-019: odds unavailable flag", 'had_historical_odds' in hdb_src or 'market_odds_available' in hdb_src)

# REQ-BT-020: TTOP
check("REQ-BT-020: TTOP reconstruction", 'ttop' in hdb_src.lower())

# REQ-BT-021: umpire effects
check("REQ-BT-021: umpire reconstruction", '_reconstruct_umpire' in hdb_src or 'umpire' in hdb_src.lower())

# REQ-BT-022: travel fatigue
check("REQ-BT-022: travel fatigue reconstruction", '_reconstruct_travel' in hdb_src or 'travel' in hdb_src.lower())

# REQ-BT-023: pitcher quality
check("REQ-BT-023: pitcher quality score", 'pitcher_quality' in hdb_src.lower() or '_compute_pitcher_quality' in hdb_src)
...
check("REQ-BT-026: ensemble model support", 'ensemble' in mr_src.lower() or 'predict_proba' in mr_src)

# REQ-BT-027: K projections
check("REQ-BT-027: predict_strikeouts()", hasattr(BacktestModelRunner, 'predict_strikeouts'))
...
check("REQ-BT-029: predict_home_runs()", hasattr(BacktestModelRunner, 'predict_home_runs'))
...
check("REQ-BT-031: fetch_game_actuals()", hasattr(BacktestActualsFetcher, 'fetch_game_actuals'))
check("REQ-BT-031b: scores extraction", 'home_score' in af_src and 'away_score' in af_src)
...
check("REQ-BT-044: offset effectiveness", 'offset' in sc_src.lower() or 'n_starts' in sc_src)

# REQ-BT-045: hit accuracy
check("REQ-BT-045: hit prediction metrics", 'hit_mae' in sc_src or 'hit_accuracy' in sc_src)
...
check("REQ-BT-049: model vs market Brier", 'model_vs_market' in sc_src or 'market_brier' in sc_src)

# REQ-BT-050: segmented analysis (month, temp, quality)
check("REQ-BT-050a: by month", 'by_month' in sc_src)
check("REQ-BT-050b: by temperature", 'by_temperature' in sc_src or 'temperature' in sc_src)
...
for col in ['id', 'run_date', 'model_version', 'date_range_start', 'date_range_end',
            'n_games', 'seed', 'brier_score', 'ml_accuracy', 'ou_accuracy',
            'total_mae', 'k_mae', 'k_gold_standard_rate', 'hr_top10_hit_rate',
...
for col in ['backtest_id', 'game_id', 'game_date', 'home_team', 'away_team',
            'predicted_home_wp', 'actual_winner', 'predicted_total', 'actual_total',
            'ml_correct', 'total_correct', 'data_quality_score']:
    check(f"REQ-BT-053: BacktestGameResult.{col}", col in bgr_cols, f"missing column {col}")

# REQ-BT-054: report generation
from app.services.etl.mlb.backtest.report import generate_summary_report, write_csv_report
...
for flag in ['start_date', 'end_date', 'n_games', 'seed', 'full_season',
             'random_dates', 'n_dates', 'team', 'month', 'models',
             'model_version', 'compare', 'include_postseason', 'output_dir',
             'verbose', 'skip_odds', 'skip_weather']:
    check(f"REQ-BT-056: --{flag.replace('_', '-')}", hasattr(args, flag), f"missing arg: {flag}")
...
check("REQ-BT-058b: --full sets include_postseason", full_args.include_postseason is True)
...
check("REQ-BT-059: data quality tracking", 'data_quality_score' in hdb_src or 'quality_score' in hdb_src)
...
check("REQ-BT-061: leakage audit", 'leakage_audit' in cli_src.lower() or 'TEMPORAL LEAKAGE' in cli_src)

# ---------------------------------------------------------------------------
# §5.9 Performance (REQ-BT-062 through REQ-BT-064)
# ---------------------------------------------------------------------------
print("\n=== §5.9 PERFORMANCE ===")
from app.services.etl.mlb.backtest.cache import cached_api_call, clear_cache, get_cache_stats
...
check("REQ-BT-063a: SQLite cache", 'sqlite3' in cache_src or 'sqlite' in cache_src.lower())
...
mock_metrics = {
    'game_metrics': {
        ...
        'rl_correct': 26,
        'calibration': [
            {'bucket': '0.45-0.55', 'predicted': 0.50, 'actual': 0.49, 'count': 20, 'status': 'OK'},
            {'bucket': '0.55-0.65', 'predicted': 0.59, 'actual': 0.61, 'count': 15, 'status': 'OK'},
        ],
    },
    'k_metrics': {
        'n_pitchers': 30,
...
comp = generate_comparison_report(
    mock_config, mock_metrics, mock_metrics, 'v1', 'v2'
)
...
# K results
for i in range(10):
    scorer.add_k_result('home', 5.5 + i * 0.2, 5 + (i % 3),
                        projected_ip=5.5, actual_ip=5.0 + i * 0.2,
                        pitcher_k9=8.5, n_starts=10)
...
check("Scorer: k_gold_standard_rate computed", km.get('k_gold_standard_rate') is not None)
