...
class BetSchedulerService:
    ...
    def start(self) -> None:
        ...
        self._running = True
        logger.info(f"Started bet verification scheduler (interval: {self.config.interval_minutes} minutes)")
    ...
    async def _run_verification(self) -> Dict:
        ...
        while retries <= self.config.max_retries:
            try:
                logger.info(f"Starting bet verification run {self.stats.total_runs} (attempt {retries + 1})")
                ...
                if result.get("success", False):
                    ...
                    logger.info(f"Verification run completed successfully: {result.get('message', 'No message')}")
                    
                    # Log detailed results
                    if result.get("verified", 0) > 0:
                        logger.info(f"Verification stats: {result.get('verified')} verified, {result.get('settled')} settled, {result.get('games_checked')} games checked")
                    ...
                    if retries <= self.config.max_retries:
                        wait_time = self.config.retry_interval_minutes * 60 * retries  # Exponential backoff
        ...
        logger.error(f"Verification run failed after {self.config.max_retries} retries. Last error: {last_error}")
    ...
    def _get_next_run_estimate(self) -> Optional[str]:
        ...
        if self._is_quiet_hours():
            ...
            if self.config.quiet_hours_start <= self.config.quiet_hours_end:
                # Normal range
                next_run = now.replace(hour=self.config.quiet_hours_end, minute=0, second=0, microsecond=0)
                if next_run <= now:
            ...
            if self.stats.last_run_time:
                next_run = self.stats.last_run_time + timedelta(minutes=self.config.interval_minutes)
            else:
