...
def reset_fantasy_tables():
    ...
    try:
        ...
        db.execute(text("CREATE INDEX idx_players_platform_id ON players(platform_player_id)"))
        db.execute(text("CREATE INDEX idx_players_position ON players(position)"))
        db.execute(text("CREATE INDEX idx_roster_spots_team_id ON roster_spots(team_id)"))
        print("  ✓ Created indexes")
        
        # Add sleeper_user_id to users table if it doesn't exist
        try:
            db.execute(text("ALTER TABLE users ADD COLUMN sleeper_user_id VARCHAR(255)"))
            db.execute(text("ALTER TABLE users ADD COLUMN sleeper_username VARCHAR(255)"))
...
def populate_simplified_data(user_id: int):
    ...
    try:
        ...
        league_result = db.execute(text("SELECT id FROM leagues WHERE user_id = :user_id AND season = 2025"), 
                                  {"user_id": user_id})
        ...
        team_result = db.execute(text("SELECT id FROM teams WHERE league_id = :league_id AND is_user_team = TRUE"), 
                                {"league_id": league_id})
        ...
        current_roster = [
            {"name": "Jayden Daniels", "platform_id": "11566", "position": "QB", "team": "WAS", "is_starter": True},
            {"name": "Bijan Robinson", "platform_id": "11651", "position": "RB", "team": "ATL", "is_starter": True},
            {"name": "Isaac Guerendo", "platform_id": "2216", "position": "RB", "team": "SF", "is_starter": True},
            {"name": "Deebo Samuel", "platform_id": "3163", "position": "WR", "team": "WAS", "is_starter": True},
            {"name": "Mike Evans", "platform_id": "4018", "position": "WR", "team": "TB", "is_starter": True},
            {"name": "Evan Engram", "platform_id": "4066", "position": "TE", "team": "DEN", "is_starter": True},
            {"name": "Joe Mixon", "platform_id": "5872", "position": "RB", "team": "HOU", "is_starter": True},
            {"name": "Minnesota Vikings", "platform_id": "MIN", "position": "DEF", "team": "MIN", "is_starter": True},
            {"name": "Jared Goff", "platform_id": "210", "position": "QB", "team": "DET", "is_starter": False},
            {"name": "C.J. Stroud", "platform_id": "324", "position": "QB", "team": "HOU", "is_starter": False},
            {"name": "Chuba Hubbard", "platform_id": "310", "position": "RB", "team": "CAR", "is_starter": False},
            {"name": "Jaylen Waddle", "platform_id": "44", "position": "WR", "team": "MIA", "is_starter": False},
            {"name": "Chris Olave", "platform_id": "294", "position": "WR", "team": "NO", "is_starter": False},
            {"name": "Tank Dell", "platform_id": "327", "position": "WR", "team": "HOU", "is_starter": False}
        ]
        ...
        for player_data in current_roster:
            ...
            # Get player ID
            player_result = db.execute(text("SELECT id FROM players WHERE platform_player_id = :platform_id"), 
        ...
        # Show summary
        roster_count = db.execute(text("SELECT COUNT(*) FROM roster_spots WHERE team_id = :team_id"), 
