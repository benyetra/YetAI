            ...
            if position == "goalies":
                for goalie in players:
                    stats = {
                        ...
                        "opponent_team_name": home_team.get("name", {}).get(
                            "default", ""
                        ),
            ...
            if position == "goalies":
                for goalie in players:
                    stats = {
                        ...
                        "opponent_team_name": away_team.get("name", {}).get(
                            "default", ""
                        ),
                        "actual_saves": goalie.get("saves", 0),
                        "actual_shots_against": goalie.get("shotsAgainst", 0),
...
def match_with_predictions(goalie_stats):
    ...
    for stats in goalie_stats:
        ...
        if prediction:
            ...
            if prediction.saves_line:
                ...
                actual_data["betting_recommendation"] = (
                ...
                if (
                    prediction.betting_recommendation
                    and prediction.betting_recommendation != "PASS"
                ):
            ...
            print(
                f"✅ {goalie_name}: {actual_saves} saves (predicted {predicted_saves:.1f})"
            )
            if actual_data.get("betting_recommendation"):
                rec_result = "✅" if actual_data.get("recommendation_correct") else "❌"
                print(
                    f"   Recommendation: {prediction.betting_recommendation} ({prediction.edge_category}) {rec_result}"
                )
        else:
    ...
    if results_processed > 0:
        ...
        all_results = (
            db_session.query(NHLGoalieActuals)
            .filter(NHLGoalieActuals.predicted_saves.isnot(None))
            .all()
        )
        ...
        if rec_results:
            ...
            print(
                f"  Recommendation Success Rate: {correct_recs}/{total_recs} ({rec_accuracy:.1f}%)"
            )
            ...
            for edge in ["HIGH", "MEDIUM", "LOW"]:
                ...
                if edge_results:
                    edge_correct = sum(
                        1 for r in edge_results if r.recommendation_correct
                    )
                    edge_total = len(edge_results)
                    edge_pct = (
                        (edge_correct / edge_total * 100) if edge_total > 0 else 0
                    )
                    print(
                        f"    {edge} Edge: {edge_correct}/{edge_total} ({edge_pct:.1f}%)"
                    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--date", type=str, help="Date to process (YYYY-MM-DD), defaults to yesterday"
    )
