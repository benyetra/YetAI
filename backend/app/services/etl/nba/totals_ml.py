"""NBA game totals ML — residual GBM on heuristic baseline (BKB-2.5)."""
...
def features_from_projection(projection: dict[str, Any]) -> dict[str, float]:
    ...
    return {
        ...
        "market_total": float(market) if market is not None else 0.0,
    }


def totals_ml_enabled() -> bool:
    return os.getenv("NBA_TOTALS_ML_ENABLED", "").strip().lower() in _TRUTHY


def _local_model_paths() -> tuple[Path, Path] | None:
    base = os.getenv("NBA_TOTALS_MODEL_LOCAL", "").strip()
    if not base:
        return None
    root = Path(base)
...
def _ensure_loaded() -> bool:
    ...
    with _LOCK:
        ...
        try:
            ...
            if local is not None:
                ...
                if not model_path.is_file() or not meta_path.is_file():
                    raise FileNotFoundError(
                        f"NBA totals model missing under {model_path.parent}"
                    )
            ...
            logger.info("NBA totals ML model unavailable: %s", exc)
            _LOAD_FAILED = True
            return False
...
def enrich_projection(projection: dict[str, Any]) -> dict[str, Any]:
    ...
    Production ``projected_total`` stays heuristic unless NBA_TOTALS_ML_ENABLED=1
    ...
    if ml_total is not None:
        logger.info(
            "NBA totals ML shadow %s @ %s: heuristic=%.1f ml=%.1f residual=%+.1f",
