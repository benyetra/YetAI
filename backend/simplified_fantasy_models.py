...
"""
from sqlalchemy import Column, Integer, String, Float, Boolean, DateTime, Text, ForeignKey, JSON, Enum, Table
...
class League(Base):
    ...
    platform_league_id = Column(String(255), nullable=False, index=True)  # Sleeper league ID
    ...
    roster_positions = Column(JSON)  # ["QB", "RB", "RB", "WR", "WR", "TE", "FLEX", "K", "DEF", "BENCH"...]
...
class Team(Base):
    ...
    # Platform info
    platform_team_id = Column(String(255), nullable=False)  # Sleeper roster ID (1, 2, 3...)
    platform_owner_id = Column(String(255))  # Sleeper owner ID
    
    # Team details
    name = Column(String(255), nullable=False)
    owner_name = Column(String(255))
    is_user_team = Column(Boolean, default=False, index=True)  # TRUE for the authenticated user's team
    ...
    roster_spots = relationship("RosterSpot", back_populates="team", cascade="all, delete-orphan")
...
class Player(Base):
    ...
    # Platform info
    platform = Column(Enum(FantasyPlatform), default=FantasyPlatform.SLEEPER)
    platform_player_id = Column(String(255), nullable=False, unique=True, index=True)  # Sleeper player ID
