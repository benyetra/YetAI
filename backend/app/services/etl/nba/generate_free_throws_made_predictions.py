"""Generate per-player free-throws-made projections via the XGBoost model.

Mirrors generate_points_predictions.py but targets the free_throws_made model
and writes to pred_free_throw_predictions. FreeThrowPredictions uses the
column `predicted_free_throws` (not `projected_*`) and has no FanDuel columns.
"""
    ...
    try:
        ...
        for player in active:
            try:
                ...
                existing = (
                    db.query(FreeThrowPredictions)
                    .filter(
                        FreeThrowPredictions.date == today,
                        FreeThrowPredictions.player_id == player.player_id,
                    )
                    .first()
                )
                if existing:
                    existing.predicted_free_throws = projected
                    ...
                    db.add(
                        FreeThrowPredictions(
                            ...
                            predicted_free_throws=projected,
                        )
                    )
                    created += 1
                db.commit()

                logger.info("%s -> %.2f ftm", player.player_name, projected)
            except Exception:
                logger.exception(
