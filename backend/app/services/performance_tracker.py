...
class PerformanceTracker:
    ...
    async def get_performance_metrics(
        ...
        try:
            ...
            resolved_bets = [
                bet for bet in recent_bets if bet["status"] in ["won", "lost"]
            ]
            ...
            for bet_type_key, bet_type_name in bet_type_mapping.items():
                type_all_bets = [
                    bet for bet in recent_bets if bet.get("bet_type") == bet_type_key
                ]
                type_resolved_bets = [
                    bet for bet in type_all_bets if bet["status"] in ["won", "lost"]
                ]
                ...
                if type_all_bets:
                    ...
                    type_metrics[bet_type_name] = {
                        ...
                        "win_rate": (
                            (len(type_won) / len(type_resolved_bets)) * 100
                            if type_resolved_bets
                            else 0
                        ),
            ...
            for sport, sport_bets in sport_metrics.items():
                sport_resolved = [
                    bet for bet in sport_bets if bet["status"] in ["won", "lost"]
                ]
                ...
                sport_breakdown[sport] = {
                    ...
                    "win_rate": (
                        (len(sport_won) / len(sport_resolved)) * 100
                        if sport_resolved
                        else 0
                    ),
                    "total_wagered": round(total_wagered, 2),
                    ...
                    "net_profit": round(
                        ...
                        - sum(
                            bet.get("amount", 0)
                            for bet in [
                                b for b in sport_resolved if b["status"] == "lost"
                            ]
                        ),
                        2,
                    ),
            ...
            week_resolved = [
                bet for bet in week_bets if bet["status"] in ["won", "lost"]
            ]
            week_won = [bet for bet in week_resolved if bet["status"] == "won"]
            week_accuracy = (
                (len(week_won) / len(week_resolved) * 100)
                if week_resolved
    ...
    async def get_best_predictions(self, limit: int = 15) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            best_predictions = [
                ...
                and p["accuracy_score"]
                and p["accuracy_score"] >= 75
            ]
                ...
                formatted_predictions.append(
                    {
                        ...
                        "days_ago": (
                            datetime.now() - datetime.fromisoformat(pred["timestamp"])
                        ).days,
                    }
                )
    ...
    async def simulate_historical_data(self) -> None:
        ...
        try:
            ...
            for i in range(100):
                ...
                actual_points = round(
                    predicted_points * accuracy_factor + random.uniform(-3, 3), 1
                )
                ...
                accuracy_score = max(
                    0, round(100 - (difference / predicted_points * 100), 1)
                )
                ...
                prediction = {
                    ...
                    ).isoformat(),  # Predicted day before
                    "type": "fantasy",
    ...
    def _generate_mock_best_predictions(self, limit: int) -> List[Dict[str, Any]]:
        ...
        for i in range(len(mock_predictions), min(limit, 15)):
            ...
            accuracy = max(
                80, round(100 - abs(predicted - actual) / predicted * 100, 1)
            )
