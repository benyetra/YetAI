...
class SleeperFantasyService(FantasyPlatformInterface):
    ...
    async def get_user_leagues(self, platform_user_id: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            logger.error(
                f"Failed to get Sleeper leagues for user {platform_user_id}: {e}"
            )
            return []
        except Exception as e:
        ...
        try:
            async with httpx.AsyncClient() as client:
                ...
                for user in users_data:
                    ...
                    logger.debug(
                        f"Added user to lookup: {user_id} -> {user.get('username', 'no_username')}"
                    )
                ...
                for i, roster in enumerate(rosters_data):
                    ...
                    logger.debug(
                        f"Processing roster {i}: owner_id={owner_id}, owner_username={owner.get('username', 'Unknown')}"
                    )
                    ...
                    try:
                        team_data = {
                            ...
                            or owner.get("username", "Unknown"),
                            "owner_id": roster["owner_id"],
                            "wins": roster.get("settings", {}).get("wins", 0),
                            "losses": roster.get("settings", {}).get("losses", 0),
                            "ties": roster.get("settings", {}).get("ties", 0),
                            "points_for": float(
                                roster.get("settings", {}).get("fpts", 0)
                            ),
                            "points_against": float(
                                roster.get("settings", {}).get("fpts_against", 0)
                            ),
                            "waiver_position": roster.get("settings", {}).get(
                                "waiver_position"
                            ),
                        ...
                        logger.error(
                            f"Error processing team data for roster {i}: {team_error}"
                        )
                    ...
                    logger.debug(
                        f"Processed team: {team_data['name']} (ID: {team_data['team_id']})"
                    )

                logger.info(
                    f"Successfully processed {len(teams)} teams for league {league_id}"
                )

                # Determine waiver settings
                waiver_settings = self._determine_waiver_settings(league_data)
            ...
            logger.error(
                f"Failed to get Sleeper league details for {league_id}: {str(e)}"
            )
            import traceback
    ...
    async def get_team_roster(
        ...
        logger.warning(
            "get_team_roster called without league context - not fully supported"
        )
        return []
    ...
    async def get_league_standings(self, league_id: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            logger.info(
                f"Generated standings for league {league_id}: {len(standings)} teams"
            )
            return standings
    ...
    async def get_roster_by_owner(self, league_id: str, owner_id: str) -> List[str]:
        ...
        try:
            ...
            if user_team:
                ...
                logger.info(
                    f"Found roster for owner {owner_id} in league {league_id}: {len(roster)} players"
                )
                return roster
            else:
                logger.warning(
                    f"No team found for owner {owner_id} in league {league_id}"
                )
                return []

        except Exception as e:
            logger.error(
                f"Failed to get roster for owner {owner_id} in league {league_id}: {str(e)}"
            )
            return []
    ...
    async def get_league_matchups(
        ...
        try:
            ...
            logger.error(
                f"Failed to get Sleeper matchups for league {league_id}, week {week}: {str(e)}"
            )
            return []
    ...
    async def get_available_players(self, league_id: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            logger.error(
                f"Failed to get available players for league {league_id}: {str(e)}"
            )
            return []

    async def get_trending_players(
        self, trend_type: str = "add"
    ) -> List[Dict[str, Any]]:
    ...
    async def _get_all_players(self) -> Dict[str, Any]:
        ...
        if (
            ...
            and (now - self._players_cache_time).seconds < 86400  # 24 hours
    ...
    def _determine_waiver_settings(self, league_data: Dict[str, Any]) -> Dict[str, Any]:
        """Determine waiver settings from league data"""
        settings = league_data.get("settings", {})

        # Check for FAAB budget
        waiver_budget = settings.get("waiver_budget", 0)
        waiver_clear_days = settings.get("waiver_clear_days", 1)
        waiver_type = settings.get(
            "waiver_type", 0
        )  # 0=rolling, 1=reverse standings, 2=FAAB

        # Determine waiver system - only waiver_type 2 is FAAB
        if waiver_type == 2:
            waiver_system = "FAAB"
        else:
            waiver_system = "waiver_priority"

        return {
            "waiver_type": waiver_system,
            "waiver_budget": waiver_budget if waiver_budget > 0 else None,
            "waiver_clear_days": waiver_clear_days,
        }

    async def get_historical_league_data(
        self, league_id: str, season: int
    ) -> Dict[str, Any]:
        """Get historical data for a league season"""
        try:
            async with httpx.AsyncClient() as client:
                # Get league details for historical season
                league_response = await client.get(
                    f"{self.base_url}/league/{league_id}"
                )
                league_response.raise_for_status()
                ...
                # Get rosters for historical season
                rosters_response = await client.get(
                    f"{self.base_url}/league/{league_id}/rosters/{season}"
                )
                rosters_response.raise_for_status()
                rosters_data = rosters_response.json()

                # Get all transactions for historical season
                all_transactions = []
                for week in range(1, 19):  # NFL weeks 1-18
                    try:
                        ...
                        week_transactions = trans_response.json()
                        all_transactions.extend(week_transactions)
                        await asyncio.sleep(0.1)  # Rate limiting
                    except Exception as e:
                        logger.debug(f"No transactions for week {week}: {e}")
                        continue
                ...
                # Process teams data with standings
                teams_data = []
                for roster in rosters_data:
                    # Find corresponding user
                    owner_id = roster.get("owner_id")
                    owner = next(
                        (u for u in users_data if u["user_id"] == owner_id), {}
                    )

                    teams_data.append(
                        {
                            "roster_id": roster["roster_id"],
                            "team_name": self._get_team_name(roster, owner),
                            "owner_id": owner_id,
                            "owner_name": owner.get(
                                "display_name", owner.get("username", "Unknown")
                            ),
                            "wins": roster.get("settings", {}).get("wins", 0),
                            "losses": roster.get("settings", {}).get("losses", 0),
                            "ties": roster.get("settings", {}).get("ties", 0),
                ...
                # Sort by wins, then points for standings
                standings = sorted(
                    teams_data, key=lambda x: (-x["wins"], -x["points_for"])
                )
                for i, team in enumerate(standings):
                    team["final_rank"] = i + 1
            ...
            logger.error(
                f"Failed to get historical data for league {league_id}, season {season}: {str(e)}"
            )
            return {}
    ...
    def _get_team_name(self, roster: Dict[str, Any], owner: Dict[str, Any]) -> str:
        ...
        display_name = owner.get("display_name") or owner.get(
    ...
    async def get_league_transactions(
        self, league_id: str, week: Optional[int] = None
    ) -> List[Dict[str, Any]]:
    ...
    async def get_league_traded_picks(self, league_id: str) -> List[Dict[str, Any]]:
        ...
        try:
            async with httpx.AsyncClient() as client:
                response = await client.get(
                    f"{self.base_url}/league/{league_id}/traded_picks"
                )
                response.raise_for_status()
                return response.json() or []
        except Exception as e:
            logger.warning("Failed to get traded picks for league %s: %s", league_id, e)
            return []
