...
def _summarize_folds(folds: list[dict]) -> tuple[dict, dict]:
    summ = {
        'mean_test_win_brier_model': float(np.mean([f['model']['win_brier'] for f in folds])),
        ...
        'mean_test_total_mae_model': float(np.mean([f['model']['total_mae'] for f in folds])),
...
def _evaluate_holdout_fold(
    ...
    *,
    ...
    if cal_train_mode == CAL_TRAIN_SPLIT:
        train_fit_df, _ = split_calibration_holdout(train_df, cal_val_fraction, min_cal_rows)
    ...
    win_e, total_e = train_game_models(
        train_fit_df, fast=fast_train, tune_weights=tune_weights,
    )
    ...
    cal_method, calibrator = fit_probability_calibrator(
        p_cal_raw, y_cal, method=calibration_method,
    )
    ...
    baseline_mae_mean = float(mean_absolute_error(tot_te, np.full_like(tot_te, tot_fit.mean())))
...
def run_seasonal_holdout(
    ...
    with app.app_context():
        ...
        if df.empty:
            raise RuntimeError('No training rows produced; check seasons and API access.')
        ...
        if folds:
            ...
            if len(modes) > 1:
                ...
                for mode in modes:
                    mode_folds = [f for f in folds if f.get('calibration_train_mode') == mode]
                    if mode_folds:
...
def main(argv=None):
    ...
    def _print_summary(label: str, summ: dict, verdict: dict) -> None:
        if not summ:
            return
        print(f'\n=== Summary ({label}) ===')
        print(f"  Model win Brier (raw):            {summ['mean_test_win_brier_model']:.4f}")
        print(f"  Model win Brier (calibrated):     {summ['mean_test_win_brier_calibrated']:.4f}")
        print(f"  Baseline win Brier (0.5):         {summ['mean_test_win_brier_baseline_0.5']:.4f}")
        if verdict:
            print(f"  calibration_improves_brier:       {'yes' if verdict.get('calibration_improves_brier') else 'no'}")
