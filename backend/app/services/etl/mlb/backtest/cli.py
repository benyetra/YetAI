...
Examples:
  ...
  python scripts/mlb/backtest.py --compare abc12345
    ...
    # Date range
    parser.add_argument('--start-date', type=str, default='2024-03-28',
    ...
    parser.add_argument('--full-season', action='store_true',
                        help='Backtest every game in range')
    parser.add_argument('--random-dates', action='store_true',
    ...
    parser.add_argument('--team', type=str, default=None,
                        help='Filter to games involving this team')
    parser.add_argument('--month', type=int, default=None,
                        help='Filter to games in this month (1-12)')
    parser.add_argument('--include-postseason', action='store_true',
                        help='Include postseason games')
    ...
    parser.add_argument('--compare', type=str, default=None,
                        help='Backtest ID to compare against')

    # Output
    parser.add_argument('--output-dir', type=str, default=None,
                        help='Directory for CSV output')
    parser.add_argument('--verbose', action='store_true',
                        help='Print per-game predictions and results')

    # Performance
    parser.add_argument('--skip-odds', action='store_true',
                        help='Skip historical odds fetch (faster)')
    parser.add_argument('--skip-weather', action='store_true',
                        help='Use estimated weather for all games (faster)')
    parser.add_argument('--cache-only', action='store_true',
                        help='Run only from cached data')
    ...
    parser.add_argument('--quick', action='store_true',
                        help='Quick preset: 20 games, skip odds/weather')
...
def run_backtest(args):
    ...
    from app.services.etl.mlb.backtest.report import (
        generate_summary_report, write_csv_report, generate_comparison_report
    )
    ...
    for i, game in enumerate(games):
        if args.verbose:
            logger.info(f"[{i+1}/{len(games)}] {game.away_name} @ {game.home_name} "
                        f"({game.game_date})")
        ...
        if args.verbose:
            logger.info(f"  Predicted: WP={prediction['predicted_home_wp']:.1%} "
        ...
        if 'hits' in models_to_test:
            hit_predictions = model_runner.predict_hits(game, metadata.get('lineup_data', {}), metadata)
        ...
        if args.verbose:
            logger.info(f"  Actual: {game.away_name} {actuals['away_score']} - "
    ...
    if args.verbose and leakage_audit_games:
        ...
        for audit in leakage_audit_games:
            ...
            status = "CLEAN"
            logger.info(f"  Game {audit['game_id']} ({audit['game_date']}): "
    ...
    csv_path = write_csv_report(
        backtest_id, scorer.game_results,
    ...
    logger.info(f"Cache: {cache_stats['total_entries']} entries across "
...
def _run_comparison(compare_id, current_config, current_metrics):
    ...
    try:
        ...
        label_a = prev.get("model_version") or prev.get("config", {}).get("model_version", "previous")
