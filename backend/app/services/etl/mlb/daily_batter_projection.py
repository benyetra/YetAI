...
from app.services.etl.mlb.hits import fetch_days_hitters, get_game_log_date, get_game_logs, calculate_metrics_actuals_v_projections

from app.models.predictions_models import HitActuals, HomerActuals, ProjectedHits, ProjectedHomers, Hitter, Homer
import logging
import argparse
...
def store_projections(date):
    ...
    for hitter in hitters:
        ...
        existing_hit_projection = db_session.query(ProjectedHits).filter_by(date=date).filter_by(batter_id=batter_id).first()
    ...
    for homer in homers:
        ...
        existing_homer_projection = db_session.query(ProjectedHomers).filter_by(date=date).filter_by(batter_id=batter_id).first()
...
def store_actuals(date):
    ...
    for hitter in hitters:
        ...
        hit_projection = db_session.query(ProjectedHits).filter_by(date=date, batter_id=batter_id).first()
        homer_projection = db_session.query(ProjectedHomers).filter_by(date=date, batter_id=batter_id).first()
        if not hit_projection or not homer_projection:
            continue

        # Store actual hit results in HitActuals
        hit_actual = db_session.query(HitActuals).filter_by(date=date, batter_id=batter_id).first()
        if hit_actual:
            hit_actual.actual_hits = hits
            hit_actual.projected_hits = hit_projection.projected_hits  # Set the projected_hits from the projection
            hit_actual.correct_prediction = hits >= 1  # Determine if the prediction was correct
        else:
        ...
        homer_actual = db_session.query(HomerActuals).filter_by(date=date, batter_id=batter_id).first()
        if homer_actual:
            homer_actual.actual_homers = homers
            homer_actual.projected_homers = homer_projection.projected_homers  # Set the projected_homers from the projection
            homer_actual.correct_prediction = homers >= 1  # Determine if the prediction was correct
        else:
            new_homer_actual = HomerActuals(
                ...
                correct_prediction=homers >= 1  # Determine if the prediction was correct
            )
