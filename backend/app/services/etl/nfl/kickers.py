        ...
        if response.status_code == 200:
            ...
            for event in schedule_data.get('events', []):
                try:
                    ...
                    if game_time >= current_time or event.get('week', {}).get('number', 0) >= current_week:
            ...
            print(f"Failed to fetch schedule for team {team_id}: {response.status_code}")
            return None
...
def get_team_starting_kicker(team_id, season):
    ...
    if response.status_code != 200:
        print(f"Failed to fetch depth chart for team {team_id} in season {season}: {response.status_code}")
        return None
        ...
        if "pk" in positions:
            ...
            if place_kicker_position["athletes"]:
                ...
                if athlete_response.status_code == 200:
                    ...
                    print(f"Failed to fetch athlete details from {athlete_ref}: {athlete_response.status_code}")
                    return None
...
def get_kicker_career_stats(player_id):
    ...
    if response.status_code == 200:
        ...
        print(f"Failed to fetch career stats for player {player_id}: {response.status_code}")
        return None
...
def get_kicker_stats(player_id):
    ...
    if response.status_code == 200:
        ...
        print(f"Failed to fetch player stats for player {player_id}: {response.status_code}")
        return {}
stat_labels = {
...
def get_kicker_game_stats(player_id, season_year):
    ...
    if response.status_code == 200:
        ...
        print(f"Failed to fetch game stats for player {player_id} in season {season_year}: {response.status_code}")
        return []
...
def get_3rd_down_conversion_rate(team_id):
    ...
    if stats:
        try:
            third_down_conversion_rate = stats['splits']['categories'][10]['stats'][15].get('value', 0)
            redzone_touchdown_pct = stats['splits']['categories'][10]['stats'][13].get('value', 0)
            redzone_field_goal_pct = stats['splits']['categories'][10]['stats'][11].get('value', 0)
            return {
...
def process_kicker_data(kicker, team_name, opponent_name, game_time, venue_name):
    ...
    if not team_stats or 'splits' not in team_stats or 'categories' not in team_stats['splits']:
    ...
    career_surface_stats = next((category.get('splits', []) for category in career_stats if category.get('displayName') == 'Surface'), [])
    career_location_stats = next((category.get('splits', []) for category in career_stats if category.get('displayName') == 'Location'), [])
    career_field_position_stats = next((category.get('splits', []) for category in career_stats if category.get('displayName') == 'Field Position'), [])
    ...
    if not opponent_team_stats or 'splits' not in opponent_team_stats or 'categories' not in opponent_team_stats['splits']:
    ...
    kicker_data = {
        ...
        "game_time": game_time,
        "team_red_zone_efficiency": team_stats['splits']['categories'][10].get('stats', [{}])[10].get('value', 0),
        "opponent_red_zone_efficiency": opponent_team_stats['splits']['categories'][10].get('stats', [{}])[10].get('value', 0),
    ...
    try:
        from app.services.etl.nfl.statistical_kicker_prediction import calculate_enhanced_statistical_score
        ...
        try:
            from weather_integration import get_live_weather_for_venue, get_venue_type, get_surface_type
            ...
            print(f"🌡️ Weather for {venue_name}: {weather_info['temperature']}°F, {weather_info['wind_speed']}mph wind ({weather_info['source']})")
...
def save_kicker_data(kicker_data):
    ...
    """
    game_date = kicker_data['game_time'].date() if isinstance(kicker_data['game_time'], datetime) else kicker_data['game_time']

    # 1. Save to current Kickers table (for current week display)
    kicker = db_session.query(Kickers).filter_by(player_id=int(kicker_data['player_id'])).first()
    if not kicker:
        kicker = Kickers(
            ...
            opponent_red_zone_efficiency=kicker_data.get('opponent_red_zone_efficiency', 0),
            ...
            career_field_position_stats=kicker_data.get('career_field_position_stats', []),
        ...
        kicker.opponent_red_zone_efficiency = kicker_data.get('opponent_red_zone_efficiency', 0)
        kicker.third_down_conversion_rate = kicker_data.get('third_down_conversion_rate', 0)
        ...
        kicker.career_field_position_stats = kicker_data.get('career_field_position_stats', [])
    ...
    existing_prediction = db_session.query(KickerPredictions).filter_by(
    ...
    if not existing_prediction:
        ...
        predicted_success_rate = min(0.95, predicted_fg_made / predicted_fg_attempts if predicted_fg_attempts > 0 else 0.85)

        # Feature importance for model transparency
        feature_importance = {
            'team_red_zone_efficiency': kicker_data.get('team_red_zone_efficiency', 0),
            'weather_impact': (kicker_data.get('wind_speed', 0) * -0.1 + (kicker_data.get('temperature', 70) - 70) * -0.02),
        ...
        historical_prediction.short_distance_prob = min(0.95, predicted_success_rate + 0.1)  # <30 yards
        historical_prediction.medium_distance_prob = predicted_success_rate  # 30-49 yards
        historical_prediction.long_distance_prob = max(0.6, predicted_success_rate - 0.2)  # 50+ yards
        ...
        print(f"📊 Saved historical prediction for {kicker_data['name']} on {game_date}: {predicted_fg_made} FGs")
    else:
        print(f"⚠️ Prediction already exists for {kicker_data['name']} on {game_date}, skipping historical save")
        ...
        for i, kicker in enumerate(kickers, 1):
            if kicker:  # Make sure kicker data exists
                ...
                try:
                    ...
                    teams = get_nfl_teams()
                    team_info = next((t for t in teams if t['id'] == kicker['team_id']), {'name': 'Unknown Team'})

                    # Get game details
                    schedule = get_team_schedule(kicker['team_id'])
                    if schedule and 'events' in schedule and len(schedule['events']) > 0:
                        next_game = schedule['events'][0]
                        game_time = datetime.now() + timedelta(days=1)  # Default to tomorrow
                        ...
                        try:
                            ...
                            venue_name = competition.get('venue', {}).get('fullName', 'Stadium')
                        ...
                        process_kicker_data(kicker, team_info['name'], opponent_name, game_time, venue_name)
                    else:
