...
def get_weather_by_gametime(game_time, stadium):
    ...
    if isinstance(game_time, str):
        game_time_obj = datetime.strptime(game_time, "%Y-%m-%dT%H:%M:%SZ").replace(tzinfo=pytz.UTC)
    else:
    ...
    if response.status_code != 200:
        logging.error(f"Failed to fetch weather for {stadium} at {game_time}: {response.status_code}")
        return None

    weather_data = response.json()
    if "timelines" not in weather_data or "hourly" not in weather_data["timelines"]:
        logging.error(f"Invalid data format for weather API response for {stadium} at {game_time}")
        return None

    forecast = None
    for hourly in weather_data["timelines"]["hourly"]:
        ts = hourly["time"]
        forecast_time = datetime.strptime(ts, "%Y-%m-%dT%H:%M:%SZ").replace(tzinfo=pytz.UTC)
        if forecast_time.hour == game_hour_utc:
            forecast = hourly["values"]
            break

    if not forecast:
        logging.warning(f"No matching hour found in forecast for {stadium} at {game_time}")
        return None
...
def build_weather_csv(output_path: str):
    ...
    for g in games:
        ...
        try:
            game_time_utc = datetime.strptime(dt_str, "%Y-%m-%dT%H:%M:%SZ").replace(tzinfo=pytz.UTC)
