...
class PlayerAnalyticsService:
    ...
    async def get_player_analytics(
        self, player_id: int, weeks: Optional[List[int]] = None, season: int = 2024
    ) -> List[Dict]:
    ...
    async def calculate_usage_trends(
        self, player_id: int, weeks: List[int], season: int = 2024
    ) -> Dict:
        ...
        try:
            ...
            red_zone_shares = [
                a["red_zone_share"] for a in analytics if a["red_zone_share"]
            ]
            ...
            if snap_shares:
                ...
                trends["snap_share_consistency"] = (
                    statistics.stdev(snap_shares) if len(snap_shares) > 1 else 0
                )

            if target_shares:
                trends["target_share_trend"] = self._calculate_trend(target_shares)
                trends["avg_target_share"] = statistics.mean(target_shares)
                trends["target_share_consistency"] = (
                    statistics.stdev(target_shares) if len(target_shares) > 1 else 0
                )
        ...
        try:
            ...
            for week_data in analytics:
                ...
                if week_data.get("points_per_target"):
                    efficiency["points_per_target"].append(
                        week_data["points_per_target"]
                    )
                ...
                if week_data.get("red_zone_efficiency"):
                    efficiency["red_zone_efficiency"].append(
                        week_data["red_zone_efficiency"]
                    )
    ...
    async def identify_breakout_candidates(
        self, position: str, season: int = 2024, min_weeks: int = 3
    ) -> List[Dict]:
        ...
        recent_weeks = list(
            range(
                max(1, self._get_current_week() - min_weeks),
                self._get_current_week() + 1,
            )
        )

        # Query for players with positive trends
        subquery = (
            self.db.query(PlayerAnalytics.player_id)
            .filter(
                PlayerAnalytics.season == season, PlayerAnalytics.week.in_(recent_weeks)
            )
            .group_by(PlayerAnalytics.player_id)
            .having(func.count(PlayerAnalytics.week) >= min_weeks)
        )
        ...
        for player_query in subquery:
            ...
            # Breakout criteria
            if (
                trends.get("snap_share_trend", 0) > 5  # Increasing snap share
                and trends.get("target_share_trend", 0) > 3  # Increasing target share
                and trends.get("avg_snap_share", 0) > 40
            ):  # Meaningful snap share
    ...
    async def get_matchup_specific_analytics(
        self, player_id: int, opponent_team: str, season: int = 2024
    ) -> Dict:
        ...
        matchup_data = (
            ...
            .filter(
                ...
                PlayerAnalytics.season.in_(
                    range(season - 2, season + 1)
                ),  # Last 3 seasons
            )
            .all()
        )
        ...
        return {
            ...
            "avg_fantasy_points": (
                statistics.mean(fantasy_points) if fantasy_points else 0
            ),
            ...
            "boom_rate": (
                len([p for p in fantasy_points if p >= 20]) / len(fantasy_points)
                if fantasy_points
                else 0
            ),
            "bust_rate": (
                len([p for p in fantasy_points if p < 5]) / len(fantasy_points)
                if fantasy_points
                else 0
            ),
    ...
    async def store_weekly_analytics(
        self, player_id: int, week: int, season: int, analytics_data: Dict
    ) -> PlayerAnalytics:
        ...
        if existing:
            ...
            analytics = PlayerAnalytics(
                player_id=player_id, week=week, season=season, **analytics_data
            )
    ...
    def _calculate_breakout_score(self, trends: Dict) -> float:
        ...
        avg_usage = (
            trends.get("avg_snap_share", 0) + trends.get("avg_target_share", 0)
        ) / 2
    ...
    def _calculate_matchup_strength(self, matchup_data: List[PlayerAnalytics]) -> str:
        ...
        avg_points = statistics.mean(
            [d.ppr_points for d in matchup_data if d.ppr_points]
        )
