...
OUTPUT_DIR = os.path.join(
    os.path.dirname(__file__), "..", "..", "..", "..", "..", "scripts", "mlb_backtest_results"
)
...
def generate_summary_report(config, metrics, data_quality_summary):
    ...
    lines.append(f' Model Version: {config.get("model_version", "current")}    '
                 f'Run: {datetime.now().strftime("%Y-%m-%d %H:%M:%S")}')
    lines.append(f' Period: {config.get("start_date", "?")} -> {config.get("end_date", "?")}    '
    ...
    if ou_acc is not None:
        ...
        lines.append(f' O/U Accuracy:             {ou_acc:.1%}         ({ou_correct}/{ou_total} with total comparisons)')
    else:
        lines.append(f' O/U Accuracy:             N/A')

    total_mae = game.get('total_mae')
    lines.append(f' Run Total MAE:            {total_mae:.2f} runs' if total_mae else ' Run Total MAE:            N/A')
    ...
    if calibration:
        ...
        lines.append(' ' + '-' * (w - 2))
        lines.append(f' {"Bucket":<16} {"Predicted":<12} {"Actual":<10} {"Count":<10} {"Status":<10}')
    ...
    if k:
        ...
        lines.append(f' Gold Standard (+/-0.25):   {gold_rate:.1%}         ({gold_count}/{n_pitchers})')

        by_k9 = k.get('by_k9_tier', {})
        if by_k9:
            lines.append(f' By K/9 Tier:              '
    ...
    if hit:
        ...
        lines.append(f' Hit Accuracy (rounded):   {hit.get("hit_accuracy", 0):.1%}         '
                     f'({hit.get("hit_correct", 0)}/{hit.get("n_predictions", 0)})')
        lines.append('')

    # HR predictions
    if hr:
        lines.append(f' HOME RUN PREDICTIONS ({hr.get("n_predictions", 0)} predictions)')
        lines.append(' ' + '-' * (w - 2))
        lines.append(f' Top-10 Hit Rate:          {hr.get("hr_top10_hit_rate", 0):.1%}         '
    ...
    if market and market.get('n_games_with_odds', 0) > 0:
        lines.append(f' MARKET COMPARISON ({market["n_games_with_odds"]} games with odds)')
        lines.append(' ' + '-' * (w - 2))
        delta = market.get('model_vs_market_brier', 0)
        verdict = 'Model Better' if delta < 0 else 'Market Better'
        lines.append(f' Model vs Market Brier:    {delta:+.4f}  ({verdict})')
        lines.append(f' Simulated ROI (edge>3%):  {market.get("simulated_roi", 0):+.1%}  '
    ...
    lines.append(' ' + '-' * (w - 2))
    lines.append(f' Avg Quality Score:        {data_quality_summary.get("avg_quality", 0):.2f}')
    lines.append(f' Games with real weather:  '
                 f'{data_quality_summary.get("real_weather", 0)}/{n} '
                 f'({data_quality_summary.get("real_weather", 0)/max(n,1):.1%})')
    lines.append(f' Games with market odds:   '
                 f'{data_quality_summary.get("with_odds", 0)}/{n} '
                 f'({data_quality_summary.get("with_odds", 0)/max(n,1):.1%})')
    lines.append(f' Games with pitcher data:  '
...
def generate_comparison_report(config, metrics_a, metrics_b, label_a, label_b):
    ...
    header = f' {"Metric":<28} {label_a:<14} {label_b:<14} {"Delta":<10} {"Verdict":<10}'
    lines.append(header)
    lines.append(' ' + '-' * (w - 2))
    ...
    lines.append(_row('ML Accuracy', ga.get('ml_accuracy'), gb.get('ml_accuracy'), '.1%', lower_better=False))
    lines.append(_row('O/U Accuracy', ga.get('ou_accuracy'), gb.get('ou_accuracy'), '.1%', lower_better=False))
    lines.append(_row('Total MAE', ga.get('total_mae'), gb.get('total_mae'), '.2f'))
    lines.append(_row('K MAE', ka.get('k_mae'), kb.get('k_mae'), '.2f'))
    lines.append(_row('K Gold Standard', ka.get('k_gold_standard_rate'), kb.get('k_gold_standard_rate'), '.1%', lower_better=False))
    lines.append(_row('HR Top-10 Hit Rate', hra.get('hr_top10_hit_rate'), hrb.get('hr_top10_hit_rate'), '.1%', lower_better=False))
...
def write_csv_report(backtest_id, game_results, output_dir=None):
    ...
    fieldnames = [
        'game_date', 'home_team', 'away_team', 'venue_name',
        'predicted_home_wp', 'predicted_total', 'predicted_run_line',
        'model_confidence', 'model_type',
        'home_score', 'away_score', 'actual_total', 'actual_winner',
        'ml_correct', 'total_error', 'run_line_correct',
        'brier_contribution', 'data_quality_score',
        'pre_calibration_wp', 'calibration_applied',
        'temperature', 'used_estimated_weather', 'had_historical_odds',
    ]
