...
def apply_probability_calibrator(p: np.ndarray, method: str, calibrator: object) -> np.ndarray:
...
class WinProbabilityCalibrator:
    ...
    def fit(self, raw_probs: np.ndarray, labels: np.ndarray, method: str = 'auto') -> WinProbabilityCalibrator:
        raw_probs = np.clip(np.asarray(raw_probs, dtype=float), 1e-6, 1.0 - 1e-6)
        labels = np.asarray(labels, dtype=int)
        self.method, self.calibrator = fit_probability_calibrator(raw_probs, labels, method=method)
        calibrated = apply_probability_calibrator(raw_probs, self.method, self.calibrator)
...
def save_win_calibrator(calibrator: WinProbabilityCalibrator, path: str | None = None) -> str:
