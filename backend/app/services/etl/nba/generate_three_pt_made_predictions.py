"""Generate per-player 3PT-made projections via the XGBoost model.

Mirrors YetiBets ``three_point_predictions_v2.py`` (ML path) and writes to
``pred_three_point_projections``. FanDuel lines come from The Odds API when
    ...
    try:
        ...
        for player in active:
            try:
                ...
                fd_line, _, fd_flag = 0.0, 0.0, "n"
                event_id = get_event_id_for_game(
                    "basketball_nba", player.team_name, player.opponent_team_name
                )
                if event_id:
                    fd_line, _, fd_flag = get_fanduel_line(
                        "basketball_nba",
                        event_id,
                        player.player_name,
                        "player_threes",
                ...
                existing = (
                    ...
                    .filter(
                        ThreePointProjections.date == today,
                        ThreePointProjections.player_id == player.player_id,
                    )
                    .first()
                )
                if existing:
                    existing.projected_three_pt_made = projected
                ...
                logger.info("%s -> %.2f 3pm", player.player_name, projected)
            except Exception:
                logger.exception(
