...
class BacktestScorer:
    ...
    def add_k_result(self, side, projected_k, actual_k, projected_ip=None, actual_ip=None,
                     pitcher_k9=None, n_starts=None, fanduel_line=None):
    ...
    def compute_game_metrics(self):
        ...
        for r in self.game_results:
            ...
            if proj != actual:  # Skip pushes
                ou_results.append(
                    (proj > actual and r['total_error'] > 0) or
    ...
    def _compute_calibration(self):
        ...
        for low, high, label in bins:
            bucket_results = [
                ...
                if low <= r['predicted_home_wp'] < high
            ]
    ...
    def compute_k_metrics(self):
        ...
        k_vs_line_accuracy = (k_vs_line_correct / len(k_vs_line_results)
                              if k_vs_line_results else None)
        ...
        return {
            ...
            'k_vs_line_accuracy': round(k_vs_line_accuracy, 4) if k_vs_line_accuracy is not None else None,
            'k_vs_line_total': len(k_vs_line_results),
            'by_experience': {k: round(np.mean(v), 2) for k, v in by_experience.items()},
    ...
    def compute_hr_metrics(self):
        ...
        try:
            ...
            if pos and neg:
                concordant = sum(1 for p_pos, _ in pos for p_neg, _ in neg if p_pos > p_neg)
                tied = sum(0.5 for p_pos, _ in pos for p_neg, _ in neg if p_pos == p_neg)
        ...
        for dt, results in by_date.items():
            sorted_by_prob = sorted(results, key=lambda x: x['projected_hr_prob'], reverse=True)
