...
def fetch_historical_matchup_data(player_id, opponent_team_id, limit=5):
    ...
    try:
        historical_games = (
            db.query(RecentGames)
            .filter_by(player_id=player_id, opponent_team_id=opponent_team_id)
            .order_by(RecentGames.game_date.desc())
            .limit(limit)
            .all()
        )
        logger.info(
            f"Fetched {len(historical_games)} historical matchups for player {player_id} against team {opponent_team_id}."
        )
...
def get_advanced_defensive_profile(recent_games):
    ...
    defensive_ratings = [
        g.defensive_rating for g in games_with_data if g.defensive_rating
    ]
    ...
    plus_minus_values = [
        g.plus_minus for g in games_with_data if g.plus_minus is not None
    ]
    if plus_minus_values:
        ...
        plus_minus_factor = 1.0 + (
            max(-20, min(avg_plus_minus, 20)) / 200
        )  # Small adjustment
    else:
        plus_minus_factor = 1.0
    ...
    def_reb_pcts = [
        g.defensive_rebound_percentage
        for g in games_with_data
        if g.defensive_rebound_percentage
    ]
    if def_reb_pcts:
    ...
    logger.info(
        f"Defensive profile: rating={rating_factor:.3f}, reb={rebound_factor:.3f}, "
...
def get_opponent_ball_handling_factor(opponent_team_id, opponent_offense_stats):
    ...
    try:
        ...
        if total_turnovers == 0:
            logger.info(
                f"Opponent team {opponent_team_id} has 0 turnovers (very careful)"
            )
        ...
        ball_handling_factor = LEAGUE_AVG_ATO / max(
            team_ato_ratio, 0.8
        ...
        assists_per_game = (
            opponent_offense_stats.assists_per_game
            if opponent_offense_stats.assists_per_game
            else 25
        )
        ...
        logger.info(
            f"Opponent A/TO ratio: {team_ato_ratio:.2f}, ball_handling_factor: {ball_handling_factor:.3f}"
        )
...
def calculate_weighted_steals_per_min(recent_games):
    ...
    logger.info(
        f"Weighted steals/min: {weighted_steals_per_min:.4f} "
...
def get_combined_pace_factor(player_team_id, opponent_pace):
    ...
    try:
        ...
        team_roster = (
            db.query(TeamRoster).filter_by(team_id=player_team_id).limit(5).all()
        )
        if not team_roster:
        ...
        logger.info(
            f"Combined pace factor: {pace_factor:.3f} "
...
def calculate_no_steals_probability(
    player_career,
    recent_games,
    opponent_offense_stats,
    historical_matchups,
    avg_minutes,
    player_position,
    player_team_id,
    ...
    try:
        ...
        career_steals_per_min = (
            ...
            if player_career and player_career.minutes and player_career.minutes > 0
            else 0
        )

        if recent_steals_per_min == 0:
            logger.warning(
                f"Player has 0 weighted steals/min in recent games, using career average"
            )
        ...
        if (
            not historical_matchups
            or len(historical_matchups) < 2
            or total_matchup_minutes == 0
        ):
            ...
            weight_career = 0.3
            logger.info(
                f"Insufficient matchup data, adjusting weights: career={weight_career}, recent={weight_recent}"
            )
        else:
            # Use weighted average for matchups too
            matchup_steals_per_min = calculate_weighted_steals_per_min(
                historical_matchups
            )
            weight_career = 0.3
            weight_recent = 0.5
            weight_matchup = 0.2
            logger.info(
                f"Using all three sources: career={weight_career}, recent={weight_recent}, matchup={weight_matchup}"
            )
        ...
        logger.info(
            f"Base steals/min: {base_steals_per_min:.4f} "
        ...
        logger.info(
            f"Position: {player_position}, Position factor: {position_factor:.2f}"
        )
        ...
        ball_handling_factor = get_opponent_ball_handling_factor(
            opponent_team_id, opponent_offense_stats
        )
        ...
        opponent_turnovers = (
            opponent_offense_stats.turnovers_per_game
            if opponent_offense_stats.turnovers_per_game
            else LEAGUE_AVG_TURNOVERS
        )
        opponent_pace = (
            opponent_offense_stats.pace
            if opponent_offense_stats.pace
    ...
    for player in players:
        try:
            ...
            if not player_career:
                logger.warning(
                    f"No current season ({current_season}) career data found for player {player.player_id}. Skipping."
                )
                continue
            ...
            if player_career.last_updated:
                days_since_update = (
                    datetime.datetime.now() - player_career.last_updated
                ).days
                if days_since_update > 7:
                    logger.warning(
                        f"Player {player.player_id} career data is {days_since_update} days old. Proceeding with caution."
                    )
            ...
            if not recent_games:
                logger.warning(
                    f"No recent game data found for player {player.player_id}. Skipping."
                )
                continue
            ...
            if days_since_last_game > 30:
                logger.warning(
                    f"Player {player.player_id} hasn't played in {days_since_last_game} days. Skipping."
                )
                continue
            elif days_since_last_game > 7:
                logger.info(
                    f"Player {player.player_id} last played {days_since_last_game} days ago. Proceeding with caution."
                )

            # Calculate average minutes played in recent games
            avg_minutes = sum(game.minutes or 0 for game in recent_games) / len(
                recent_games
            )
            ...
            if avg_minutes < 17:
                logger.info(
                    f"Player {player.player_id} has average minutes {avg_minutes:.2f}, skipping."
                )
                continue

            # Fetch historical matchup data
            historical_matchups = fetch_historical_matchup_data(
                player.player_id, player.opponent_team_id
            )

            # Fetch opponent offensive stats
            opponent_offense_stats = (
                db.query(TeamOffenseStats)
                .filter_by(team_id=player.opponent_team_id)
                .first()
            )
            if not opponent_offense_stats:
                logger.warning(
                    f"No offensive stats found for opponent team {player.opponent_team_id}. Skipping."
                )
                continue
            ...
            if not player_position:
                ...
                roster_entry = (
                    db.query(TeamRoster).filter_by(player_id=player.player_id).first()
                )
                player_position = roster_entry.position if roster_entry else None

            if not player_position:
                logger.warning(
                    f"No position found for player {player.player_id}, using default factor"
                )

            logger.info(f"\n{'='*60}")
            logger.info(
                f"Processing: {player.player_name} ({player_position}) vs {player.opponent_team_name}"
            )
            logger.info(f"{'='*60}")
            ...
            if probability_no_steals <= 0.80:
                logger.info(
                    f"Player {player.player_id} has no steals probability {probability_no_steals:.2f}, skipping."
                )
                continue
            ...
            logger.info(
                f"Player {player.player_id} added with probability {probability_no_steals:.2f} and avg minutes {avg_minutes:.2f}."
            )
        except Exception as e:
...
def save_yesterdays_results():
    ...
    for player in players:
        try:
            ...
            prediction = (
                db.query(NoStealsProjections)
                .filter_by(date=yesterday, player_id=player.player_id)
                .first()
            )
            if not prediction:
                logger.warning(
                    f"No prediction found for player {player.player_id} on {yesterday}. Skipping."
                )
                continue
            ...
            recent_game = (
                db.query(RecentGames)
                .filter_by(player_id=player.player_id, game_date=yesterday)
                .first()
            )
            if not recent_game:
                logger.warning(
                    f"No actual game data found for player {player.player_id} on {yesterday}. Skipping."
                )
                continue
            ...
            confidence_level = (
                "very_high" if prediction.probability_no_steals >= 0.90 else "high"
            )
            ...
            logger.info(
                f"Player {player.player_id} ({player.player_name}): "
...
def run() -> dict:
    ...
    try:
        ...
        count = (
            db.query(NoStealsProjections)
            .filter(NoStealsProjections.date == _today())
            .count()
        )
