...
from app.models.predictions_models import (
    NHLGoaliePredictions, NHLGoalieActuals, NHLPlayer, NHLTeam, NHLGoalie, NHLGameStats
)
...
def generate_goalie_predictions(games, odds_events):
    ...
    for game in games:
        ...
        home_goalie = db_session.query(NHLGoalie).filter(
            NHLGoalie.team_name == home_team,
            NHLGoalie.games_played > 0
        ).order_by(NHLGoalie.games_played.desc()).first()

        away_goalie = db_session.query(NHLGoalie).filter(
            NHLGoalie.team_name == away_team,
            NHLGoalie.games_played > 0
        ).order_by(NHLGoalie.games_played.desc()).first()
        ...
        goalie_lines = {}
        if event_id:
            goalie_lines = get_goalie_saves_odds_for_event(event_id, odds_home, odds_away)
            ...
            try:
                ...
                if 'error' not in prediction:
                    ...
                    db_pred = NHLGoaliePredictions(
                        ...
                        betting_recommendation=prediction.get('betting_recommendation', 'PASS')
                    )
                    db_session.add(db_pred)
                    predictions_generated += 1
                    logging.info(f"✅ {goalie_name}: {prediction['predicted_saves']:.1f} saves predicted")
...
def generate_player_predictions(games, odds_events):
    ...
    for game in games:
        ...
        if 'startTimeUTC' in game:
            from datetime import datetime
            game_time_et = datetime.fromisoformat(game['startTimeUTC'].replace('Z', '+00:00'))
            # Extract the actual game date from the UTC time
        ...
        for team_id, team_name, opponent_name, is_home in [
            ...
            players = db_session.query(NHLPlayer).filter(
                NHLPlayer.team_id == team_id,
                NHLPlayer.shots_per_game > 0
            ).order_by(NHLPlayer.shots_per_game.desc()).limit(5).all()

            logging.info(f"  Found {len(players)} players for {team_name} (team_id={team_id})")
            ...
            for player in players:
                try:
                    ...
                    if 'error' not in pred:
                        ...
                        db_session.add(db_pred)
                        predictions_generated += 1
                        logging.info(f"✅ {pred['player_name']}: {pred['predicted_shots']:.1f} shots predicted")
                    else:
                        logging.warning(f"⚠️  Error in prediction for {player.name}: {pred.get('error')}")
                except Exception as e:
...
def generate_team_totals_predictions(games, odds_events):
    ...
    from app.services.etl.nhl.generate_daily_predictions import get_team_totals_odds_for_event, get_odds_api_team_name
        ...
        try:
            ...
            if 'error' not in pred:
                ...
                if odds_key in odds_lookup:
                    ...
                    totals_odds = get_team_totals_odds_for_event(event_id, odds_home, odds_away)
                ...
                db_session.add(db_pred)
                predictions_generated += 1
                logging.info(f"✅ {away_team} @ {home_team}: {pred['predicted_total_goals']:.1f} total goals predicted")
                logging.info(f"   Home: {pred['predicted_home_goals']:.1f}, Away: {pred['predicted_away_goals']:.1f}")
    ...
    logging.info(f"✅ Generated and saved {predictions_generated} team totals predictions")
    return predictions_generated
...
def collect_actuals_from_completed_games():
    ...
    try:
        ...
        logging.info(f"📊 Found {len(completed_games)} completed games from {yesterday}")
        ...
        for game in completed_games:
            ...
            if 'awayTeam' in box_data['playerByGameStats']:
                for position, players in box_data['playerByGameStats']['awayTeam'].items():
                    if position == 'goalies':
                        for goalie in players:
                            actuals_saved += save_goalie_actual(
                                goalie, game_id, yesterday,
            ...
            if 'homeTeam' in box_data['playerByGameStats']:
                for position, players in box_data['playerByGameStats']['homeTeam'].items():
                    if position == 'goalies':
                        for goalie in players:
                            actuals_saved += save_goalie_actual(
                                goalie, game_id, yesterday,
...
def save_goalie_actual(goalie, game_id, game_date, team_name, opponent_name):
    ...
    try:
        ...
        actual = NHLGoalieActuals(
            ...
            predicted_shots_against=prediction.predicted_shots_against if prediction else None,
            prediction_error=(
        ...
        logging.info(f"✅ {goalie_name}: {actual.actual_saves} saves (predicted: {actual.predicted_saves or 'N/A'})")
        return 1
