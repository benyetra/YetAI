...
from app.models.fantasy_models import (
    FantasyLeague, FantasyTeam, LeagueHistoricalData, CompetitorAnalysis
)
...
class ComprehensiveLeagueSync:
    ...
    async def sync_complete_league_history(
        ...
        try:
            ...
            for season in sorted(all_seasons):
                ...
                if season_data:
                    # Store in database
                    await self._store_historical_data(db, league.id, season, season_data)
            ...
            db.commit()
            logger.info(f"Successfully synced {len(all_seasons)} seasons for league {league_id}")
            return sync_results
    ...
    async def _sync_season_data(self, league_id: str, season: int) -> Optional[Dict[str, Any]]:
        ...
        try:
            async with httpx.AsyncClient(timeout=30.0) as client:
                ...
                # Get draft data
                drafts_response = await client.get(f"{self.base_url}/league/{league_id}/drafts")
    ...
    async def _store_historical_data(
        ...
        existing = db.query(LeagueHistoricalData).filter(
            and_(
        ...
        if existing:
            ...
            historical_data = LeagueHistoricalData(
                ...
                scoring_type=season_data["league_info"].get("scoring_settings", {}).get("rec", 0),
    ...
    async def _build_manager_profiles(
        ...
        for record in historical_records:
            ...
            for transaction in record.transactions_data or []:
                if transaction.get("status") == "complete":
                    ...
                    if transaction.get("adds"):
                        ...
                        for player_id, add_roster_id in transaction.get("adds", {}).items():
                            roster_id = add_roster_id
                            break
                    elif transaction.get("drops"):
                        # Find who made the drop
                        for player_id, drop_roster_id in transaction.get("drops", {}).items():
            ...
            draft_data = record.standings_data.get("draft_picks", []) if record.standings_data else []
    ...
    async def _analyze_manager_behavior(
        ...
        transaction_analysis = self._analyze_transaction_patterns(manager_data["transactions"])
    ...
    def _analyze_transaction_patterns(self, transactions: List[Dict]) -> TransactionPattern:
        """Analyze waiver wire and transaction behavior"""
        
        if not transactions:
            return TransactionPattern(
                waiver_adds_per_season=0, faab_spent_per_season=0,
                high_faab_bid_threshold=0, preferred_positions=[],
                waiver_aggressiveness_score=0, panic_drop_tendency=0,
                season_phase_activity={}, common_position_needs=[],
        ...
        # Calculate averages
        avg_waiver_adds = sum(s["waiver_adds"] for s in season_stats) / len(season_stats) if season_stats else 0
        avg_faab_spent = sum(s["faab_spent"] for s in season_stats) / len(season_stats) if season_stats else 0
        
        # Determine aggressiveness score (0-1)
        aggressiveness = min(1.0, avg_waiver_adds / 20)  # Normalized to 20 adds per season max
    ...
    async def _generate_competitive_insights(
        ...
        for owner_id, profile in manager_profiles.items():
            ...
            if profile.transaction_patterns.faab_conservation_tendency == "conservative":
