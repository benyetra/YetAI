...
class StatisticalKickerPredictor:
    ...
    def load_historical_data(self):
        ...
        try:
            ...
            if os.path.exists(fg_path):
                ...
                print(
                    f"Loaded {len(self.historical_data)} historical field goal attempts"
                )
    ...
    def get_distance_success_rate(self, distance):
        ...
        if len(similar_kicks) > 10:  # Minimum sample size
            ...
            if distance < 30:
                band_data = self.historical_data[
                    self.historical_data["distance_band"] == "<30"
                ]
            elif distance < 40:
                band_data = self.historical_data[
                    self.historical_data["distance_band"] == "30-39"
                ]
            elif distance < 50:
                band_data = self.historical_data[
                    self.historical_data["distance_band"] == "40-49"
                ]
            elif distance < 60:
                band_data = self.historical_data[
                    self.historical_data["distance_band"] == "50-59"
                ]
            else:
                band_data = self.historical_data[
                    self.historical_data["distance_band"] == "60+"
                ]
    ...
    def estimate_field_goal_attempts(self, team_data, weather_data=None):
        ...
        if red_zone_eff > 70:
            ...
            base_attempts *= (
                1.15  # Inefficient teams attempt more FGs (less aggressive)
            )
        ...
        if third_down_rate > 45:
            base_attempts *= (
                0.95  # Good conversion = fewer FG attempts (less aggressive)
            )
        elif third_down_rate < 35:
            base_attempts *= (
                1.08  # Poor conversion = more FG attempts (less aggressive)
            )
    ...
    def predict_kicker_performance(
        self, kicker_data, team_data, weather_data=None, game_context=None
    ):
        """
...
def calculate_enhanced_statistical_score(
    kicker_data, team_data, weather_data=None, game_context=None
):
    """
...
if __name__ == "__main__":
    ...
    test_weather = {"temperature": 45, "wind_speed": 12}
