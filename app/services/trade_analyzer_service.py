...
class TradeAnalyzerService:
    ...
    def propose_trade(
        ...
        try:
            ...
            team1 = (
                ...
                .filter(
                    and_(
                        FantasyTeam.id == proposing_team_id,
    ...
    def evaluate_trade(
        self, trade_id: int, force_refresh: bool = False
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            existing_eval = (
                self.db.query(TradeEvaluation)
                .filter(TradeEvaluation.trade_id == trade_id)
                .first()
            )
    ...
    def _validate_trade_assets(
        ...
        try:
            ...
            if not (
                team1_gives.get("players")
                or team1_gives.get("picks")
                or team1_gives.get("faab", 0) > 0
            ):
                return {"valid": False, "error": "Team 1 must give something"}

            if not (
                team2_gives.get("players")
                or team2_gives.get("picks")
                or team2_gives.get("faab", 0) > 0
            ):
    ...
    def _generate_comprehensive_evaluation(self, trade: Trade) -> Dict[str, Any]:
        ...
        # Calculate grades
        team1_grade = self._calculate_trade_grade(
            team1_analysis, team1_values, team2_values
        )
        team2_grade = self._calculate_trade_grade(
            team2_analysis, team2_values, team1_values
        )
        ...
        key_factors = self._extract_key_factors(
            team1_analysis, team2_analysis, league_context
        )
    ...
    def _get_league_context(self, league_id: int) -> Dict[str, Any]:
        ...
        playoff_race = {
            ...
            "bubble_teams": len(
                [t for t in sorted_teams[playoff_spots : playoff_spots + 2]]
            ),
    ...
    def _get_team_context(self, team_id: int, league_context: Dict) -> Dict[str, Any]:
        ...
        all_teams = (
            self.db.query(FantasyTeam)
            .filter(FantasyTeam.league_id == team.league_id)
            .all()
        )
    ...
    def _calculate_trade_side_value(
        self, assets: Dict, team_context: Dict, league_context: Dict
    ) -> Dict[str, float]:
        """Calculate total value for one side of trade"""
        total_value_given = 0.0
        total_value_received = (
            0.0  # This will be calculated by the opposite perspective
        )

        breakdown = {"players": {}, "picks": {}, "faab": 0}
    ...
    def _select_appropriate_value(
        self, player_value: PlayerValue, league_context: Dict
    ) -> float:
    ...
    def _estimate_player_value(
        self, player: FantasyPlayer, league_context: Dict
    ) -> float:
    ...
    def _calculate_contextual_multiplier(
        ...
        if (
            team_context.get("championship_contender")
            and league_context["trade_deadline_weeks"] <= 3
        ):
    ...
    def _analyze_positional_impact(
        ...
        return {
            ...
            "most_improved_position": max(
                position_summary.keys(),
                key=lambda p: position_summary[p]["net_value_change"],
                default=None,
            ),
            "most_hurt_position": min(
                position_summary.keys(),
                key=lambda p: position_summary[p]["net_value_change"],
                default=None,
            ),
    ...
    def _analyze_roster_construction_impact(
        self, team_id: int, gives: Dict, receives: Dict, team_context: Dict
    ) -> Dict[str, Any]:
    ...
    def _calculate_depth_impact(
        ...
        return {
            ...
            "depth_impact": (
                "Improved"
                if depth_change > 0
                else ("Reduced" if depth_change < 0 else "Neutral")
            ),
        }

    def _calculate_starter_impact(
        self, team_id: int, gives: Dict, receives: Dict
    ) -> Dict[str, Any]:
    ...
    def _grade_roster_construction(
        self, age_impact: Dict, depth_impact: Dict, starter_impact: Dict
    ) -> str:
    ...
    def _analyze_strategic_fit(
        self, gives: Dict, receives: Dict, team_context: Dict, league_context: Dict
    ) -> Dict[str, Any]:
        ...
        return {
            ...
            "strategy_description": self._describe_team_strategy(
    ...
    def _calculate_championship_impact(
        self,
        team_id: int,
        gives: Dict,
        receives: Dict,
        ...
        gives_value = sum(
            self._get_simple_player_value(pid) for pid in gives.get("players", [])
        )
        receives_value = sum(
            self._get_simple_player_value(pid) for pid in receives.get("players", [])
        )
        ...
        probability_change = (
            value_difference * 0.001
        )  # 1 point = 0.1% championship probability
        ...
        return {
            ...
            "championship_impact_description": self._describe_championship_impact(
                probability_change
            ),
        }
    ...
    def _assess_trade_risks(
        self, gives: Dict, receives: Dict, league_context: Dict
    ) -> Dict[str, Any]:
        ...
        if injury_risk["high_risk_players"] > 0:
            risks.append(
                f"Acquiring {injury_risk['high_risk_players']} injury-prone player(s)"
            )
    ...
    def _calculate_overall_benefit(
        self,
        positional_impact: Dict,
        roster_impact: Dict,
        strategic_fit: Dict,
        championship_impact: Dict,
    ) -> float:
        ...
        if positional_impact.get("position_summary"):
            positional_score = sum(
                pos["net_value_change"]
                for pos in positional_impact["position_summary"].values()
            ) / len(positional_impact["position_summary"])

        strategic_score = strategic_fit.get("strategic_alignment_score", 0) * 10
        championship_score = (
            championship_impact.get("championship_probability_change", 0) * 100
        )

        # Combine scores with weights
        overall_score = (
            positional_score * 0.4 + strategic_score * 0.3 + championship_score * 0.3
        )
    ...
    def _calculate_trade_grade(
        self, team_analysis: Dict, team_values: Dict, opponent_values: Dict
    ) -> TradeGrade:
        ...
        strategic_score = team_analysis.get("strategic_fit", {}).get(
            "strategic_alignment_score", 0
        )

        # Risk component
        risk_score = team_analysis.get("risk_assessment", {}).get(
    ...
    def _calculate_fairness_score(
        self, team1_values: Dict, team2_values: Dict
    ) -> float:
    ...
    def _generate_ai_summary(
        ...
        team1_analysis: Dict,
        team2_analysis: Dict,
        team1_grade: TradeGrade,
        team2_grade: TradeGrade,
        fairness_score: float,
    ) -> str:
        """Generate dynamic AI summary of the trade based on actual analysis"""

        team1 = (
            self.db.query(FantasyTeam).filter(FantasyTeam.id == trade.team1_id).first()
        )
        team2 = (
            self.db.query(FantasyTeam).filter(FantasyTeam.id == trade.team2_id).first()
        )

        summary_parts = []

        # Get actual trade values for analysis
        team1_gives_value = sum(
            self._get_simple_player_value(pid)
            for pid in trade.team1_gives.get("players", [])
        )
        team2_gives_value = sum(
            self._get_simple_player_value(pid)
            for pid in trade.team2_gives.get("players", [])
        )
        ...
        if fairness_score >= 90 and value_difference < 5:
            summary_parts.append(
                "This is an excellent trade with nearly equal value exchange."
            )
        elif fairness_score >= 80:
            summary_parts.append(
                "This trade offers good value for both teams with reasonable balance."
            )
        elif fairness_score >= 65:
            summary_parts.append(
                "This trade shows moderate value exchange with some favorability to one side."
            )
        else:
        ...
        for pos, impact in team1_positions.items():
            if abs(impact.get("net_value_change", 0)) > 5:
                direction = (
                    "upgrades"
                    if impact.get("net_value_change", 0) > 0
                    else "downgrades"
                )
        ...
        if significant_changes:
            summary_parts.append(
                " ".join(significant_changes[:2])
            )  # Limit to 2 key changes

        # Strategic fit assessment based on grades
        grade_avg = (
            self._grade_to_numeric(team1_grade) + self._grade_to_numeric(team2_grade)
        ) / 2
        if grade_avg >= 85:
            summary_parts.append(
                "Both teams execute their strategic vision well with this move."
            )
        elif grade_avg >= 70:
            summary_parts.append(
                "The trade reasonably aligns with at least one team's strategy."
            )
        else:
            summary_parts.append(
                "This trade may not optimally serve either team's competitive goals."
            )
    ...
    def _grade_to_numeric(self, grade: TradeGrade) -> float:
        ...
        grade_values = {
            TradeGrade.A_PLUS: 97,
            TradeGrade.A: 93,
            TradeGrade.A_MINUS: 90,
            TradeGrade.B_PLUS: 87,
            TradeGrade.B: 83,
            TradeGrade.B_MINUS: 80,
            TradeGrade.C_PLUS: 77,
            TradeGrade.C: 73,
            TradeGrade.C_MINUS: 70,
            TradeGrade.D: 60,
            TradeGrade.F: 40,
        }
    ...
    def _extract_key_factors(
        self, team1_analysis: Dict, team2_analysis: Dict, league_context: Dict
    ) -> List[Dict[str, Any]]:
        ...
        for i, (team_analysis, team_name) in enumerate(
            [(team1_analysis, "Team 1"), (team2_analysis, "Team 2")], 1
        ):
    ...
    def _calculate_evaluation_confidence(
        self, team1_analysis: Dict, team2_analysis: Dict
    ) -> float:
        ...
        # Limited player data
        if not team1_analysis.get("positional_impact") or not team2_analysis.get(
            "positional_impact"
        ):
            confidence -= 15

        # High risk trades
        team1_risk = team1_analysis.get("risk_assessment", {}).get(
            "overall_risk_score", 0
        )
        team2_risk = team2_analysis.get("risk_assessment", {}).get(
            "overall_risk_score", 0
        )
