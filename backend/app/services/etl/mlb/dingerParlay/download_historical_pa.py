...
def download_historical_pa(
    start_year: int, end_year: int, combined_csv: str, out_dir: str
):
    all_dfs = []
    ...
    if "game_date" not in full_df.columns:
        print(
            "Error: downloaded Statcast data contains no 'game_date'.", file=sys.stderr
        )
        sys.exit(1)
...
if __name__ == "__main__":
    ...
    p.add_argument(
        "--start-year", type=int, required=True, help="First season (e.g. 2013)"
    )
    p.add_argument(
        "--end-year", type=int, required=True, help="Last season (e.g. 2025)"
    )
    p.add_argument(
        "--combined",
        required=True,
    ...
    download_historical_pa(args.start_year, args.end_year, args.combined, args.out_dir)
